...
public class SaleInvoiceUI extends ToftPanel implements BillTableMouseListener,
		ICheckRetVO, ILinkApprove, ILinkQuery, IFreshTsListener,
		IInvoiceListPanel, IInvoiceCardPanel, ILinkMaintain, ILinkAdd,
		IBatchWorker, BillActionListener {
	private boolean UnitePrintFlag = false; // 联合打印标志 add by QuSida 2010-8-7
											// （顺德骏杰）
	...
	private ButtonObject m_boUnitePrint = new ButtonObject("合并打印", "合并打印",
			"合并打印");// 合并打印 add by QuSida 2010-8-6 10:09:58 （顺德骏杰）
	private HashMap invMap = null;// 合并打印使用,存储非应税劳务存货 add by QuSida 2010-8-6
									// 10:09:58 （顺德骏杰）

	/**
	 * 方法功能描述：二次开发扩展，获取插件代理类。 <b>参数说明</b>
	 * 
	 * @return
	...
	public InvokeEventProxy getPluginProxy() {
		try {
			if (null == this.pluginproxy)
				this.pluginproxy = new InvokeEventProxy("so",
						SaleBillType.SaleInvoice, new SOPluginUI(this,
	 ...
	 *            单据ID
	 */
	public SaleInvoiceUI(String pk_corp, String billType, String businessType,
	...
	public void onAfterAction(ButtonObject bo) {
		...
		try {
			PfUtilClient.processAction(bo.getTag(), SaleBillType.SaleInvoice,
					SaleInvoiceTools.getLoginDate().toString(), voInv, voInv
	...
	/**
	 * 方法功能描述：根据列表界面数据显示刷新缓存内容。 <b>参数说明</b>
	 * 
	 * @author fengjb
	...
	private void updateCacheVOByList() {
		...
		getVOCache().setSaleinvoiceVO(voInvoice.getHeadVO().getCsaleid(),
				voInvoice);
	...
	private void updateCacheVOByCard() {
		...
		if (null == getVOCache().getVO_Load(voHead.getCsaleid())) {
			SaleinvoiceVO catchevo = getVOCache().getVO_NotLoad(
					getVOCache().getPos());
			if (null == catchevo || null == catchevo.getHeadVO().getCsaleid()) {
				...
				if (getBillCardPanel().isOutSumMakeInvoice())
					...
					getVOCache().setSaleinvoiceVO(getVOCache().getPos(),
							voInvoice);
			} else {
				getVOCache().addVO(voInvoice);
			}
		...
		// 缓存位置设置到当前VO
		getVOCache().setPos(
				getVOCache().findPos(voInvoice.getHeadVO().getCsaleid()));

	}

	/**
	 * 方法功能描述：销售发票毛利预估功能实现。 <b>参数说明</b>
	 * 
	 * @author fengjb
	...
	private void onPrifit() {
		...
		if (getShowState() == ListShow) {
			...
			// 库存组织名称
			headVO.setCcalbodyname((String) getBillListPanel()
					.getHeadBillModel().getValueAt(row, "ccalbodyname"));
			// 单据类型
			...
			for (int i = 0; i < bodyVOs.length; i++) {
				...
				// 存货ID
				bodyVO.setCinventoryid((String) getBillListPanel()
						.getBodyBillModel().getValueAt(i, "cinventoryid"));
				// 存货编码
				...
				String gg = (String) getBillListPanel().getBodyBillModel()
						.getValueAt(i, "GG");
				gg = gg == null ? "" : gg;
				String xx = (String) getBillListPanel().getBodyBillModel()
						.getValueAt(i, "XX");
				xx = xx == null ? "" : xx;
				// 规格型号
				bodyVO.setSize(gg + xx);
				// 批次
				bodyVO.setCbatchid((String) getBillListPanel()
						.getBodyBillModel().getValueAt(i, "cbatchid"));
				// 数量
				bodyVO.setNumber((UFDouble) getBillListPanel()
						.getBodyBillModel().getValueAt(i, "nnumber"));
				// 净价
				bodyVO.setNnetprice((UFDouble) getBillListPanel()
						.getBodyBillModel().getValueAt(i, "nnetprice"));

				// yt add 2003-11-22
				bodyVO.setCbodycalbodyid((String) getBillListPanel()
						.getBodyBillModel().getValueAt(i, "cadvisecalbodyid"));
				bodyVO
						.setCbodycalbodyname((String) getBillListPanel()
								.getBodyBillModel().getValueAt(i,
										"cadvisecalbodyname"));
				bodyVO.setCbodywarehouseid((String) getBillListPanel()
						.getBodyBillModel().getValueAt(i, "cbodywarehouseid"));
				bodyVO
						.setCbodywarehousename((String) getBillListPanel()
								.getBodyBillModel().getValueAt(i,
										"cbodywarehousename"));
				if (getBillListPanel().getBodyBillModel().getValueAt(i,
						"blargessflag") != null
			...
			for (int i = 0; i < bodyVOs.length; i++) {
				nc.vo.so.so006.ProfitItemVO bodyVO = new nc.vo.so.so006.ProfitItemVO();
				// 存货ID
				bodyVO.setCinventoryid((String) getBillCardPanel()
				...
				bodyVO.setSize(gg + xx);
				// 批次
				bodyVO.setCbatchid((String) getBillCardPanel().getBodyValueAt(
						i, "cbatchid"));
				// 数量
				bodyVO.setNumber((UFDouble) getBillCardPanel().getBodyValueAt(
						i, "nnumber"));
				// 净价
				bodyVO.setNnetprice((UFDouble) getBillCardPanel()
						.getBodyValueAt(i, "nnetprice"));

				// yt add 2003-11-22
				bodyVO.setCbodycalbodyid((String) getBillCardPanel()
						.getBodyValueAt(i, "cadvisecalbodyid"));
				bodyVO.setCbodycalbodyname((String) getBillCardPanel()
						.getBodyValueAt(i, "cadvisecalbodyname"));
				bodyVO.setCbodywarehouseid((String) getBillCardPanel()
				...
				if (getBillCardPanel().getBodyValueAt(i, "blargessflag") != null
						&& getBillCardPanel().getBodyValueAt(i, "blargessflag")
								.toString().equals("false"))
					bodyVO.m_blargessflag = new UFBoolean(false);
				else
					bodyVO.m_blargessflag = new UFBoolean(true);
				// 无税金额
				bodyVO.setNmny((UFDouble) getBillCardPanel().getBodyValueAt(i,
						"nmny"));
				bodyVOs[i] = bodyVO;
			}
		...
		try {
			PfUtilClient.processAction(this, ISaleInvoiceAction.Prifit,
					...
					getBillCardPanel().getVO(), voProfit);
		} catch (nc.vo.pub.ValidationException e) {
			showErrorMessage(e.getMessage());
		} catch (Exception e) {
			showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"40060501", "UPP40060501-000069")/* @res "失败！" */);
	...
	private void doUnApproveWork() {
		...
		ShowToolsInThread
				.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
						.getStrByID("40060501", "UPP40060501-000055")/*
																	 * @res
																	 * "正在进行弃审前的准备..."
																	 */);
		try {
			tmapvos = (TreeMap) getBillListPanel()
					.getBatchWorkInvoiceVOs(false);
		...
		if (m_proccdlg.isInterrupted()) {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-000042")/*
																		 * @res
																		 * "弃审操作被用户中断！"
																		 */);
		} else {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-000044")/*
																		 * @res
																		 * "弃审操作结束！"
																		 */);
		}
		...
		if (hSuccess.size() > 0) {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-000054")/*
																		 * @res
																		 * "正在更新界面数据..."
																		 */);
	...
	// //返回的排序VO
	// SortedMap<Integer, SaleinvoiceVO> smapRowAndVO = new TreeMap<Integer,
	// SaleinvoiceVO>();
	// //表体行为空的发票VO
	// HashMap<String, SaleinvoiceVO> hmapIdAndVO = new HashMap<String,
	// SaleinvoiceVO>();
	//      
	// String csaleid = null;
	...
	private void doApproveWork() {
			...
			try {
				...
				if (m_proccdlg.getckHint().isSelected()) {
					sMsg = nc.ui.ml.NCLangRes.getInstance().getStrByID(
							...
							new String[] { ((SaleVO) saleinvoice.getParentVO())
									.getVreceiptcode() });
					if (ShowToolsInThread.showYesNoDlg(m_proccdlg, e
							.getMessage()
							+ "\n"
							+ sMsg
							+ nc.ui.ml.NCLangRes.getInstance().getStrByID(
		...
		if (m_proccdlg.isInterrupted()) {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-000041")/*
																		 * @res
																		 * "审批操作被用户中断！"
																		 */);
		} else {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-000043")/*
																		 * @res
																		 * "审批操作结束！"
																		 */);
		}
	...
	private void setButtonsStateBrowse() {
		...
		int iBillStatus = -1;
		{
			...
			if (haveVOSelected
					&& (iBillStatus == BillStatus.NOPASS || iBillStatus == BillStatus.FREE)) {
				getBtns().m_boModify.setEnabled(true);
				// modify by fengjb 20080918 审批中状态按钮控制改变
			} else if (iBillStatus == BillStatus.AUDITING) {
				// 是否有审批人
				boolean isApprove = voFromPanel.getHeadVO().getCapproveid() == null ? false
						: true;
			...
			// // 对冲生成不可用
			// if (!haveVOSelected
			// || voFromPanel.getHeadVO().getFcounteractflag() ==
			// SaleVO.FCOUNTERACTFLAG_COUNTERACT_GEN) {
			// getBtns().m_boUnite.setEnabled(false);
			// getBtns().m_boUniteCancel.setEnabled(false);
			// }
			// else {
			// // 只有发票总金额>0 才可进行冲减。
			// if(voFromPanel.getHeadVO().getFstatus() == BillStatus.FREE &&
			// voFromPanel.getHeadVO().isLgtZero() &&
			// !voFromPanel.getHeadVO().isStrike()){
		...
		if (!getBtns().m_boModify.isEnabled()
				&& !getBtns().m_boCancel.isEnabled()
				&& !getBtns().m_boBlankOut.isEnabled()
				&& !getBtns().m_boCancelTransfer.isEnabled()
				&& !getBtns().m_boUnite.isEnabled()
				&& !getBtns().m_boUniteCancel.isEnabled()) {
		...
		getBtns().m_boAction.setEnabled(haveVOSelected);
		{
			if (haveVOSelected) {
				...
				// SaleBillType.SaleInvoice,
				// voFromPanel.getHeadVO().getCbiztype(),
				// // NO_BUSINESS_TYPE,
				...
				if (voFromPanel.getHeadVO().isNew()) {
					...
					if (BillStatus.FREE == iBillStatus
							|| BillStatus.NOPASS == iBillStatus) {
						getBtns().m_boSendAudit.setEnabled(true);
						getBtns().m_boUnApprove.setEnabled(false);
			...
			if (getShowState() == ListShow) {
				if (getVOCache().isEmpty()) {
					...
					int selectedNum = getBillListPanel().getHeadTable()
							.getSelectedRowCount();
					getBtns().m_boSelectAll
							.setEnabled(selectedNum != getVOCache().getSize());
		...
		getBtns().m_boAssistFunction.setEnabled(true);
		{
			...
			// if (cfreezeid != null && cfreezeid.toString().trim().length() !=
			// 0) {
			// // 根据单据状态设置单据
		...
		getBtns().m_boAssistant.setEnabled(haveVOSelected);
		{
			if (isInMsgPanel())
				...
				getBtns().m_boOrderQuery.setEnabled(haveVOSelected
						&& !voFromPanel.getHeadVO().isNew());
	...
	private void onBlankOut() {
		...
		if (voInvoice.getHeadVO().isStrike()) {
			if (MessageDialog.showYesNoDlg(this, nc.ui.ml.NCLangRes
					...
					NCLangRes.getInstance().getStrByID("40060501",
							"UPP40060501-000161")/* @res"是否取消合并开票?" */) != MessageDialog.ID_YES) {
				return;
			}

			if (null == voInvoice.getHsSelectedARSubHVO()
					|| voInvoice.getHsSelectedARSubHVO().size() == 0) {
				try {
					// 查询合并开票关系
					Hashtable htstrike = SaleinvoiceBO_Client
							.queryStrikeData(voInvoice.getHeadVO()
									.getPrimaryKey());
					voInvoice.setHsSelectedARSubHVO(htstrike);
				} catch (Exception e) {
					SCMEnv.out("删除时查询合并开票关系出错：" + e.getMessage());
					return;
				}
			}
		}
		...
		try {
			// 插件支持
			getPluginProxy().beforeAction(Action.DELETE,
					new SaleinvoiceVO[] { voInvoice });
			onDelete(voInvoice);
		} catch (Exception e) {
			showErrorMessage(e.getMessage());
			nc.vo.scm.pub.SCMEnv.out(e.getMessage());
		...
		if (getShowState() == ListShow) {
			getBillListPanel().getHeadBillModel().delLine(
					new int[] { getBillListPanel().getHeadTable()
							.getSelectedRow() });
	...
	private void onList() {
		...
		// getBillListPanel().bodyRowChange(new
		// BillEditEvent(getBillListPanel().getHeadTable(),-1,getVOCache().getPos()));

		getBillListPanel().getHeadTable().getSelectionModel()
				.setSelectionInterval(getVOCache().getPos(),
						getVOCache().getPos());

	}

	/**
	 * 方法功能描述：获得发票卡片panel。 <b>参数说明</b>
	 * 
	 * @return
	 * @author fengjb
	...
	/**
	 * 方法功能描述返回当前卡片界面上表体每行上层来源行ID。 <b>参数说明</b>
	 * 
	 * @return
	 * @author fengjb
	...
	public HashSet getUpSourcrBillbid() {
		...
		if (ISaleInvoiceOperState.STATE_EDIT == getOperationState()) {
			SaleinvoiceVO billvo = (SaleinvoiceVO) getBillCardPanel()
	...
	/**
	 * 方法功能描述：销售发票批量自动合并开票功能实现。 <b>参数说明</b>
	 * 
	 * @author fengjb
	...
	private void batchAutoUnit() {
			...
			try {
				...
				if (saleinvoice.getHeadVO().getFstatus().intValue() != BillStatus.FREE)
					throw new BusinessException(nc.ui.ml.NCLangRes
							.getInstance().getStrByID("40060501",
									"UPP40060501-100063")/*
														 * @res "非自由状态，不能合并开票"
														 */);
					...
					throw new BusinessException(nc.ui.ml.NCLangRes
							.getInstance().getStrByID("40060501",
									"UPP40060501-100064")/*
														 ...
														 * "已合并开票，先取消后才会再次合并开票"
														 */);
				saleinvoice = SaleinvoiceBO_Client.autoUniteInvoiceToUI(
						saleinvoice, new ClientLink(ClientEnvironment
								.getInstance()));
				if (saleinvoice.getHsArsubAcct() != null
						&& saleinvoice.getHsArsubAcct().size() > 0) {
		...
		if (m_proccdlg.isInterrupted()) {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-100041")/*
																		 * @res
																		 * "合并开票操作被用户中断！"
																		 */);
		} else {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-100043")/*
																		 * @res
																		 * "合并开票操作结束！"
																		 */);
		}
	...
	private void initialize() {
			...
			bodyMenuItems[ilength - 2].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					nc.ui.scm.pub.report.BillRowNo.addNewRowNo(
							getBillCardPanel(), SaleBillType.SaleInvoice,
							"crowno");
				}
			});
		}
		...
		getBtns().m_boPrintManage.addChildButton(m_boUnitePrint); // 合并打印 add by
																	// QuSida
																	// 2010-8-6
																	// 10:24:33
																	// (顺德骏杰)
	...
	private void onApprove() {
		...
		try {
			if (SaleInvoiceTools.getLoginDate().before(
					voInvoice.getHeadVO().getDbilldate()))
				throw new BusinessException("审批日期不能小于开票日期");

			// onApproveCheckWorkflow(voInvoice);
			// 填充审批日期、时间
			voInvoice.getHeadVO().setDapprovedate(
					getClientEnvironment().getDate());
			voInvoice.getHeadVO().setDaudittime(
					new UFDateTime(System.currentTimeMillis()));

			hsnewvo = (HashMap) PfUtilClient.processActionFlow(this,
					ISaleInvoiceAction.Approve, SaleBillType.SaleInvoice,
					getClientEnvironment().getDate().toString(), voInvoice,
					null);
		} catch (Exception e) {
			showErrorMessage(e.getMessage());
			SCMEnv.out(e.getMessage());
		...
		if (getShowState() == ListShow)
			...
			status = SmartVODataUtils.getInteger(getBillCardPanel()
					.getHeadItem("fstatus").getValueObject());
	...
	private void onUnApprove() {
		...
		try {
			...
			hsnewvo = (HashMap) PfUtilClient.processActionFlow(this,
					ISaleInvoiceAction.UnApprove, SaleBillType.SaleInvoice,
					getClientEnvironment().getDate().toString(), voInvoice,
					null);
		...
		if (PfUtilClient.isSuccess()) {
			...
			showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"scmcommon", "UPPSCMCommon-000184")/* @res "弃审成功！" */);
		}
	}
	...
	private void onATP() {
		try {
			PfUtilClient.processActionNoSendMessage(this,
					ISaleInvoiceAction.Atp, SaleBillType.SaleInvoice,
					getClientEnvironment().getDate().toString(),
					getBillCardPanel().getVO(), getBillCardPanel()
							.getVOOnlySelectedRow(), null, null);
		} catch (Exception e) {
			SCMEnv.out(e.getMessage());
			showErrorMessage(e.getMessage());
		}
	...
	private void onCustInfo() {
		...
		try {
			PfUtilClient.processActionNoSendMessage(this,
					ISaleInvoiceAction.CustInfo,
					nc.ui.scm.so.SaleBillType.SaleInvoice,
					getClientEnvironment().getDate().toString(),
					getVOForAction(), ob, null, null);
		} catch (Exception e) {
	...
	private void onCustCredit() {
		...
		if (getShowState() == ListShow) {
			int iselbodyrow = getBillListPanel().getBodyTable()
					.getSelectedRow();
			if (iselbodyrow < 0)
				iselbodyrow = 0;
			...
			if (st.SO_27 != null && st.SO_27.booleanValue()
					&& getBillListPanel().getBodyBillModel().getRowCount() > 0) {
				cproductid = (String) getBillListPanel().getBodyBillModel()
						.getValueAt(0, "cprolineid");
			}
		} else {
			...
			if (st.SO_27 != null && st.SO_27.booleanValue()
				...
				cproductid = (String) getBillCardPanel().getBodyValueAt(0,
						"cprolineid");
			}
		}
	...
	private void onImportTaxCode() {
		...
		try {
			// 根据单据号更新金税票号
			SaleinvoiceBO_Client.updateSaleinvoiceValue(saleVOs,
	...
	private void onSoTax() {
		...
		if (getShowState() == ListShow) {
			SaleinvoiceVO[] saleinvoiceVOs = getBillListPanel()
					.getSelectedVOs();
			for (SaleinvoiceVO saleinvoiceVO : saleinvoiceVOs) {
				goldList.add(saleinvoiceVO.convertGoldTaxVO());
				voMapOfId.put(((SaleVO) saleinvoiceVO.getParentVO())
						.getPrimaryKey(), saleinvoiceVO);
			}
		} else {
			// 直接从卡片界面获取VO即可
		...
		goldTaxDlg.setGoldTaxVOs(goldList
				.toArray(new GoldTaxVO[goldList.size()]));
		if (UIDialog.ID_OK == goldTaxDlg.showModal()) {
			...
			try {
				...
				Map<String, UFDateTime> tsMap = SaleinvoiceBO_Client
						.updateWhenToGoldTax(new ArrayList<String>(voMapOfId
								.keySet()), transTime);
				if (getShowState() == ListShow) {
					...
					for (int row = 0; row < rowCount; row++) {
						String id = (String) getBillListPanel()
								.getHeadBillModel().getValueAt(row, "csaleid");
						if (!StringUtil.isEmpty(id)) {
							UFDateTime ts = tsMap.get(id);
							if (null != ts) {
								getBillListPanel().getHeadBillModel()
										.setValueAt(ts, row, "ts");
								getBillListPanel().getHeadBillModel()
										.setValueAt(UFBoolean.TRUE, row,
												"btogoldtax");
								getBillListPanel().getHeadBillModel()
										.setValueAt(transTime.toString(), row,
												"dtogoldtaxtime");

								SaleinvoiceVO saleinvoiceVO = voMapOfId.get(id);
								if (null != saleinvoiceVO) {
									saleinvoiceVO.getParentVO()
											.setAttributeValue("ts", ts);
									saleinvoiceVO.getParentVO()
											.setAttributeValue("btogoldtax",
													UFBoolean.TRUE);
									saleinvoiceVO.getParentVO()
											.setAttributeValue(
													"dtogoldtaxtime",
					...
					String id = (String) getBillCardPanel().getHeadItem(
					...
					if (!StringUtil.isEmpty(id)) {
						...
						if (null != ts) {
							...
							getBillCardPanel().setHeadItem("btogoldtax",
									UFBoolean.TRUE);
							getBillCardPanel().setHeadItem("dtogoldtaxtime",
									transTime.toString());

							SaleinvoiceVO saleinvoiceVO = voMapOfId.get(id);
							if (null != saleinvoiceVO) {
								saleinvoiceVO.getParentVO().setAttributeValue(
										"ts", ts);
								saleinvoiceVO.getParentVO().setAttributeValue(
										"btogoldtax", UFBoolean.TRUE);
								saleinvoiceVO.getParentVO().setAttributeValue(
										"dtogoldtaxtime", transTime.toString());
								// 更新缓存
								getVOCache()
										.setSaleinvoiceVO(
												((SaleVO) saleinvoiceVO
	...
	/**
	 * 方法功能描述：在新增或者修改时依据当前业务类型设置"参照增行"可参照的。 <b>参数说明</b>
	 * 
	 * @param nowBusitype
	 * @author fengjb
	...
	private void changeRefAddButtonByBusiType(String nowBusitype) {
		...
		BillbusinessVO[] billBusiVOs = PfUIDataCache
				.getSourceByCorpAndBillAndBusi(SaleInvoiceTools
						.getLoginPk_Corp(), SaleBillType.SaleInvoice,
						nowBusitype);
		getBtns().m_boRefAdd.removeAllChildren();
		if (billBusiVOs == null)
			return;
		...
		for (int i = 0; i < billBusiVOs.length; i++) {
			String showName = Pfi18nTools.i18nBilltypeName(billBusiVOs[i]
					.getPk_billtype(), billBusiVOs[i].getBilltypename());
	...
	private void onOutSumMakeInvoice() {
		...
		getBillCardPanel().getHeadItem("finvoicetype").setValue(
	...
	/**
	 * 方法功能描述：销售发票批量取消合并开票功能实现。 <b>参数说明</b>
	 * 
	 * @author fengjb
	...
	private void batchCancelUnit() {
		...
		while (iter.hasNext()) {
			...
			ShowToolsInThread.showMessage(m_proccdlg, nc.ui.ml.NCLangRes
					.getInstance().getStrByID(
							...
							"UPP40060501-100058",
							null,
							new String[] { ((SaleVO) voInvoice.getParentVO())
									.getVreceiptcode() })); /*
															 * proccdlg,
															 * "正在合并开票...[" /* +
															 * ((SaleVO)
															 ...
															 * () /* + "]");
															 */
			voInvoice.setCuruserid(getClientEnvironment().getUser()
					.getPrimaryKey());
			...
			if (sBusinessException != null) {
				...
				if (m_proccdlg.getckHint().isSelected()) {
					sMsg = nc.ui.ml.NCLangRes.getInstance().getStrByID(
							...
							new String[] { ((SaleVO) voInvoice.getParentVO())
									.getVreceiptcode() });
					if (ShowToolsInThread.showYesNoDlg(m_proccdlg,
							sBusinessException
									+ "\n"
									+ sMsg
		...
		if (m_proccdlg.isInterrupted()) {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-100042")/*
																		 * @res
																		 * "合并开票操作被用户中断！"
																		 */);
		} else {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-100044")/*
																		 * @res
																		 * "合并开票操作结束！"
																		 */);
		}
	...
	private void onNew(ButtonObject bo) {
		...
		// 2010-11-02 MeiChao begin
		// 由于如果新增之前卡片为多选状态的话,新增时会出错,修改为新增时,不可多选,新增后VO放置结束,可多选.
	...
	/**
	 * 方法功能描述：设置销售发票表头金额合计字段的数值，设置表体行的冲减前金额。 <b>参数说明</b>
	 * 
	 * @param retvos
	...
	/**
	 * @author jianghp 根据销售发票模板上的默认发票类型来初始化下拉框
	 * @return
	 */
	...
	/**
	 * 方法功能描述：保存发票数据 因有些程序需要返回异常，所以有异常时返回异常。 <b>参数说明</b>
	 * 
	 * @return
	 * @author fengjb
	...
	private String onSave() {
		...
		Object ofcounteractflag = getBillCardPanel().getHeadItem(
				"fcounteractflag").getValueObject();
		Integer fcounteractflag = SmartVODataUtils.getInteger(ofcounteractflag);
		if (getBillCardPanel().isNewBill() && null != fcounteractflag
				&& SaleVO.FCOUNTERACTFLAG_COUNTERACT_GEN == fcounteractflag) {
			...
			try {
				voNewHead = SaleinvoiceBO_Client.queryHeadDataByID(voUp
						.getHeadVO().getCsaleid());
			} catch (Exception e) {
				SCMEnv.out(">>>>>>>保存结束后查询对冲发票来源发票信息异常：" + e);
				sMessage = e.getMessage();
			}

			if (null != voNewHead) {
				voUp.getHeadVO().setTs(voNewHead.getTs());
				voUp.getHeadVO().setFcounteractflag(
						voNewHead.getFcounteractflag());
				voUp.setParentVO(voNewHead);
				getVOCache().setSaleinvoiceVO(voUp.getHeadVO().getCsaleid(),
						voUp);
			}
		}
		// 卡片下刷新界面和缓存数据
	...
	protected ArrayList getFormulaItemHeader() {
		...
		// 表头及表尾涉及到的公式
		String[] aryItemField1 = new String[] { "deptname", "cdeptname",
				"cdeptid" };
		arylistHeadField.add(aryItemField1);

		String[] aryItemField2 = new String[] { "psnname", "cemployeename",
				"cemployeeid" };
	...
	private boolean onApprove(SaleinvoiceVO saleinvoice) throws Exception {
		...
		try {
			if (SaleInvoiceTools.getLoginDate().after(
					saleinvoice.getHeadVO().getDbilldate()))
			...
			if (null != otemp) {
				...
				if (ErrMsg != null && ErrMsg.startsWith("ERR")) {
					ShowToolsInThread.showMessage(m_proccdlg, ErrMsg
							.substring(3));
				}
			}
   ...
   */
	...
	private void onExecQuery() {
		...
		try {
			PfUtilClient.processActionNoSendMessage(this,
					ISaleInvoiceAction.ExecReport,
					nc.ui.scm.so.SaleBillType.SaleInvoice,
					getClientEnvironment().getDate().toString(), voInv, voInv
							.getHeadVO().getCsaleid(), null, null);
		} catch (Exception e) {
			showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"40060501", "UPP40060501-000069")/* @res "失败！" */);
			nc.vo.scm.pub.SCMEnv.out(e.getMessage());
	...
	private void doDeleteWork() {
		...
		try {
			...
			if (null == tmapvos || tmapvos.size() <= 0) {
				showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
						"40060501", "UPP40060501-100071")/* @res "请选择待删除的发票！" */);
				return;
			}
		} catch (Exception ev) {
			showErrorMessage(ev.getMessage());
		...
		if (m_proccdlg.isInterrupted()) {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-100069")/*
																		 * @res
																		 * "删除操作被用户中断！"
																		 */);
		} else {
			ShowToolsInThread
					.showMessage(m_proccdlg, nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060501", "UPP40060501-100070")/*
																		 * @res
																		 * "删除操作结束！"
																		 */);
		}
	...
	private void onBillCombin() {
		...
		SOCollectSettingDlg dlg = new SOCollectSettingDlg(this,
				SaleBillType.SaleInvoice, SaleInvoiceTools.getNodeCode(),
				SaleInvoiceTools.getLoginPk_Corp(), SaleInvoiceTools
						.getLoginUserId(), SaleinvoiceVO.class.getName(),
				SaleVO.class.getName(), SaleinvoiceBVO.class.getName());
		...
		// 获取表体所有存货行
		SaleinvoiceBVO[] allBVOs = (SaleinvoiceBVO[]) this.getBillCardPanel()
				.getBillModel().getBodyValueVOs(SaleinvoiceBVO.class.getName());
		// 如果表体无存货
		if (allBVOs == null || allBVOs.length == 0) {
			MessageDialog.showErrorDlg(this.getParent(), "错误",
					"对不起,该单据并无金额信息,无法合并显示.");
			return;
		}
		// 构建一个表体存货备份数组(从Panel重新获取一次,因为如果直接用原allBVOs数组进行clone,两个数组指向的是同一组地址)
		SaleinvoiceBVO[] allBVOsBackup = (SaleinvoiceBVO[]) this
				.getBillCardPanel().getBillModel().getBodyValueVOs(
						SaleinvoiceBVO.class.getName());
		// 获取表体所有选中存货行
		SaleinvoiceBVO[] selectedBVOs = (SaleinvoiceBVO[]) this
				.getBillCardPanel().getBillModel().getBodySelectedVOs(
						SaleinvoiceBVO.class.getName());
		// 如果表体无选中行
		if (selectedBVOs == null || selectedBVOs.length == 0) {
			...
			// int[]
			// selectedRows=this.getBillCardPanel().getBillModel().getBodyTable().getSelectedRows();
			...
			/**
			 * 开始合并前判断,需要判断的内容有: 1. 所选择的存货行中是否存在非费用信息,如果有,则示警后退出方法.如果无,则累加费用金额.
			...
			for (int i = 0; i < selectedBVOs.length; i++) { // 判断表体所选择存货，应税劳务标示为N时.退出.否则累加金额.
				...
				if (o != null
						&& (((String) o).trim().toUpperCase()).equals("Y")) {
					// 费用合计累加
					expensesum += selectedBVOs[i].getNoriginalcursummny()
							.doubleValue();// 价税合计
				} else if (o != null
						&& (((String) o).trim().toUpperCase()).equals("N")) {
			...
			if (ishaveexpense) {
				MessageDialog.showHintDlg(this.getParent(), "警告",
						"合并显示结果中存在未分摊的费用项.");
			}
			/**
			...
			for (int i = 0, j = 0; i < newBVOs.length; i++) { // 判断表体存货，当表体存货的应税劳务标示为N时.分摊费用.否则略过.
				...
				if (unselectedexpensenum == null
						|| unselectedexpensenum.size() == 0
						|| i != Integer.valueOf(unselectedexpensenum.get(j)
								.toString())) {
					if (unselectedexpensenum == null
							&& unselectedexpensenum.size() == 0
							&& j < unselectedexpensenum.size()) {
						j++;
					}
					// 按照数量占总量百分比分摊费用
					if (newBVOs[i].getNnumber() != null
						...
						newBVOs[i].setNoriginalcursummny(new UFDouble(
								newBVOs[i].getNnumber().doubleValue()
										/ invnumber
										* expensesum
										+ newBVOs[i].getNoriginalcursummny()
												.doubleValue()));// 数量累加.
					}
				}
			}
		...
		Configuration configuation = nc.vo.scm.goldtax.Configuration
				.load(getClientEnvironment().getCorporation().getPk_corp());
		...
		dlg.initData(getBillCardPanel(), fixgroup, null, new String[] {
				...
				"nnumber", "noriginalcurdiscountmny", "ntaxmny", "nmny",
				"nsummny", "ndiscountmny", "nsimulatecostmny", "ncostmny",
				"nsubsummny", "nsubcursummny" }, null, new String[] {
				"nsubtaxnetprice", "nsubqutaxnetpri", "nsubqunetpri",
				"nsubqutaxpri", "nsubqupri", "nqutaxnetprice", "nqunetprice",
				"nqutaxprice", "nquprice", "nqocurtaxnetpri",
				"nquoricurnetpri", "nquoricurtaxpri", "nquoricurpri",
				"ntaxnetprice", "nnetprice", "ntaxprice", "nprice",
				"noriginalcurtaxnetprice", "noriginalcurnetprice",
				"noriginalcurtaxprice", "noriginalcurprice" }, "nnumber");
		dlg.showModal();
	...
	private void onDocument() {
		...
		if (getShowState() == ListShow) {
			int selectrow = getBillListPanel().getHeadTable().getSelectedRow();
			id = getBillListPanel().getHeadBillModel().getValueAt(selectrow,
					"csaleid").toString();
		} else {
			id = (String) getBillCardPanel().getHeadItem("csaleid")
					.getValueObject();
		}
	...
	public void freshTs(String sBillID, String sTS, Integer iPrintCount) {
		...
		if (ListShow == getShowState()) {
			...
			if (null == sBillID || sBillID.trim().length() <= 0)
				return;
			String csaleid = null;
			for (int i = 0, iloop = getBillListPanel().getHeadTable()
					.getRowCount(); i < iloop; i++) {
				csaleid = (String) getBillListPanel().getHeadBillModel()
						.getValueAt(i, "csaleid");
				if (sBillID.equals(csaleid)) {
					getBillListPanel().getHeadBillModel().setValueAt(sTS, i,
							"ts");
					getBillListPanel().getHeadBillModel().setValueAt(
							iPrintCount, i, "iprintcount");
					break;
				}
			}
	 ...
	 */
	private void onPrint(boolean printflag) {
		if (ListShow == getShowState()) {
			onCard();
		}
		...
		m_printList = new ScmPrintTool(panel, printData, listVo,
				getModuleCode());
		try {
			if(printflag){
	...
	/**
	 * 方法功能描述：V55修改为使用billprinttools实现支持多模板选择。 <b>参数说明</b>
	 * 
	 * @param previewflag
	 * @author fengjb
...
//	      alPrintVO.add(saleinvoice);
	    ...
	    //合并打印的处理  add by QuSida 2010-8-7 （顺德骏杰） --- begin
					...
					if (CardShow == getShowState() && UnitePrintFlag == true) {
						if (invMap == null) {
							this.showErrorMessage("非应税劳务存货为空，取消合并打印状态");
							UnitePrintFlag = false;
							return;
						}

						ArrayList bodySelectRows = new ArrayList();
						// 存储表体选中的行INDEX
						for (int i = 0; i < getBillCardPanel().getBillModel()
								.getRowCount(); i++) {
							int rowSelectedState = 4;
							if (getBillCardPanel().getBillModel().getRowState(i) == rowSelectedState
									&& !invMap.containsKey(i)) {
								bodySelectRows.add(i);
							}
						}
						// 得到单据VO
						SaleinvoiceVO saleinvoice = (SaleinvoiceVO) getBillCardPanel()
						...
						// 当表体的选中的行数少于表体的总行数时进行筛选处理
						// if(saleinvoice!=null&&bodySelectRows.size() !=
						// getBillCardPanel().getBillTable().getRowCount()){
						SaleinvoiceBVO[] bodyVO = saleinvoice.getChildrenVO();

						Double noriginalcursummny = 0.0;
						Double nnumber = 0.0;

						for (int i = 0; i < bodyVO.length; i++) {
							if (bodySelectRows.contains(i)) {

								noriginalcursummny = noriginalcursummny
										+ bodyVO[i].getNoriginalcursummny()
												.toDouble();

							} else if (invMap.containsKey(i)) {
								nnumber = nnumber
										+ bodyVO[i].getNnumber().toDouble();
							}
						}

						ArrayList<SaleinvoiceBVO> bodyVOList = new ArrayList<SaleinvoiceBVO>(
								getBillCardPanel().getBillModel().getRowCount());

						for (int j = 0; j < bodyVO.length; j++) {
							if (invMap.containsKey(j)) {
								UFDouble param1 = bodyVO[j].getNnumber().div(
										nnumber).multiply(noriginalcursummny);
								bodyVO[j].setAttributeValue("noriginalcursummny",
										bodyVO[j].getNoriginalcursummny().add(
												param1));
								bodyVO[j].setAttributeValue("noriginalcurtaxprice",
										bodyVO[j].getNoriginalcursummny().div(
												bodyVO[j].getNnumber()));
								bodyVOList.add(bodyVO[j]);
							} else if (!bodySelectRows.contains(j)) {
								bodyVOList.add(bodyVO[j]);
							}

						}
						SaleinvoiceBVO[] newBodyVO = new SaleinvoiceBVO[bodyVOList
								.size()];
...
//						alPrintVO.add(getVO(true));
	    ...
	    //合并打印的处理  add by QuSida 2010-8-7 （顺德骏杰）--- end
...
//	        	printTool.onBatchPrint(arg0, arg1, arg2);
	      ...
	      UnitePrintFlag = false;//将合并打印的标示清空  add by QuSida 2010-8-10 （顺德骏杰）
	...
	private void UnitePrint(boolean printflag){
		...
		// 得到单据VO
			SaleinvoiceVO saleinvoice = (SaleinvoiceVO) getBillCardPanel()
					.getBillValueVO(SaleinvoiceVO.class.getName(),
							SaleVO.class.getName(),
							SaleinvoiceBVO.class.getName());
			SaleinvoiceBVO[] childVO = getDataVO(false).getChildrenVO();
			SaleinvoiceBVO[] bodyVO = getDataVO(true).getChildrenVO();
		...
		if (CardShow == getShowState()) {
			...
			for(int j = 0;j < childVO.length; j++){
					nnumber = nnumber
							+ childVO[j].getNnumber().toDouble();
				}
			ArrayList<SaleinvoiceBVO> bodyVOList = new ArrayList<SaleinvoiceBVO>(
					getBillCardPanel().getBillModel().getRowCount());
			...
			for (int j = 0; j < childVO.length; j++) {
					...
					UFDouble param1 = childVO[j].getNnumber().div(
							nnumber).multiply(noriginalcursummny);
					childVO[j].setAttributeValue("noriginalcursummny",
							childVO[j].getNoriginalcursummny().add(
									param1));
					childVO[j].setAttributeValue("noriginalcurtaxprice",
							childVO[j].getNoriginalcursummny().div(
									childVO[j].getNnumber()));
					bodyVOList.add(childVO[j]);
			}
			SaleinvoiceBVO[] newBodyVO = new SaleinvoiceBVO[bodyVOList
					.size()];
			bodyVOList.toArray(newBodyVO);
			// 修改单据的表体VO,过滤掉未选择的表体行
			SaleinvoiceVO salevo = getDataVO(false);
			salevo.setChildrenVO(newBodyVO);
			
			alPrintVO.add(salevo);
		}

		BillPrintTool printTool;
		try {
			printTool = new BillPrintTool(this, 
			  SaleInvoiceTools.getNodeCode(), alPrintVO, getBillCardPanel().getBillData(),
			  null, SaleInvoiceTools.getLoginPk_Corp(), SaleInvoiceTools
			  .getLoginUserId(), "vreceiptcode", "csaleid");
			if(printflag){
			...
			showHintMessage(getPrintLogClient().getPrintResultMsg(printflag));
	...
	/*
	 * add by ouyangzhb 2011-02-28 把 销售发票的 存货和费用分开保存
	 */
	public SaleinvoiceVO getDataVO(boolean costflag) {
		...
		SaleinvoiceVO saleinvoicevo = (SaleinvoiceVO) getBillCardPanel()
				.getBillValueVO(SaleinvoiceVO.class.getName(),
						SaleVO.class.getName(), SaleinvoiceBVO.class.getName());
	...
	private boolean onDelete(SaleinvoiceVO vo) throws Exception {
		...
		try {
			newsourheadvo = (SaleVO) PfUtilClient.processActionFlow(this,
					ISaleInvoiceAction.BlankOut, SaleBillType.SaleInvoice,
		...
		if (SaleVO.FCOUNTERACTFLAG_COUNTERACT_GEN == blankhead
				.getFcounteractflag()
				&& null != newsourheadvo) {
			String newbillid = newsourheadvo.getCsaleid();
			for (int i = 0, iloop = getBillListPanel().getHeadBillModel()
					.getRowCount(); i < iloop; i++) {
				String currentpk = (String) getBillListPanel()
						.getHeadBillModel().getValueAt(i, "csaleid");
				if (newbillid.equals(currentpk)) {
					...
					if (oldvo != null) {
						...
						oldvo.getHeadVO().setFcounteractflag(
								newsourheadvo.getFcounteractflag());
						getVOCache().setSaleinvoiceVO(newbillid, oldvo);
						// 更改表头数据
						getBillListPanel().getHeadBillModel().setValueAt(
								newsourheadvo.getTs(), i, "ts");
						getBillListPanel().getHeadBillModel().setValueAt(
								newsourheadvo.getFcounteractflag(), i,
								"fcounteractflag");
					}
				}
			}
		...
		if (PfUtilClient.isSuccess()) {
			showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"scmsopreor", "UPPscmsopreor-000055")/* @res "作废成功！" */);
			return true;
		}
	...
	private void onSendAudit() {
		...
		if (null == voCur || null == voCur.getParentVO()) {
			...
			try {
				...
				// iWorkflowstate =
				// nc.ui.pub.pf.PfUtilClient.queryWorkFlowStatus(voCur
				// .getHeadVO().getCbiztype(), SaleBillType.SaleInvoice, voCur
				// .getParentVO().getPrimaryKey());
				// if (iWorkflowstate !=
				// nc.vo.pub.pf.IWorkFlowStatus.BILL_NOT_IN_WORKFLOW
				// && iWorkflowstate !=
				// nc.vo.pub.pf.IWorkFlowStatus.NOT_APPROVED_IN_WORKFLOW) {
				// if (iWorkflowstate ==
				...
				// */);
				// else if (iWorkflowstate ==
				// nc.vo.pub.pf.IWorkFlowStatus.WORKFLOW_FINISHED)
				// showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
				// "SCMCOMMON", "UPPSCMCommon-000078")/*
				// * @res "单据已审批完成"
				// */);
				// else if (iWorkflowstate ==
				// nc.vo.pub.pf.IWorkFlowStatus.WORKFLOW_ON_PROCESS)
				// showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
				// "SCMCOMMON", "UPPSCMCommon-000079")/*
				// * @res "单据正在审批中"
				// */);
				// else if (iWorkflowstate ==
				// nc.vo.pub.pf.IWorkFlowStatus.BILLTYPE_NO_WORKFLOW)
				// showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
				...
				HashMap<String, SmartVO> hsnewvo = (HashMap) PfUtilClient
						.processAction(ISaleInvoiceAction.SendAudit,
								SaleBillType.SaleInvoice,
								getClientEnvironment().getDate().toString(),
								voCur);
				...
				if (PfUtilClient.isSuccess()) {
					...
					showHintMessage(nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060301", "UPP40060301-000291")/*
																		 * @res
																		 * "送审成功!"
																		 */);
				} else {
					showHintMessage(nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40060301", "UPP40060301-000292")/*
																		 * @res
																		 * "送审操作已经被用户取消！"
																		 */);
				}
	...
	/**
	 * 方法功能描述：送审、审批、弃审等操作后 更新当前界面UI数据和数据库中一致。 <b>参数说明</b>
	 * 
	 * @param voCur
	 * @param hsts
	 * @author fengjb
	...
	public void onButtonClicked(nc.ui.pub.ButtonObject bo) {
		...
		if (getShowState() == ListShow) {
			if (getBillListPanel().getHeadTable().getSelectedRowCount() > 1) {
				if (bo == getBtns().m_boApprove || bo == getBtns().m_boBlankOut
						|| bo == getBtns().m_boUnApprove
						|| bo == getBtns().m_boUnite
						|| bo == getBtns().m_boUniteCancel) {
					m_proccdlg = new ProccDlg(this, bo.getHint(), this, bo
							.getTag());
		...
		if (!bo.getCode().equals(getBtns().m_boFetchCost.getCode())
				&& existErrRows) {
	...
	/**
	 * 方法功能描述：V55销售发票支持无利润销售，增加“取成本价”按钮。 <b>参数说明</b>
	 * 
	 * @author fengjb
	...
	private void onFetchCost() {
		...
		if (keyset.size() > 0) {
			...
			for (int i = 0, iloop = saleoutbid.length; i < iloop; i++) {
				...
				if (hcostprice.containsKey(saleoutbid[i])
					...
					UFDouble ncostprice = new UFDouble((BigDecimal) hcostprice
							.get(saleoutbid[i]));
					// 原币无税单价 = 本币无税单价 * 汇率
					UFDouble noriginalcurprice = null;
					try {
						String pk_corp = saleinvoice.getHeadVO().getPk_corp();
						BusinessCurrencyRateUtil currateutil = new BusinessCurrencyRateUtil(
								pk_corp);
						SOCurrencyRateUtil socurrateutil = new SOCurrencyRateUtil(
								pk_corp);
						String pk_curtype = saleinvoice.getHeadVO()
								.getCcurrencyid();
						// 币种VO
						noriginalcurprice = currateutil
								.getAmountByOpp(
										socurrateutil.getLocalCurrPK(),
										pk_curtype,
										ncostprice,
										nexchangeotobrate,
										saleinvoice.getHeadVO().getDbilldate() == null ? getClientEnvironment()
												.getDate().toString()
												: saleinvoice.getHeadVO()
														.getDbilldate()
														.toString());
					} catch (Exception e) {
					...
					if (st.SA_02.booleanValue()) {
						// 税率
						UFDouble ntaxrate = body[row].getNtaxrate() == null ? new UFDouble(
								0)
								: body[row].getNtaxrate();
						// 原币含税净价 = 原币无税单价 * (1+税率/100)
						UFDouble noriginalcurtaxnetprice = noriginalcurprice
								.multiply(new UFDouble(1).add(ntaxrate
										.div(new UFDouble(100))));
						getBillCardPanel().setBodyValueAt(
								noriginalcurtaxnetprice, row,
								"noriginalcurtaxnetprice");

					} else
						getBillCardPanel().setBodyValueAt(noriginalcurprice,
								row, "noriginalcurnetprice");

					// 重新计算数量单价金额
					// 缓存原先的整单折扣
					UFDouble ndiscountrate = getBillCardPanel().getBodyValueAt(
							row, "ndiscountrate") == null ? new UFDouble(100)
							: SmartVODataUtils.getUFDouble(getBillCardPanel()
									.getBodyValueAt(row, "ndiscountrate"));
					// 把 整单折扣 * 单品折扣作为订单折扣放到整单折扣字段上
					UFDouble nitemdiscount = getBillCardPanel().getBodyValueAt(
							row, "nitemdiscountrate") == null ? new UFDouble(
							100) : SmartVODataUtils
							.getUFDouble(getBillCardPanel().getBodyValueAt(row,
									"nitemdiscountrate"));
					;
					getBillCardPanel().setBodyValueAt(
							ndiscountrate.multiply(nitemdiscount).div(
									new UFDouble(100)), row, "ndiscountrate");
					nc.ui.scm.pub.panel.RelationsCal.calculate(row,
							getBillCardPanel(),
							getBillCardPanel().getCalculatePara(
									"noriginalcurtaxnetprice"),
							"noriginalcurtaxnetprice", SaleinvoiceBVO
									.getDescriptions(), SaleinvoiceBVO
									.getKeys(),
							"nc.vo.so.so002.SaleinvoiceBVO",
							"nc.vo.so.so002.SaleVO");
					// 恢复整单折扣值
					getBillCardPanel().setBodyValueAt(ndiscountrate, row,
							"ndiscountrate");
					// 重新设置冲减前金额
					getBillCardPanel().setBodyValueAt(
							getBillCardPanel().getBodyValueAt(row,
									"noriginalcursummny"), row, "nsubsummny");
					getBillCardPanel().setBodyValueAt(
							getBillCardPanel().getBodyValueAt(row, "nsummny"),
							row, "nsubcursummny");
		...
		if (aryrowno.size() > 0) {
			...
			for (int i = 0; i < aryrowno.size(); i++) {
				errstr.append(">").append(aryrowno.get(i)).append(
						"< 没有取到成本价 \n");
			}
			nc.ui.scm.pub.panel.SetColor.setErrRowColor(getBillCardPanel(),
					errindex);
			showErrorMessage(errstr.toString());
			existErrRows = true;
		}
		getBillCardPanel().setHeadItem("ninvoicediscountrate",
				new UFDouble(100));
		getBillCardPanel().setHeadItem("ntotalsummny",
				getBillCardPanel().calcurateTotal("noriginalcursummny"));
		getBillCardPanel().execHeadFormula("nnetmny->ntotalsummny-nstrikemny");

	}

	/**
	 * 方法功能描述：销售发票支持编辑状态追加，完成“参照增行”功能。 <b>参数说明</b>
	 * 
	 * @param bo
	 *            本流程可参照增加的单据类型
	 * @author fengjb
	...
	private void onRefAddLine(ButtonObject bo) {
		...
		// 发票合并
		SaleinvoiceSplitTools splittools = new SaleinvoiceSplitTools();
		SaleinvoiceVO[] aryRetVO = splittools
				.splitSaleinvoiceVOs(vosRefAddLine);

		// 得到符合要求的需要追加的当前卡片界面上的表体VO数组
		SaleinvoiceBVO[] addToCardBody = checkBeforeRefAdd(aryRetVO);
		/** 处理多次开票尾差 */
		// 把当前卡片界面上的以前参照增行过来的数据过滤出来加入到尾差处理过程中
		SaleinvoiceVO curcardvo = (SaleinvoiceVO) getBillCardPanel()
				.getBillData().getBillValueVO(SaleinvoiceVO.class.getName(),
						SaleVO.class.getName(), SaleinvoiceBVO.class.getName());
		...
		if (addToCardBody.length > 0) {
			...
			getBillCardPanel().getBillModel().setBodyRowVOs(addToCardBody,
					addlinerow);
			BillRowNo.addLineRowNos(getBillCardPanel(),
					SaleBillType.SaleInvoice, "crowno", oldrowcount - 1,
					addlinerow);
			// 参照增行增加行的辅计量编辑性和换算率
			...
			for (Integer rowindex : addlinerow) {
				...
				// 如果存货分类字段显示且为空
				if (isshow
						&& null == getBillCardPanel().getBodyValueAt(rowindex,
								"cinvclassid")) {
					String[] invclformul = new String[] { "cinvclassid->getColValue(bd_invbasdoc,pk_invcl,pk_invbasdoc,cinvbasdocid)" };
					getBillCardPanel().execBodyFormulas(rowindex, invclformul);
				}
				// 参照增行新增时要设置赠品字段编辑属性
				// 设置赠品行编辑性
				Object largess = getBillCardPanel().getBodyValueAt(rowindex,
						"blargessflag");
				boolean blargess = largess == null ? false : SmartVODataUtils
						.getUFBoolean(largess).booleanValue();
				// 设置编辑性
				getBillCardPanel().setCellEditable(rowindex, "blargessflag",
						false);
				getBillCardPanel().setCellEditable(rowindex, "cinventorycode",
						false);
				;
				getBillCardPanel().getBillCardTools().setCellEditableByLargess(
						blargess && !st.SO_59.booleanValue(), rowindex);
				// 辅计量存货编辑性
				getBillCardPanel().setAssistChange(rowindex);
				if (null == getBillCardPanel().getBodyValueAt(rowindex,
						"cpackunitid")
						|| "".equals(getBillCardPanel().getBodyValueAt(
								rowindex, "cpackunitid")))
					continue;
				...
				getBillCardPanel().execBodyFormulas(rowindex,
						appendFormulaViaPrice);
	...
	private SaleinvoiceBVO[] checkBeforeRefAdd(SaleinvoiceVO[] addvos) {
		...
		// 获得界面上VO值
		SaleinvoiceVO curbill = (SaleinvoiceVO) getBillCardPanel()
				.getBillValueVO(SaleinvoiceVO.class.getName(),
						SaleVO.class.getName(), SaleinvoiceBVO.class.getName());
		...
		for (SaleinvoiceVO addvo : addvos) {
			...
			// 客户
			if (checkHeadItem(curhead.getCreceiptcorpid(), addhead
					.getCreceiptcorpid())) {
				errmsg.append(getErrMsg(addvo, "ccustbaseid"));
				continue;
			}
			// 销售组织
			if (checkHeadItem(curhead.getCsalecorpid(), addhead
					.getCsalecorpid())) {
			...
			// 币种
			if (checkHeadItem(curhead.getCcurrencyid(), addhead
					.getCcurrencyid())) {
	...
	/**
	 * 方法功能描述：参照增行时如果数据不匹配组织错误提示。 <b>参数说明</b>
	 * 
	 * @param vo
	 * @param key
	 * @return
			...
			for (int i = 0, loop = vo.getBodyVO().length; i < loop; i++) {
				errmsg.append(
						NCLangRes.getInstance().getStrByID(
								...
								new String[] {
										vo.getBodyVO()[i]
												.getCupsourcebillcode(),
										vo.getBodyVO()[i].getUpsourcerowno(),
										keyname })).append("\n");
			}
		}
		if (errmsg.length() > 0) {
	...
	/**
	 * 方法功能描述：参照增行时判断指定表头项和销售发票原有表头项是否相等。 <b>参数说明</b>
	 * 
	 * @param curstr
	 * @param addstr
	 * @return
	...
	private void onRefresh() {
		...
		try {
			vosInvoice = SaleinvoiceBO_Client.queryBillData(getQueryDlg()
					.getWhere(), SaleInvoiceTools.getLoginUserId(),
					getQueryDlg().getIsSelAuditing());
		} catch (Exception e) {
			SCMEnv.out(e);
			MessageDialog
					.showErrorDlg(this,
		...
		timer.addExecutePhase(">>>>>>>>>查询用时：>>>>>>>>>>>");
		// 设置缓存
		getVOCache().setCacheData(vosInvoice);
		timer.addExecutePhase(">>>>>>>>>添加缓存用时：>>>>>>>>>>>");
		// 切换到列表
		onList();
		timer.addExecutePhase(">>>>>>>>>切换列表用时：>>>>>>>>>>>");

		// 输出各个业务操作占用时间分布
		timer.showAllExecutePhase(">>>>>>>>>销售发票查询结束：>>>>>>>>>>>");
	}
	...
	/**
	 * 方法功能描述：设置当前是卡片显示还是列表显示。 <b>参数说明</b>
	 * 
	 * @param showState
	 * @author fengjb
	...
	private void onCancelTransfer() {
		...
		// 卡片下放弃该张单据
		if (getShowState() == ListShow) {
			int[] iaSerialNum = getBillListPanel().getHeadTable()
					.getSelectedRows();
	...
	public void updateCache() {
		...
		try {
			...
			if (getQueryDlg().getConditionVO() != null) {
				sales = SaleinvoiceBO_Client.queryBillData(getQueryDlg()
						.getWhere(), getClientEnvironment().getUser()
						.getPrimaryKey(), getQueryDlg().getIsSelAuditing());
			}
		} catch (Exception e) {
			SCMEnv.out(e);
			showErrorMessage(e.getMessage());
	...
	private void setButtonsStateEdit() {
		...
		if (getBillCardPanel().getVO() != null) {
			...
			if (getBillCardPanel().getVO().getHeadVO().getFcounteractflag() == SaleVO.FCOUNTERACTFLAG_COUNTERACT_GEN) {
				...
				// 只有发票总金额>0 才可进行冲减。
				getBtns().m_boUnite.setEnabled(getBillCardPanel().getVO()
						.getHeadVO().isLgtZero());
		...
		if (null != getBillCardPanel().getVO()
				&& SaleVO.FCOUNTERACTFLAG_COUNTERACT_GEN == getBillCardPanel()
						.getVO().getHeadVO().getFcounteractflag()) {
			getBtns().m_boLineOper.setEnabled(false);
			getBtns().m_boAddLine.setEnabled(false);
			getBtns().m_boDelLine.setEnabled(false);
			getBtns().m_boInsertLine.setEnabled(false);
		...
		if (null != getBillCardPanel().getVO()
				&& (SaleVO.FCOUNTERACTFLAG_COUNTERACT_GEN == getBillCardPanel()
						.getVO().getHeadVO().getFcounteractflag() || SaleVO.FCOUNTERACTFLAG_COUNTERACT_FINISH == getBillCardPanel()
						.getVO().getHeadVO().getFcounteractflag())) {
			getBtns().m_boRefAdd.setEnabled(false);
		} else {
		...
		// bCanSendAudit = nc.ui.pub.pf.PfUtilBO_Client.isExistWorkFlow(
		// SaleBillType.SaleInvoice,
		// getBillCardPanel().getVO().getHeadVO().getCbiztype(),
		// // NO_BUSINESS_TYPE,
		// getClientEnvironment().getCorporation().getPk_corp(),
		// getClientEnvironment().getUser().getPrimaryKey());
		// }catch(Exception e){
	...
	public void reloadDataAfterRun() {
		...
		for (int i = 0; i < getBillListPanel().getHeadBillModel().getRowCount(); i++) {
			oTemp = getBillListPanel().getHeadBillModel().getValueAt(i,
					"csaleid");
			if (oTemp != null)
		...
		if (vTemp.size() > 0) {
			...
			SaleinvoiceVO[] vosInvoice = null;
			try {
				vosInvoice = SaleinvoiceBO_Client.queryBillData(sql, null,
						false);
			} catch (Exception e) {
				SCMEnv.out(e);
				MessageDialog.showErrorDlg(this, nc.ui.ml.NCLangRes
	...
	public void doAddAction(ILinkAddData addData) {
		...
		try {
			...
			setButtonsStateEdit();
			showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"SCMCommon", "UPPSCMCommon-000350") /* * @res "编辑单据..." */);
	...
	private Object execQuery(String pk_invbasdoc) {
		...
		Object o = null;
		String sql = "select laborflag from bd_invbasdoc where pk_invbasdoc = '"
				+ pk_invbasdoc + "'";
		try {
	...
	private void onUnitePrint() {
		...
		for (int i = 0; i < bodyVOs.length; i++) { // 判断表体存货，当表体存货的应税劳务标示为N时设为默认选中,并将其存入非应税劳务存货列表中
			String invbasdocid = bodyVOs[i].getCinvbasdocid();
			Object o = execQuery(invbasdocid);
			if (o != null && (((String) o).trim().toUpperCase()).equals("N")) {
				getBillCardPanel().getBillModel().setRowState(i, 4);
				getBillCardPanel().getBillModel().getRowAttribute(i).setEdit(
						false);
		...
		// getBtns().m_boAssistant.setEnabled(false);
		// getBtns().m_boBusiType.setEnabled(false);
		// getBtns().m_boAssistFunction.setEnabled(false);
		// getBtns().m_boOpposeAct.setEnabled(false);
		// getBtns().m_boSoTax.setEnabled(false);
		// getBtns().m_boDocument.setEnabled(false);
		// getBtns().m_boAssistant.setEnabled(false);
		// getBtns().m_boOrderQuery.setEnabled(false);
		// getBtns().m_boATP.setEnabled(false);
		// getBtns().m_boAuditFlowStatus.setEnabled(false);
		// getBtns().m_boCustInfo.setEnabled(false);
		// getBtns().m_boExecRpt.setEnabled(false);
		// getBtns().m_boCustCredit.setEnabled(false);
		// getBtns().m_boPrifit.setEnabled(false);
		// ButtonObject[] bos = getButtons();
		// for (int i = 0; i < bos.length; i++) {
		// bos[i].setEnabled(false);
		//	
		// }
		// getBtns().m_boUnitePrint.setEnabled(true);
		// getBtns().m_boPrintManage.setEnabled(true);
		// getBtns().m_boPreview.setEnabled(true);
		// getBtns().m_boPrint.setEnabled(true);
		// getBtns().m_boBillCombin.setEnabled(false);
