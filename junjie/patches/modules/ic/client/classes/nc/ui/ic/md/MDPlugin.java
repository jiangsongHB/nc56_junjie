...
public class MDPlugin extends SCMBasePlugin{
	...
	public void beforSave(AggregatedValueObject[] arg1 )
		...
		for (AggregatedValueObject avo : arg1) {
			...
			GeneralBillItemVO[] items = (GeneralBillItemVO[])avo.getChildrenVO();
	 ...
	 * 
	 * @param items
	 * @throws BusinessException 如果该行存货存在码单信息时则抛出该异常
	 */
	 ...
	 * 
	 * @param items 存货行VO
	 * @throws BusinessException 如果必须维护码单信息的VO没有维护,则抛出该异常
	 */
	...
	boolean checkNeedMD(String cinvbasid){
		try {
			SuperVO[] vo= HYPubBO_Client.queryByCondition(InvbasdocVO.class, " isnull(dr,0)=0 and isnull(def20,'N')='Y' and pk_invbasdoc = '"+cinvbasid+"' ");
			if(vo!=null&&vo.length>0){
				return true;
			}
		} catch (UifException e) {
