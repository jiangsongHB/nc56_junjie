 ...
 */
public class MDioDialog extends UIDialog implements ActionListener,BillEditListener,BillEditListener2{
	...
	GeneralBillVO getGeneralBillVO(){
        ...
        {
            nowVObill = (GeneralBillVO)ui.getM_alListData().get(ui.getM_iLastSelListHeadRow());
        }
        return nowVObill;
	}
...
//            WhID = (String)nowVObill.getHeaderValue("cwarehouseid");
            InvID = (String)nowVObill.getItemValue(getGenSelectRowID(), "cinventoryid");
        }
        getBillCardPanel().getHeadItem("invpk").setValue(InvID);// 存货档案PK
        getBillCardPanel().getHeadItem("realWeight").setValue(getGeneralBillVO().getItemValue(getGenSelectRowID(), "ninnum"));// 实收数量
        getBillCardPanel().execHeadLoadFormulas();

        String gg = (String)getBillCardPanel().getHeadItem("thickness").getValueObject();//规格
		UFDouble g;
		try {
			g = new UFDouble(gg);
		} catch (Exception e) {
        ...
        mdxclvo.setCwarehouseidb((String)nowVObill.getHeaderValue("cwarehouseid"));// 仓库
        mdxclvo.setCinvbasid((String)nowVObill.getItemValue(getGenSelectRowID(), "cinvbasid"));// 基本档案
	...
	private void initData() {
		...
		if(this.status==MDUtils.INIT_CANEDIT){
			String cwarehouseid = (String)getGeneralBillVO().getHeaderValue("cwarehouseid");
			UIRefPane panel = (UIRefPane)getBillCardPanel().getBodyItem("box").getComponent();
			panel.getRefModel().addWherePart(" and bd_stordoc.pk_stordoc='"+cwarehouseid+"'");
		}
	}
	...
	private void initBodyData() {
		if(canloaddata){
//			getGeneralBillVO().getItemValue(getGenSelectRowID(), "cgeneralbid")
			try {
				MdcrkVO[] vos = (MdcrkVO[])HYPubBO_Client.queryByCondition(MdcrkVO.class, " isnull(dr,0)=0 and cgeneralbid='"+getGeneralBillVO().getItemValue(getGenSelectRowID(), "cgeneralbid")+"'");
...
//			buttonPanel.setBorder(BorderFactory.createLineBorder(Color.LIGHT_GRAY));
	...
	BillCardPanel getBillCardPanel(){
		if(cardPanel==null){
			...
			cardPanel.loadTemplet("MD01", "MD01", getEnvironment().getUserID(), getEnvironment().getCorpID());
			...
			if(e.getSource() instanceof UIButton){
				...
				if(button.getText().equals(MDUtils.ADDLINE_BUTTON)){
					...
					if(this.status==MDUtils.EDITING){
						int i = MessageDialog.showYesNoDlg(this, "提示", "正在维护码单信息,是否要取消操作?");
						if(i==this.ID_YES){
							closeCancel();
						}
					}else
...
//		ui.getButtonManager().onButtonClicked(ui.getButtonManager().getButton(ICButtonConst.BTN_BROWSE_REFRESH));
	...
	private void onCalc() throws BusinessException  {
		...
		UFDouble num = new UFDouble((String)getBillCardPanel().getHeadItem("realWeight").getValueObject());
		String ispj = (String)getBillCardPanel().getHeadItem("ispj").getValueObject();
	...
	MdcrkVO[] getBodyVOs(){
		return (MdcrkVO[])getBillCardPanel().getBillData().getBodyValueVOs(MdcrkVO.class.getName());
	}
	...
	private void onSave() throws BusinessException {
		...
		if(edited){
			...
			tools.saveMDrk(mdvos, mdxclvo, (String)getGeneralBillVO().getItemValue(getGenSelectRowID(), "cgeneralbid"));
			setMessage("保存成功...");
			setStatus(MDUtils.INIT_CANEDIT);
			initBodyData();
...
//			ui.getButtonManager().onButtonClicked(ui.getButtonManager().getButton(ICButtonConst.BTN_BROWSE_REFRESH));
	...
	private void onAddline() {
		...
		int i = getBillCardPanel().getRowCount();
		// 单据类型
		getBillCardPanel().getBillModel().setValueAt(ui.getBillType(), i-1, "cbodybilltypecode");
		
		// 单据方向
		UFBoolean frep = (UFBoolean)getGeneralBillVO().getHeaderValue("freplenishflag");//是否退货标识;
		...
		getBillCardPanel().getBillModel().setValueAt(getGeneralBillVO().getHeaderValue("cwarehouseid"), i-1, "cwarehouseidb");
		
		// ccalbodyidb 库存组织PK  pk_calbody
		getBillCardPanel().getBillModel().setValueAt(getGeneralBillVO().getHeaderValue("pk_calbody"), i-1, "ccalbodyidb");
		
		getBillCardPanel().getBillModel().setValueAt(getGeneralBillVO().getItemValue(getGenSelectRowID(), "cgeneralbid"), i-1, "cgeneralbid");
		getBillCardPanel().getBillModel().setValueAt(getEnvironment().getCorpID(), i-1, "pk_corp");
		getBillCardPanel().getBillModel().setValueAt(getEnvironment().getLogDate(), i-1, "dmakedate");
		getBillCardPanel().getBillModel().setValueAt(getEnvironment().getUserID(), i-1, "voperatorid");

		getBillCardPanel().getBillModel().setValueAt(getBillCardPanel().getHeadItem("thickness").getValueObject(), i-1, "def6");
		getBillCardPanel().getBillModel().setValueAt(getBillCardPanel().getHeadItem("ispj").getValueObject(), i-1, "sfpj");// 是否磅计
...
//		if(key.equals("srkzl")){
//			UFDouble hsl = (UFDouble)getGeneralBillVO().getItemValue(getGenSelectRowID(), "hsl");
//			
//			UFDouble zl = new UFDouble((String)editEvent.getValue());
//			getBillCardPanel().getBillModel().setValueAt(hsl.multiply(zl), editEvent.getRow(), "srkzs");
//		}else 
//			
		if(key.equals("ispj")){
		...
		if(key.equals("md_meter")){
			getBillCardPanel().getBillModel().setValueAt(null, editEvent.getRow(), "md_width");
			getBillCardPanel().getBillModel().setValueAt(null, editEvent.getRow(), "md_length");
			getBillCardPanel().getBillModel().setValueAt(null, editEvent.getRow(), "srkzl");
		}else if(key.equals("md_width")||key.equals("md_length")){
			getBillCardPanel().getBillModel().setValueAt(null, editEvent.getRow(), "md_meter");
			getBillCardPanel().getBillModel().setValueAt(null, editEvent.getRow(), "srkzl");
