...
public class MBJSDialog extends UIDialog implements ActionListener{
    ...
    public MBJSDialog(Container parent,
			HashMap<String, UFDouble> hmpfrom, Object cinvbasid,
			Object cproviderid) {
        super(parent, "毛边计算");
    ...
    private void initdata() {
    	getBillCardPanel().getHeadItem("edgeprice").setValue(hmpfrom.get("grossprice"));
    	getBillCardPanel().getHeadItem("edgezl").setValue(hmpfrom.get("grossweight"));
    	getBillCardPanel().getHeadItem("edgemny").setValue(hmpfrom.get("grosssumny"));
    	getBillCardPanel().getHeadItem("materprice").setValue(hmpfrom.get("stuffprice"));
    	getBillCardPanel().getHeadItem("materzl").setValue(hmpfrom.get("stuffweight"));
    	getBillCardPanel().getHeadItem("matermny").setValue(hmpfrom.get("stuffsumny"));
    ...
    BillCardPanel getBillCardPanel(){
    	if(card==null){
    		...
    		card.loadTemplet("MD02", "MD02", ClientEnvironment.getInstance().getUser().getPrimaryKey(), ClientEnvironment.getInstance().getCorporation().getPrimaryKey());
    	}
    	return card;
    }
...
//        	UIButtonCalcu.addActionListener(new java.awt.event.ActionListener() {
...
//            });
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() instanceof UIButton){
			...
			if(button.equals(getUIButtonCalcu())){
				...
				if(isCalc){
					hmpResult.put("grossprice", new UFDouble((String)getBillCardPanel().getHeadItem("edgeprice").getValueObject()));
					hmpResult.put("grosssumny", new UFDouble((String)getBillCardPanel().getHeadItem("edgemny").getValueObject()));
					hmpResult.put("grossweight",new UFDouble((String)getBillCardPanel().getHeadItem("edgezl").getValueObject()));
					hmpResult.put("stuffprice", new UFDouble((String)getBillCardPanel().getHeadItem("materprice").getValueObject()));
					hmpResult.put("stuffweight",new UFDouble((String)getBillCardPanel().getHeadItem("materzl").getValueObject()));
					hmpResult.put("stuffsumny", new UFDouble((String)getBillCardPanel().getHeadItem("matermny").getValueObject()));
					closeOK();
				}else{
	...
	private void onCalc() {
		String gg = (String)getBillCardPanel().getHeadItem("gg").getValueObject();//规格
		boolean isnum = true;
		...
		grossprice = new UFDouble((String)getBillCardPanel().getHeadItem("edgeprice").getValueObject());
		UFDouble materzl = new UFDouble((String)getBillCardPanel().getHeadItem("materzl").getValueObject());
		...
		UFDouble mbxs = MDUtils.getMBXS(cinvbasid.toString(), cproviderid.toString(), hd);
