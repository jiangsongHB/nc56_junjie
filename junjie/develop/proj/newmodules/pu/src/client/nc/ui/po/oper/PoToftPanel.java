...
public abstract class PoToftPanel extends nc.ui.pub.ToftPanel implements
  ...
  //since v55
	...
	public HashMap<String, SmartVO> CostVOs = new HashMap<String, SmartVO>();//查询出来的费用信息VO
...
//		for(int i = 0;i<getPoListPanel().getHeadBillModel().getRowCount();i++){
//        String pk = (String)getPoListPanel().getHeadBillModel().getValueAt(i, "corderid");   
//             if(i == 0)
//            	 whereSql.append(" and cbillid = '"+pk+"'");
//             else
//            	 whereSql.append(" or cbillid = '"+pk+"'");
//		
//		}
//		SmartVO[] smartVOs = null;
//		try {
//			 smartVOs = ScmPubHelper.querySmartVOs(InformationCostVO.class, null, whereSql.toString());
//		} catch (Exception e) {			
//			SCMEnv.out(e);
//			return;
//		}
//		if(smartVOs!=null&&smartVOs.length!=0)
//	   for (int i = 0; i < smartVOs.length; i++) {
//		  CostVOs.put(smartVOs[i].getAttributeValue("cbillid").toString(), smartVOs[i]);
//	}
//		}
//		getPoListPanel().getBodyBillModel("jj_scm_informationcost").set
		updateUI();
	...
	protected void onBillModify() {
		...
		OrderVO voCur = getOrderDataVOAt(getBufferVOManager().getVOPos());
		if(isFromOtherBill() && !(cupsourcetype != null && (cupsourcetype.equals(BillTypeConst.CT_ORDINARY) || cupsourcetype.equals(BillTypeConst.CT_BEFOREDATE)))
...
//		}
	...
	protected boolean onBillSave() {
		// 审批中修订保存
		getPoCardPanel().getBodyTabbedPane().setSelectedIndex(0);
		if (bRevise) {
...
//			checkStorAndInvIsCapital(voSavedOrder);
	...
	private void onBillPush() {
		...
		try {
			nc.ui.pub.pf.PfUtilClient.processBatchFlow(this, "PurchaseOrder",
					BillTypeConst.SALE_ORDER, PoPublicUIClass.getLoginDate()
							.toString(), new OrderVO[] { voSavedOrder }, null);
			// 刷新标志
			OrderVO[] voLightRefreshed = OrderHelper.queryOrderVOsLight(
					new String[] { voSavedOrder.getHeadVO().getCorderid() },
					"AUDIT");
	 ...
	 */
	private void onButtonClickedBillAssistQry(nc.ui.pub.ButtonObject bo) {
...
//				getCorpPrimaryKey());
	...
	public OrderVO onSave(ToftPanel pnlToft, PoCardPanel pnlBill,
		...
		UFDouble number = new UFDouble();//add by QuSida 2010-8-26 (佛山骏杰) 费用信息中的数量是订单中存货数量的和
		for (int i = 0; i < voSaved.getBodyVO().length; i++) {
			number = number.add(voSaved.getBodyVO()[i].getNordernum());
		}
		//add by QuSida 2010-8-13 (佛山骏杰)  --- begin
		//function:得到费用信息页签的vo,并将录入的费用信息分配到要保存的订单vo中
		InformationCostVO[] infoCostVOs = (InformationCostVO[])getPoCardPanel().getBillData().getBodyValueVOs("jj_scm_informationcost", InformationCostVO.class.getName());

		if(infoCostVOs!=null&&infoCostVOs.length!=0){
		//将录入的费用信息分配到要保存的订单vo中
		costInfoDistribute(infoCostVOs,voSaved);
		}
		//add by QuSida 2010-8-13 (佛山骏杰)  --- end
		...
		//add by QuSida 2010-8-24 (佛山骏杰)  --- begin
		//function:将数量,含税金额,无税金额回写到界面上
//		for (int i = 0; i < infoCostVOs.length; i++) {
//			getPoCardPanel().setBodyValueAt(infoCostVOs[i].getNnumber(), i, "nnumber", "jj_scm_informationcost");
//			getPoCardPanel().setBodyValueAt(infoCostVOs[i].getNoriginalcurmny(), i, "noriginalcurmny", "jj_scm_informationcost");
//			getPoCardPanel().setBodyValueAt(infoCostVOs[i].getNoriginalcursummny(), i, "noriginalcursummny", "jj_scm_informationcost");
//		}
		//add by QuSida 2010-8-25 (佛山骏杰)  --- end
		m_renderYellowAlarmLine.setRight(false);
		hMaxPrice.clear();
		pnlToft.showHintMessage(NCLangRes.getInstance().getStrByID(
		...
		try {
			...
			pnlToft.showWarningMessage(NCLangRes.getInstance()
					.getStrByID("4004020201", "UPP4004020201-000087")/*
																		 ...
																		 */);
			voSaved = null;
			number = null;          //add by QuSida 2010-8-26 (佛山骏杰)  出现异常要将变量置为空
			infoCostVOs = null;
			...
			try {
				...
				pnlToft.showWarningMessage(NCLangRes.getInstance()
						.getStrByID("4004020201", "UPP4004020201-000087")/*
																			 ...
																			 */);
				voSaved = null;
				number = null;   //add by QuSida 2010-8-26 (佛山骏杰)  结束后将变量置为空
				infoCostVOs = null;
			}
		}
		// 业务日志 
