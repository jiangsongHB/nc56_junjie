 ...
 * @version 最后修改日期
 * @see 需要参见的其它类
 * @since 从产品的那一个版本，此类被添加进来。（可选） <li><b>根据V56需求，发票类型增加“其它”</b></li>
 */
public class InvoiceUI extends nc.ui.pub.ToftPanel implements BillEditListener,
		...
		nc.ui.scm.pub.bill.IBillExtendFun, IBillRelaSortListener2,
		ILinkMaintain,// 关联修改
		ILinkAdd,// 关联新增
		ILinkApprove,// 审批流
		ILinkQuery,// 逐级联查
	...
	// 消息中心审批界面按钮
	private ButtonObject m_btnAudit = new ButtonObject(
			nc.ui.ml.NCLangRes.getInstance().getStrByID("common",
					"UC001-0000027")/* @res "审批" */, nc.ui.ml.NCLangRes
					.getInstance().getStrByID("common", "UC001-0000027")/*
																		 * @res
																		 * "审批"
																		 */, 2,
			"审批"); /* -=notranslate=- */

	private ButtonObject m_btnUnAudit = new ButtonObject(
			nc.ui.ml.NCLangRes.getInstance().getStrByID("common",
					"UC001-0000028")/* @res "弃审" */,
			nc.ui.ml.NCLangRes.getInstance().getStrByID("40040401",
					"UPP40040401-000149")/*
										 * @res "执行弃审操作"
										 */, 5, "弃审"); /* -=notranslate=- */
	...
	private ButtonObject[] aryForAudit = new ButtonObject[] { m_btnAudit,
			m_btnUnAudit, m_btnInvBillAssist, m_btnOthersFuncs };
	...
	// 金额,税额,价税合计主键, 设置精度
	private String m_itemMny_fi[] = new String[] { "noriginalpaymentmny" };// 原币累计付款

	// 付款金额主键, 设置精度
	private String m_itemMny_bu[] = new String[] { "noriginalcurmny",
			"noriginaltaxmny", "noriginalsummny" };// 原币无税金额、原币税额、原币价税合计
	...
	private void createBtnInstances() {
		...
		// 业务类型组按钮
		m_btnInvBillBusiType = getBtnTree().getButton(
				IButtonConstInv.BTN_BUSINESS_TYPE);// 业务类型
		...
		m_btnInvBillModify = getBtnTree().getButton(
				IButtonConstInv.BTN_BILL_EDIT);// 修改
		m_btnInvBillCancel = getBtnTree().getButton(
				IButtonConstInv.BTN_BILL_CANCEL);// 取消
		m_btnInvBillDiscard = getBtnTree().getButton(
				IButtonConstInv.BTN_BILL_DELETE);// 删除
		m_btnInvBillCopy = getBtnTree()
				.getButton(IButtonConstInv.BTN_BILL_COPY);// 复制

		// 行操作组按钮
		m_btnInvBillRowOperation = getBtnTree().getButton(
				IButtonConstInv.BTN_LINE);// 行操作
		m_btnInvBillAddRow = getBtnTree().getButton(
				IButtonConstInv.BTN_LINE_ADD);// 增行
		m_btnInvBillDeleteRow = getBtnTree().getButton(
				IButtonConstInv.BTN_LINE_DELETE);// 删行
		m_btnInvBillInsertRow = getBtnTree().getButton(
				IButtonConstInv.BTN_LINE_INSERT);// 插入行
		m_btnInvBillCopyRow = getBtnTree().getButton(
				IButtonConstInv.BTN_LINE_COPY);// 复制行
		m_btnInvBillPasteRow = getBtnTree().getButton(
				IButtonConstInv.BTN_LINE_PASTE);// 粘贴行
		m_btnInvBillPasteRowTail = getBtnTree().getButton(
				IButtonConstInv.BTN_LINE_PASTE_TAIL);// 粘贴行到表尾
		// 支持弹出菜单中“重排行号”功能
		...
		//
		btnBillAddContinue = getBtnTree().getButton(
				IButtonConstPo.BTN_ADDCONTINUE);// 参照增行
		// 执行组按钮
		m_btnInvBillExcute = getBtnTree()
				.getButton(IButtonConstInv.BTN_EXECUTE);
		m_btnSendAudit = getBtnTree().getButton(
				IButtonConstInv.BTN_EXECUTE_AUDIT);// 送审
		m_btnInvBillAudit = getBtnTree().getButton(IButtonConstInv.BTN_AUDIT);// 审核
		m_btnInvBillUnAudit = getBtnTree().getButton(
				IButtonConstInv.BTN_EXECUTE_AUDIT_CANCEL);// 弃审
		m_btnInvBillConversion = getBtnTree().getButton(
				IButtonConstInv.BTN_REF_CANCEL);// 放弃转单
		...
		m_btnInvBillRefresh = getBtnTree().getButton(
				IButtonConstInv.BTN_BROWSE_REFRESH);// 卡片刷新
		m_btnInvBillGoFirstOne = getBtnTree().getButton(
				IButtonConstInv.BTN_BROWSE_TOP);// 首页
		m_btnInvBillGoNextOne = getBtnTree().getButton(
				IButtonConstInv.BTN_BROWSE_NEXT);// 下一页
		m_btnInvBillGoPreviousOne = getBtnTree().getButton(
				IButtonConstInv.BTN_BROWSE_PREVIOUS);// 上一页
		m_btnInvBillGoLastOne = getBtnTree().getButton(
				IButtonConstInv.BTN_BROWSE_BOTTOM);// 末页

		m_btnInvSelectAll = getBtnTree().getButton(
				IButtonConstInv.BTN_BROWSE_SELECT_ALL);// 全选
		m_btnInvDeselectAll = getBtnTree().getButton(
				IButtonConstInv.BTN_BROWSE_SELECT_NONE);// 全消
		...
		m_btnInvBillPreview = getBtnTree().getButton(
				IButtonConstInv.BTN_PRINT_PREVIEW);// 预览
		m_btnInvBillPrint = getBtnTree().getButton(
				IButtonConstInv.BTN_PRINT_PRINT);// 打印
		btnBillCombin = getBtnTree().getButton(
				IButtonConstInv.BTN_PRINT_DISTINCT);// 合并显示

		// 辅助查询组按钮
		m_btnInvBillAssist = getBtnTree().getButton(
				IButtonConstInv.BTN_ASSIST_QUERY);// 辅助查询
		m_btnLnkQuery = getBtnTree().getButton(
				IButtonConstInv.BTN_ASSIST_QUERY_RELATED);// 联查
		m_btnQueryForAudit = getBtnTree().getButton(
				IButtonConstInv.BTN_ASSIST_QUERY_WORKFLOW);// 审批流状态(状态查询)
		m_btnHqhp = getBtnTree().getButton(IButtonConstInv.BTN_HQHP);// 优质优价取价

		// 辅助功能组按钮
		m_btnOthersFuncs = getBtnTree().getButton(
				IButtonConstInv.BTN_ASSIST_FUNC);
		...
		m_btnDocManage = getBtnTree().getButton(
				IButtonConstInv.BTN_ASSIST_FUNC_DOCUMENT);// 文档管理
	}
	...
	private void setMaxMnyDigit(int iMaxDigit) {
		...
		// 金额相关
		getBillCardPanel().getBodyItem("noriginalcurmny").setDecimalDigits(
				iMaxDigit);
		getBillCardPanel().getBodyItem("noriginaltaxmny").setDecimalDigits(
				iMaxDigit);
		getBillCardPanel().getBodyItem("noriginalsummny").setDecimalDigits(
				iMaxDigit);
		getBillCardPanel().getBodyItem("noriginalpaymentmny").setDecimalDigits(
				iMaxDigit);
		...
		getBillCardPanel().getBodyItem("npaymentmny").setDecimalDigits(
				iMaxDigit);
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				// // 取得币种
				// String sCurrId = (String)
				// getBillListPanel().getBodyBillModel().getValueAt(i,
				// "ccurrencytypeid");
				// int[] iaExchRateDigit = null;
				// if(sCurrId == null){
				// sCurrId =
				// CurrParamQuery.getInstance().getLocalCurrPK(getPk_corp());//本币ID
				// BusinessCurrencyRateUtil caCurCorp = new
				// BusinessCurrencyRateUtil(getPk_corp());
				// }
				// // 得到折本、折辅汇率精度
				// iaExchRateDigit =
				// m_listPoPubSetUI2.getBothExchRateDigit(getPk_corp(),
				// sCurrId);
				// if (iaExchRateDigit == null || iaExchRateDigit.length == 0) {
				// }
				// else {
				getBillCardPanel().getBodyItem("nsummny").setDecimalDigits(
						iCurMnyDigit);// 本币价税合计
				getBillCardPanel().getBodyItem("ntaxmny").setDecimalDigits(
						iCurMnyDigit);// 本币税额
				getBillCardPanel().getBodyItem("nmoney").setDecimalDigits(
						iCurMnyDigit);// 本币无税金额
				getBillCardPanel().getBodyItem("npaymentmny").setDecimalDigits(
						iCurMnyDigit);// 本币累计付款
				// }
	...
	private void setMaxMnyDigitList(int iMaxDigit) {
		...
		// int iMaxDigit = nMaxDigit.intValue() ;
		// 金额相关
		getBillListPanel().getBodyItem("noriginalcurmny").setDecimalDigits(
				iMaxDigit);
		getBillListPanel().getBodyItem("noriginaltaxmny").setDecimalDigits(
				iMaxDigit);
		getBillListPanel().getBodyItem("noriginalsummny").setDecimalDigits(
				iMaxDigit);
		getBillListPanel().getBodyItem("noriginalpaymentmny").setDecimalDigits(
				iMaxDigit);
		...
		getBillListPanel().getBodyItem("npaymentmny").setDecimalDigits(
				iMaxDigit);
		getBillListPanel().getBodyItem("nexchangeotobrate").setDecimalDigits(
				iMaxDigit);
		...
		for (int i = 0; i < size; i++) {
			...
			getBillListPanel().getBodyItem("nsummny").setDecimalDigits(
					iCurMnyDigit);// 本币价税合计
			getBillListPanel().getBodyItem("ntaxmny").setDecimalDigits(
					iCurMnyDigit);// 本币税额
			getBillListPanel().getBodyItem("nmoney").setDecimalDigits(
					iCurMnyDigit);// 本币无税金额
			getBillListPanel().getBodyItem("npaymentmny").setDecimalDigits(
					iCurMnyDigit);// 本币累计付款
	...
	/**
	 * 作者：王印芬 功能：点击一个参照的按钮时，其他相关项的变化。 参数：ActionEvent e 捕捉到的ActionEvent事件 返回：无
	 * 例外：无 日期：(2002-3-13 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-03-20 wyf
	 * 提高效率，在该函数中即完成分支 wyf add/modify/delete 2002-03-21 begin/end 2002-09-18 wyf
	 * 加入对仓库参照的响应
	 */
	public void actionPerformed(ActionEvent e) {

		if (e.getSource() == ((UIRefPane) getBillCardPanel().getHeadItem(
		...
		// }
		else if (e.getSource() == ((UIRefPane) getBillCardPanel().getBodyItem(
				"cprojectphasename").getComponent()).getUIButton()) {
			actionPerformedProjectPhase(e);
		} else if (e.getSource() == ((UIRefPane) getBillCardPanel()
				.getBodyItem("cwarehousename").getComponent()).getUIButton()) {
			PuTool.restrictWarehouseRefByStoreOrg(getBillCardPanel(),
					getPk_corp(), getBillCardPanel().getHeadItem(
	...
	private void afterEditWhenBodyCurrency(BillEditEvent e) {
		...
		UFDouble ninvoicenum = (UFDouble) getBillCardPanel().getBodyValueAt(
				iRow, "nexchangeotobrate");
		UFDouble ncurprice = (UFDouble) getBillCardPanel().getBodyValueAt(iRow,
				"noriginalcurprice");
		UFDouble summy = (UFDouble) getBillCardPanel().getBodyValueAt(iRow,
				"noriginalsummny");
		String sChangedKey = null;
		if (ninvoicenum != null && ninvoicenum.doubleValue() > 0.00
				&& ncurprice != null && ncurprice.doubleValue() > 0.00) {
		...
		afterEditWhenBodyRelationsCal(new BillEditEvent(getBillCardPanel()
				.getBodyItem(sChangedKey), getBillCardPanel().getBodyValueAt(
				iRow, sChangedKey), sChangedKey, iRow, BillItem.BODY));
		updateUI();
	}
	...
	private void afterEditWhenBodyRelationsCal(BillEditEvent e) {
		...
		// }
		// UFDouble dOldNum = (oNum == null ||
		// oNum.toString().trim().equals("")) ?
		// VariableConst.ZERO
	...
	private void setExchangeRateBody(int iRow, boolean bResetExchValue,
		...
		String dInvoiceDate = getBillCardPanel().getHeadItem("dinvoicedate")
				.getValue();
		String sCurrId = (String) getBillCardPanel().getBodyValueAt(iRow,
				"ccurrencytypeid");
		if (sCurrId == null || sCurrId.trim().length() == 0) {
			sCurrId = getBillCardPanel().getHeadItem("ccurrencytypeid")
					.getValue();
		}
		// 首先设置显示精度
		setRowDigits_ExchangeRate(getPk_corp(), iRow, getBillCardPanel()
				.getBillModel(), m_cardPoPubSetUI2);
		UFDouble nexchangeotobrate = null;
		if (items != null) {
			nexchangeotobrate = items.getNexchangeotobrate();
		} else {
			nexchangeotobrate = PuPubVO
					.getUFDouble_NullAsZero(getBillCardPanel().getBodyValueAt(
							iRow, "nexchangeotobrate"));
		}
		// 设置值
		if (bResetExchValue && dInvoiceDate != null
			...
			daRate = m_cardPoPubSetUI2.getBothExchRateValue(getPk_corp(),
					sCurrId, new UFDate(dInvoiceDate));
			// UFDouble nexchangeotobrate =
			// (UFDouble)getInvBillPanel().getBodyValueAt(iRow,
			// "nexchangeotobrate");
			// 币种编辑后加入默认值
			if (isEditCur == 1) {
				...
				if (nexchangeotobrate != null) {
					getBillCardPanel().setBodyValueAt(nexchangeotobrate, iRow,
							"nexchangeotobrate");
				} else {
					getBillCardPanel().setBodyValueAt(daRate[0], iRow,
							"nexchangeotobrate");
				...
				&& nexchangeotobrate.doubleValue() > 0) {
			getBillCardPanel().setBodyValueAt(nexchangeotobrate, iRow,
					"nexchangeotobrate");
		...
		baEditable = m_cardPoPubSetUI2.getBothExchRateEditable(getPk_corp(),
				sCurrId);

		getBillCardPanel().getBillModel().setCellEditable(iRow,
	...
	private void actionPerformedProjectPhase(ActionEvent e) {
		...
		// 项目管理ID
		String strProjectMngPk = (String) getBillCardPanel().getBodyValueAt(
				nRow, "cprojectid");
		// 设置根据项目ID的项目阶段参照
		UIRefPane pane = ((UIRefPane) getBillCardPanel().getBodyItem(
	...
	public void afterEdit(BillEditEvent e) {
		...
		if (getCurPanelMode() == INV_PANEL_CARD) {
			...
			// 处理自定义项PK
			setHeadDefPK(e);
			if (e.getSource() == getBillCardPanel()
					.getHeadItem("cvendormangid").getComponent()) {
				afterEditInvBillVendor(e);
			} else if (e.getSource() == getBillCardPanel().getHeadItem(
				...
				if (getBillCardPanel().getHeadItem("ntaxrate") != null) {
					getBillCardPanel().setHeadItem(
							"ntaxrate",
							getBillCardPanel().getHeadItem("ntaxrate")
									.getValue());
				}
			} else if (e.getSource() == getBillCardPanel().getHeadItem(
					"darrivedate").getComponent()) {
				afterEditInvBillDArriveDate(e);
			} else if (e.getSource() == getBillCardPanel().getHeadItem(
					"cstoreorganization").getComponent()) {
				PuTool.afterEditStoreOrgToWarehouse(getBillCardPanel(), e,
						getPk_corp(), getBillCardPanel().getHeadItem(
				...
				// 库存组织ID
				String sCstoreorganization = getBillCardPanel().getHeadItem(
						"cstoreorganization").getValue();
				for (int i = 0; i < size; i++) {
					sMangIds[i] = (String) getBillCardPanel().getBillModel()
				...
				if (uPrice != null) {
					for (int i = 0; i < size; i++)
						getBillCardPanel().getBillModel().setValueAt(uPrice[i],
								i, "nplanprice");
				}
			} else if (e.getSource() == getBillCardPanel().getHeadItem(
			...
			// 表体
				...
				if (e.getKey().equals("cprojectname")) {
					...
					try {
						if (e.getKey().equals("ninvoicenum")) {
							aftereditWhenBodyInvoicenum(this,
									getBillCardPanel(), e);
						}
		...
		if (e.getKey().equals("vinvoicecode")) {
			...
			if (PuPubVO.getString_TrimZeroLenAsNull(getBillCardPanel()
					.getHeadItem("vinvoicecode").getValue()) != null) {
				getBillCardPanel().getHeadItem("vinvoicecode").setValue(
						getBillCardPanel().getHeadItem("vinvoicecode")
	...
	private void afterEditWhenHeadCurrency(BillEditEvent e) {
		...
		// 关闭合计开关
		boolean bOldNeedCalc = getBillCardPanel().getBillModel()
				.isNeedCalculate();
		getBillCardPanel().getBillModel().setNeedCalculate(false);

		try {
			String sCurrId = getBillCardPanel().getHeadItem("ccurrencytypeid")
					.getValue();
			String dInvoiceDate = getBillCardPanel()
					.getHeadItem("dinvoicedate").getValue();
			...
			if (CurrParamQuery.getInstance().getLocalCurrPK(getPk_corp()) != null
					&& !listAllCurrId.contains(CurrParamQuery.getInstance()
							.getLocalCurrPK(getPk_corp()))) {
				listAllCurrId.add(CurrParamQuery.getInstance().getLocalCurrPK(
						getPk_corp()));
			}
			...
			getBillCardPanel().getBillModel().execEditFormulaByKey(-1,
					"currname");// 优化
			// V31
	...
	private void setExchangeRateHead(String dInvoicedate, String sCurrId) {
		...
		int[] iaExchRateDigit = m_cardPoPubSetUI2.getBothExchRateDigit(
				getPk_corp(), sCurrId);
		getBillCardPanel().getHeadItem("nexchangeotobrate").setDecimalDigits(
				iaExchRateDigit[0]);
		// 设置值
		...
		// 可编辑性
		boolean[] iaEditable = m_cardPoPubSetUI2.getBothExchRateEditable(
				getPk_corp(), sCurrId);
		getBillCardPanel().getHeadItem("nexchangeotobrate").setEnabled(
				iaEditable[0]);
	...
	private void afterEditInvBillDArriveDate(BillEditEvent e) {
		String strCurr = getBillCardPanel().getHeadItem("ccurrencytypeid")
				.getValue();
	...
	/**
	 * 作者：王印芬 功能：表头部门变化后，业务员相应变化 参数：BillEditEvent e 捕捉到的BillEditEvent事件 返回：无
	 * 例外：无 日期：(2002-3-13 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-03-20 wyf
	 * 提高效率，在该函数中即完成分支 wyf add/modify/delete 2002-03-21 begin/end
	 */
	private void afterEditInvBillDept(BillEditEvent e) {

		UIRefPane pane = (UIRefPane) getBillCardPanel().getHeadItem("cdeptid")
				.getComponent();
	...
	/**
	 * 作者：王印芬 功能：表头业务员变化后，部门相应变化 参数：BillEditEvent e 捕捉到的BillEditEvent事件 返回：无
	 * 例外：无 日期：(2002-3-13 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-03-20 wyf
	 * 提高效率，在该函数中即完成分支 wyf add/modify/delete 2002-03-21 begin/end
	 */
	private void afterEditInvBillEmployee(BillEditEvent e) {
		...
		// 设置该部门的默认业务员, 该部门对应的业务员动态参照
		String cemployeeid = ((UIRefPane) getBillCardPanel().getHeadItem(
	...
	/**
	 * 作者：王印芬 功能：表体自由项变化后，五个自由项的值相应改变 参数：BillEditEvent e 捕捉到的BillEditEvent事件
	 * 返回：无 例外：无 日期：(2002-3-13 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-03-20 wyf
	 * 提高效率，在该函数中即完成分支 wyf add/modify/delete 2002-03-21 begin/end
	 */
	private void afterEditInvBillFree(BillEditEvent e) {
		...
		((FreeItemRefPane) getBillCardPanel().getBodyItem("vfree0")
				.getComponent()).setFreeItemParam(invVO);
	}

	/**
	 * 作者：王印芬 功能：表头散户变化后，相应电话等变化 参数：BillEditEvent e 捕捉到的BillEditEvent事件 返回：无
	 * 例外：无 日期：(2002-3-13 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-03-20 wyf
	 * 提高效率，在该函数中即完成分支 wyf add/modify/delete 2002-03-21 begin/end
	 */
	private void afterEditInvBillFreeCust(BillEditEvent e) {

		String freeId = getBillCardPanel().getHeadItem("cfreecustid")
				.getValue();
	...
	private void afterEditInvBillInvcode(BillEditEvent e) {
		...
		for (int i = iBeginRow; i <= iEndRow; i++) {
			...
			if (sBodyCurrId == null || sBodyCurrId.equals("")) {
				...
				// 获取币种名称
				// 驱动表体相关币种的改动
				afterEditWhenBodyCurrency(new BillEditEvent(getBillCardPanel()
		...
		if (it.getEditFormulas() != null && it.getEditFormulas().length > 0) {
			getBillCardPanel().getBillModel().execFormulas(
					it.getEditFormulas(), iBeginRow, iEndRow);
		}
		// For V56 缓存费用存货信息
		PuTool.loadFeeInventoryBatch(new InvoiceVO[] { getCurVOonCard() },
				"cbaseid");
	...
	/**
	 * 作者：王印芬 功能：表体项目变化后，项目阶段ID清空 参数：BillEditEvent e 捕捉到的BillEditEvent事件 返回：无
	 * 例外：无 日期：(2002-3-13 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-03-20 wyf
	 * 提高效率，在该函数中即完成分支 wyf add/modify/delete 2002-03-21 begin/end
	 */
	 ...
	 * 例外：无 日期：(2002-3-13 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-03-20 wyf
	 * 提高效率afterEdit()中完成分支 wyf add/modify/delete 2002-03-21 begin/end
	 * 2002-08-26 wyf 扣税类别未选中时不进行计算 wyf add/modify/delete 2002-03-21 begin/end
	 */
	public void afterEditInvBillRelations(BillEditEvent e) {
		int iRow = e.getRow();
		// 设置原\本\辅精度
		// setRowDigits_Mny(getPk_corp(),
		// iRow,getInvBillPanel().getBillModel(),m_cardPoPubSetUI2);
		int[] descriptions = new int[] { RelationsCal.DISCOUNT_TAX_TYPE_NAME,
				...
				RelationsCal.NET_TAXPRICE_ORIGINAL, RelationsCal.TAXRATE,
				RelationsCal.MONEY_ORIGINAL, RelationsCal.TAX_ORIGINAL,
				RelationsCal.SUMMNY_ORIGINAL };

		InvoiceVO tempVO = new InvoiceVO(getBillCardPanel().getRowCount());
		getBillCardPanel().getBillValueVO(tempVO);
		...
		String[] keys = new String[] { s, "idiscounttaxtype", "ninvoicenum",
	...
	private void afterEditInvBillVendor(BillEditEvent e) {
		...
		// added by fangy 2002-10-23 12:22 end
			...
			// 供应商
			String cvendormangid = getBillCardPanel().getHeadItem(
			...
			// 收付款协议
			String payTermId = getBillCardPanel()
					.getHeadItem("ctermprotocolid").getValue();
			...
			// 收付款单位
			String sPayUnit = getBillCardPanel().getHeadItem("cpayunit")
					.getValue();
			// 丰原生化项目问题：修改发票上的供应商为B单位，保存签字后，生成应付单上的供应商还是是A单位
			// if (sPayUnit == null || sPayUnit.trim().length() == 0) {
			// sPayUnit = (String)
			// PiPqPublicUIClass.getAResultFromTable("bd_cumandoc",
			// "pk_payterm",
			// "pk_cumandoc", cvendormangid);
			...
			if (strFreeFlag.equals("N")) {
				...
				((UIRefPane) getBillCardPanel().getHeadItem("cfreecustid")
						.getComponent()).getUITextField().setText(null);
				((UIRefPane) getBillCardPanel().getHeadItem("caccountbankid")
						.getComponent()).setButtonVisible(true);
				getBillCardPanel().getHeadItem("caccountbankid").setEnabled(
						true);
			...
			if (getBillCardPanel().getHeadItem("cemployeeid").getValue() == null) {
				...
				// ;
				String employeeID = (String) PiPqPublicUIClass
		...
		if (getCurPanelMode() == INV_PANEL_CARD) {
			if (e.getPos() == BillItem.BODY) {
				...
				if (e.getKey().equals("vfree0")) {
					return PuTool.beforeEditInvBillBodyFree(getBillCardPanel(),
				...
				// 批次号
					...
					PuTool.beforeEditWhenBodyBatch(getBillCardPanel(),
							getPk_corp(), e, new String[] {
									// 存货管理ID、编码、名称、规格、型号、单位
									"cmangid", "invcode", "invname", "invspec",
									"invtype", "measname",
									// 辅计量单位、是否辅计量管理
				...
				// 仓库,从采购入库单生成的发票仓库不能更改
					...
					Object oCupsourceBillType = getBillCardPanel()
							.getBodyValueAt(e.getRow(), "cupsourcebilltype");
							...
							showHintMessage(nc.ui.ml.NCLangRes.getInstance()
									.getStrByID("40040401",
											"UPP40040401-000009")/*
																 ...
																 * "从采购入库单生成的发票仓库不能更改"
																 */);
							getBillCardPanel().getBodyItem("cwarehousename")
									.setEnabled(false);
						}
					} else
						getBillCardPanel().getBodyItem("cwarehousename")
	...
	public boolean beforeEdit(BillItemEvent e) {
		...
		if (e.getSource().equals(
			...
			PuTool.restrictStoreOrg(getBillCardPanel().getHeadItem(
					"cstoreorganization").getComponent(), false);
		}

		// 用供应商限制开户银行
		if (e.getSource().equals(
				getBillCardPanel().getHeadItem("caccountbankid"))) {
			String cvendormangid = getBillCardPanel().getHeadItem(
					"cvendormangid").getValue();
			String cvendorbaseid = (String) PiPqPublicUIClass
					.getAResultFromTable("bd_cumandoc", "pk_cubasdoc",
	...
	private boolean beforeEditInvBillBodyMemo(BillEditEvent e) {
		...
		((UIRefPane) getBillCardPanel().getBodyItem("vmemo").getComponent())
				.setText((String) ob);
	...
	private boolean calNativeAndAssistCurrValue(InvoiceVO invVO) {
		...
		try {
			...
			for (int i = 0; i < invVO.getBodyVO().length; i++) {
				...
				nmoney = POPubSetUI.getCurrArith_Busi(pk_corp).getAmountByOpp(
						itemVO.getCcurrencytypeid(),
						CurrParamQuery.getInstance().getLocalCurrPK(pk_corp),
						itemVO.getNoriginalcurmny(),
						itemVO.getNexchangeotobrate(), strRateDate);
				nsummny = POPubSetUI.getCurrArith_Busi(pk_corp).getAmountByOpp(
						itemVO.getCcurrencytypeid(),
						CurrParamQuery.getInstance().getLocalCurrPK(pk_corp),
				...
				nmoney = POPubSetUI.getCurrArith_Finance(pk_corp)
						.getAmountByOpp(
								itemVO.getCcurrencytypeid(),
								CurrParamQuery.getInstance().getLocalCurrPK(
										pk_corp),// POPubSetUI.getCurrArith_Busi(pk_corp).getLocalCurrPK(),
	...
	public void computeValueFrmOriginal(int row) throws Exception {
		...
		// 折本汇率
		Object bRate = getBillCardPanel().getBodyValueAt(row,
				"nexchangeotobrate");
		if (bRate == null || bRate.toString().trim().length() == 0) {
			bRate = getBillCardPanel().getHeadItem("nexchangeotobrate")
					.getValue();
		...
		UFDouble ufNoriginalcurmny = PuPubVO
				.getUFDouble_ZeroAsNull(getBillCardPanel().getBodyValueAt(row,
						"noriginalcurmny"));
		...
		if (getBillCardPanel().getBodyValueAt(row, "ccurrencytypeid") == null
				|| getBillCardPanel().getBodyValueAt(row, "ccurrencytypeid")
						.toString().trim().equals("")) {
			String strCurrTypeId = ((UIRefPane) getBillCardPanel().getHeadItem(
					"ccurrencytypeid").getComponent()).getRefPK();
			getBillCardPanel().setBodyValueAt(strCurrTypeId, row,
					"ccurrencytypeid");
		}
		try {
			// 原币金额
			if (ufNoriginalcurmny == null
				...
				ufMoney = POPubSetUI.getCurrArith_Busi(getPk_corp())
						.getAmountByOpp(
								...
								ufNoriginalcurmny,
								ufBRate,
								nc.ui.po.pub.PoPublicUIClass.getLoginDate()
										.toString());
			}
		...
		try {
			for (int i = 0; i < items.length; i++) {
				...
				if (dOrderDate != null) {
					...
					daRate = m_listPoPubSetUI2.getBothExchRateValue(
	...
	private void dealWhenCopy() {
		...
		getBillCardPanel().getHeadItem("dinvoicedate").setValue(
				nc.ui.pub.ClientEnvironment.getInstance().getDate());
		getBillCardPanel().getHeadItem("darrivedate").setValue(
				nc.ui.pub.ClientEnvironment.getInstance().getDate());

		// 设置制单人
		((UIRefPane) getBillCardPanel().getTailItem("coperator").getComponent())
				.setPK(nc.ui.pub.ClientEnvironment.getInstance().getUser()
						.getPrimaryKey());
		((UIRefPane) getBillCardPanel().getTailItem("dauditdate")
		...
		try {
			...
			setInventoryRefFilter(getBillCardPanel().getHeadItem("cbiztype")
					.getValue());
		} catch (Exception e) {
			SCMEnv.out(e);
		}
	...
	private void execBillQuery(InvQueDlg curDlg) {
		...
		showHintMessage(getHeadHintText()
				+ nc.ui.ml.NCLangRes.getInstance().getStrByID("40040401",
						"UPP40040401-000013")/* @res "正在查询" */
				+ CommonConstant.SPACE_MARK + CommonConstant.BEGIN_MARK
		...
		InvoiceVO[] VOs = null;
		try {
			VOs = InvoiceHelper.queryInvoiceVOsByCondsMy(getClientEnvironment()
					.getUser().getPrimaryKey(), normalVOs, definedVOs);
	...
	public void execHeadTailFormula(InvoiceVO vo) {
		...
		String[] saFormula = new String[] { "getColValue(bd_calbody,bodyname,pk_calbody,cstoreorganization)" };
		PuTool.getFormulaParse().setExpressArray(saFormula);

		int iFormulaLen = saFormula.length;
		for (int i = 0; i < iFormulaLen; i++) {
			PuTool.getFormulaParse().addVariable("cstoreorganization",
		...
		if (nc.vo.scm.pu.PuPubVO.getString_TrimZeroLenAsNull(refpane
	...
	public InvBillPanel getBillCardPanel() {
		if (m_billPanel == null) {
			try {
				m_billPanel = new InvBillPanel(nc.ui.pub.ClientEnvironment
						.getInstance(), this);
			} catch (java.lang.Exception e) {
				SCMEnv.out(e);
				MessageDialog.showHintDlg(this,
			...
			// 按行不同币种设置金额精度
			new DefaultCurrTypeBizDecimalListener(m_billPanel.getBillModel(),
					"ccurrencytypeid", m_itemMny_bu);
			new DefaultCurrTypeDecimalAdapter(m_billPanel.getBillModel(),
					"ccurrencytypeid", m_itemMny_fi);
			// 表体排序监听
			m_billPanel.getBillModel().addSortRelaObjectListener2(
	...
	public InvListPanel getBillListPanel() {
		...
		if (m_listPanel == null) {
			try {
				m_listPanel = new InvListPanel(nc.ui.pub.ClientEnvironment
						.getInstance(), this);
			...
			new DefaultCurrTypeBizDecimalListener(m_listPanel
					.getBodyBillModel(), "ccurrencytypeid", m_itemMny_bu);
			new DefaultCurrTypeDecimalAdapter(m_listPanel.getBodyBillModel(),
					"ccurrencytypeid", m_itemMny_fi);

			// since v55, 支持整行选中
			// PuTool.setLineSelectedList(m_listPanel);
			// 表体排序监听
			m_listPanel.getBodyBillModel().addSortRelaObjectListener2(
					new IBillRelaSortListener2() {
						public Object[] getRelaSortObjectArray() {
							return getCurrBodyVOs();
						}
	...
	public nc.vo.pi.InvoiceVO[] getInvVOsByContinue() {
		...
		for (int i = 0; i < invoiceItemVO.length; ++i) {
			UFDouble nexchgRate = (UFDouble) getBillCardPanel().getBodyValueAt(
					i, "nexchangeotobrate");
	...
	public InvQueDlg getQueDlg() {
		...
		if (m_InvQueDlg == null) {
			m_InvQueDlg = new InvoiceUIQueDlg(this, getModuleCode(), null,
					ScmConst.PO_Invoice, getCurOperator(), getPk_corp());
			// m_InvQueDlg.setValueRef("po_invoice.dinvoicedate","日历");
			// m_InvQueDlg.setDefaultValue("po_invoice.dinvoicedate",nc.ui.pub.ClientEnvironment.getInstance().getDate().toString(),"");
			// setDefaultValue("po_invoice.pk_purcorp",ClientEnvironment.getInstance().getCorporation().getPrimaryKey()
			// ,"");
			// 加载自定义项名称
	...
	private InvoiceVO getSavedInvVOFromBill() {
		...
		if (getBillCardPanel().getBillData().getBodyItem("nexchangeotobrate") != null) {
			BillItem exchangeItem = getBillCardPanel().getBillData()
		...
		// iinvoicetype
		UIComboBox comItem = (UIComboBox) getBillCardPanel().getHeadItem(
				"iinvoicetype").getComponent();
		head.setIinvoicetype(new Integer(comItem.getSelectedIndex()));
		// finitflag
		UICheckBox initCheck = (UICheckBox) getBillCardPanel().getHeadItem(
		...
		// cvendorbaseid
		if ((head.getCvendorbaseid() == null || head.getCvendorbaseid().trim()
				.equals(""))
				&& (head.getCvendormangid() != null && head.getCvendormangid()
						.trim() != "")) {
			// String cvendorbaseid =
		...
		// cvendormaneid
		if ((head.getCvendormangid() == null || head.getCvendormangid().trim()
				.equals(""))
				&& (head.getCvendorbaseid() != null && head.getCvendorbaseid()
						.trim() != "")) {
			// String cvendorbaseid =
			// getResultFromFormula("getColValue(bd_cumandoc,pk_cubasdoc,pk_cumandoc,cvendormangid)","cvendormangid",head.getCvendormangid())
			// ;
		...
		head.setPk_corp(nc.ui.pub.ClientEnvironment.getInstance()
				.getCorporation().getPrimaryKey());
		head.setCaccountyear(nc.ui.pub.ClientEnvironment.getInstance()
				.getAccountYear());
		...
		if (head.getCinvoiceid() == null
			...
			head.setCoperator(nc.ui.pub.ClientEnvironment.getInstance()
		...
		for (int i = 0; i < iLen; i++) {
			...
			String[] value = new String[] { new Integer(i + 1).toString() };
			if (nexchangeotobrate == null
					|| (nexchangeotobrate != null && (nexchangeotobrate
							.doubleValue() < 0 || nexchangeotobrate
							.doubleValue() == 0))) {
				// return "表头：\n折本汇率不能为空！" ;
				MessageDialog.showWarningDlg(this, nc.ui.ml.NCLangRes
						.getInstance().getStrByID("SCMCOMMON",
		...
		// =======改变的项：正常，新增，修改，删除
		InvoiceItemVO[] changedBodyVOs = (InvoiceItemVO[]) getBillCardPanel()
		...
		if (changedBodyVOs != null && changedBodyVOs.length != 0) {
			...
			for (int i = 0; i < changedBodyVOs.length; i++) {
				if (changedBodyVOs[i].getStatus() == VOStatus.DELETED
	...
	private InvoiceVO getSavedVO() {
		...
		if (sKey != null && sKey.trim().length() > 0) {
			...
			retVO.setSource(InvoiceVO.FROM_HAND);
				...
				&& sUpsourceBillType.trim().length() > 0) {
			if (sUpsourceBillType.equals(ScmConst.PO_Order)
					|| sUpsourceBillType.equals(ScmConst.SC_Order))
				retVO.setSource(InvoiceVO.FROM_ORDER);
	...
	public String getTitle() {
		...
		String title = nc.ui.ml.NCLangRes.getInstance().getStrByID("40040401",
				"UPP40040401-000160")/*
									 * @res "维护发票"
									 */;
	 ...
	 * 日期：(2001-5-13 11:39:21) 修改日期，修改人，修改原因，注释标志：
	 */
	public nc.vo.pub.AggregatedValueObject getLinkQueryVo(String pk_corp)
			throws Exception {
		...
		try {
			VOs = InvoiceHelper.queryInvoiceVOsByCondsMy(getClientEnvironment()
					.getUser().getPrimaryKey(), normalVOs, null);
			// added by czp on 2003-03-01 begin
			if (VOs != null && VOs.length > 0) {
				loadItemsForInvoiceVOs(new InvoiceVO[] { VOs[0] });
			}
	...
	private void initBillListener() {
		...
		// 项目参照加监听
		((UIRefPane) getBillCardPanel().getBodyItem("cprojectphasename")
				.getComponent()).getUIButton().addActionListener(this);
		// 自由项加监听
		((UIRefPane) getBillCardPanel().getBodyItem("vfree0").getComponent())
	...
	private void initButtons() {
		...
		// getBtnMaintain();
		// 业务类型初始化
		PfUtilClient.retBusinessBtn(m_btnInvBillBusiType, ClientEnvironment
		...
		PuTool.initBusiAddBtns(m_btnInvBillBusiType, m_btnInvBillNew,
				ScmConst.PO_Invoice, getPk_corp());
		// 支持产业链功能扩展
	...
	private void initCard() {
		...
		m_btnInvShift.setName(NCLangRes.getInstance().getStrByID("SCMCOMMON",
				"UPPSCMCommon-000464")/** @res "列表显示" */
		);
	...
	private void initi() {
		...
		try {
			...
			if (t != null && t.size() > 0) {
				...
				isAllowedModifyByOther = (oTemp == null || oTemp.equals("N")) ? false
						: true;
				oTemp = t.get("PO30");
				m_sStock2InvoiceSettleMode = (oTemp == null) ? null : oTemp
						.toString();
				oTemp = t.get("PO46");
				m_sOrder2InvoiceSettleMode = (oTemp == null) ? null : oTemp
						.toString();
				oTemp = t.get("PO52");
				m_sZGYF = (oTemp == null) ? null : oTemp.toString();
				oTemp = t.get("PO84");
				iChange = "调整折扣".equalsIgnoreCase((oTemp == null) ? null
						: oTemp.toString()) ? 7 : 8;
	...
	private void initRefpane(BillData bd) {
		...
		refpane
				.getRefModel()
				.setWherePart(
						" (bd_deptdoc.deptattr = '2' or bd_deptdoc.deptattr = '4') and bd_deptdoc.pk_corp = '"
								+ pk_corp + "' ");
		...
		UIRefPane refBiz = (UIRefPane) bd.getHeadItem("cemployeeid")
				.getComponent();
		// refpane.getRefModel().addWherePart(" and (bd_psndoc.pk_deptdoc in
		// (select
		// bd_deptdoc.pk_deptdoc from bd_deptdoc where (bd_deptdoc.deptattr = 2
		// or
		...
		if (bd.getBodyItem("cassistunitname") != null) {
			refpanel = ((UIRefPane) (bd.getBodyItem("cassistunitname")
					.getComponent()));
		...
		if (bd.getBodyItem("nexchangerate") != null) {
			refpanel = (UIRefPane) bd.getBodyItem("nexchangerate")
					.getComponent();
	...
	private boolean isCouldMakedForABizType(String strBizType) {
		...
		// //有自制单据，则可增行
		// if
		// (billSourceVOs[i].getPk_billtype().toUpperCase().equals("MAKEFLAG"))
		// {
		// return true;
		// }
		// }
	...
	private void loadBDData() {
		...
		refpnl = (UIRefPane) getBillCardPanel().getHeadItem("cbiztype")
				.getComponent();
		...
		strVarValue = (String) vo.getParentVO().getAttributeValue(
				"cstoreorganization");
		refpnl = (UIRefPane) getBillCardPanel().getHeadItem(
				"cstoreorganization").getComponent();
		strValue = refpnl.getUITextField().getText();
		if (strVarValue != null
				&& (strValue == null || strValue.trim().equals(""))) {
		...
		strVarValue = (String) vo.getParentVO().getAttributeValue(
				"cvendorbaseid");
		refpnl = (UIRefPane) getBillCardPanel().getHeadItem("cvendormangid")
				.getComponent();
		strValue = refpnl.getUITextField().getText();
		if (strVarValue != null
				&& (strValue == null || strValue.trim().equals(""))) {
		...
		/* 业务员 */
		// strVarValue = (String)
		// vo.getParentVO().getAttributeValue("cemployeeid");
		// refpnl = (UIRefPane)
		...
		refpnl = (UIRefPane) getBillCardPanel().getHeadItem("cfreecustid")
				.getComponent();
		strValue = refpnl.getUITextField().getText();
		if (strVarValue != null
				&& (strValue == null || strValue.trim().equals(""))) {
		...
		strVarValue = (String) vo.getParentVO().getAttributeValue(
				"ctermprotocolid");
		refpnl = (UIRefPane) getBillCardPanel().getHeadItem("ctermprotocolid")
				.getComponent();
		strValue = refpnl.getUITextField().getText();
		if (strVarValue != null
				&& (strValue == null || strValue.trim().equals(""))) {
		...
		strVarValue = (String) vo.getParentVO().getAttributeValue(
				"ccurrencytypeid");
		refpnl = (UIRefPane) getBillCardPanel().getHeadItem("ccurrencytypeid")
				.getComponent();
		strValue = refpnl.getUITextField().getText();
		if (strVarValue != null
				&& (strValue == null || strValue.trim().equals(""))) {
		...
		if (getBillCardPanel().getHeadItem("cvendoraccount") != null) {
			strVarValue = getBillCardPanel().getHeadItem("cvendoraccount")
					.getValue();
			if (strVarValue != null && strVarValue.trim().length() == 20) {
				String cfreecustid = getBillCardPanel().getHeadItem(
				...
				if (cfreecustid != null && cfreecustid.trim().length() > 0) {
					...
					UIRefPane refpane = (UIRefPane) getBillCardPanel()
							.getHeadItem("caccountbankid").getComponent();
					String pkcorp = getBillCardPanel().getCorp();
					String vendorbaseid = (String) getBillCardPanel()
				...
				Object ob = getBillCardPanel().execHeadFormula(strFormula);
				getBillCardPanel().getHeadItem("cvendoraccount").setValue(
						(String) ob);
			}
		}
	}
	...
	/**
	 * 作者：方益 功能：动态装载表体VO 参数： 返回： 例外： 日期：(2003-02-08 13:09:22)
	 * 修改日期，修改人，修改原因，注释标志：
	 * 
	 * @param inv
	 *            nc.vo.pi.InvoiceVO
	...
	public void onActionAppendLine() {
		...
		nc.ui.scm.pub.report.BillRowNo.addLineRowNo(getBillCardPanel(),
				nc.vo.scm.pu.BillTypeConst.PO_INVOICE, "crowno");
	...
	public void onActionInsertLines(int iBeginRow, int iEndRow, int iInsertCount) {
		...
		for (int i = 0; i < iInsertCount; i++) {
			...
			// 设置新插行的默认值
			// if (getInvBillPanel().getBillTable().getSelectedRows().length >
			// 0) {
		...
		// 设置行号
		BillRowNo.insertLineRowNos(getBillCardPanel(),
				nc.vo.scm.pu.BillTypeConst.PO_INVOICE, "crowno", iFinalEndRow,
				iInsertCount);
	...
	private void setPopMenuBtnsEnable() {
		// 没有分配行操作权限
		if (m_btnInvBillRowOperation == null
				|| m_btnInvBillRowOperation.getChildCount() == 0) {
		...
		// 分配行操作权限
			...
			getBillCardPanel().getBodyPanel().getMiAddLine().setEnabled(
					isPowerBtn(m_btnInvBillAddRow.getCode()));
			getBillCardPanel().getBodyPanel().getMiCopyLine().setEnabled(
					isPowerBtn(m_btnInvBillCopyRow.getCode()));
			getBillCardPanel().getBodyPanel().getMiDelLine().setEnabled(
					isPowerBtn(m_btnInvBillDeleteRow.getCode()));
			getBillCardPanel().getBodyPanel().getMiInsertLine().setEnabled(
					isPowerBtn(m_btnInvBillInsertRow.getCode()));
			getBillCardPanel().getBodyPanel().getMiPasteLine().setEnabled(
					isPowerBtn(m_btnInvBillPasteRow.getCode()));
			// 粘贴到行尾与粘贴可用性逻辑相同
			getBillCardPanel().getBodyPanel().getMiPasteLineToTail()
					.setEnabled(isPowerBtn(m_btnInvBillPasteRowTail.getCode()));
			if (m_miReSortRowNo != null) {
				m_miReSortRowNo.setEnabled(isPowerBtn(m_btnReSortRowNo
						.getCode()));
			}
			if (m_miCardEdit != null) {
	...
	private void onAdd() {
		...
		// 业务类型
		UIRefPane busiRef = (UIRefPane) getBillCardPanel().getHeadItem(
		...
		getBillCardPanel().getHeadItem("idiscounttaxtype").setValue(
				new Integer(1));
		// 制单人
		((UIRefPane) getBillCardPanel().getTailItem("coperator").getComponent())
				.setPK(nc.ui.pub.ClientEnvironment.getInstance().getUser()
		...
		try {
			...
			String sLocalCurrId = CurrParamQuery.getInstance().getLocalCurrPK(
					getPk_corp());
			getBillCardPanel().getHeadItem("ccurrencytypeid").setValue(
					sLocalCurrId);
			// 汇率
	 ...
	 */
	private void setInventoryRefFilter(String cBizType)
			throws BusinessException {
		boolean checker = false;
		if (!cBizTypeTable.contains(cBizType)) {
			Object oTemp = CacheTool.getCellValue("bd_busitype", "pk_busitype",
					"verifyrule", cBizType);
			if (oTemp != null) {
	...
	private void onAppendLine() {
		...
		BillRowNo.addLineRowNo(getBillCardPanel(), ScmConst.PO_Invoice,
				"crowno");
		SCMEnv.out("m_btnInvBillDeleteRow==" + m_btnInvBillDeleteRow.isPower());
		// 浮动菜单右键功能权限控制
		setPopMenuBtnsEnable();
	...
	public void onUnAudit() {
		InvoiceVO[] proceVOs = new InvoiceVO[] { getInvVOs()[getCurVOPos()] };
		...
		try {
			...
			for (int i = 0; i < proceVOs.length; i++) {
				...
				proceVOs[i].getHeadVO().setCoperatoridnow(
						getClientEnvironment().getUser().getPrimaryKey());
				proceVOs[i].getHeadVO().setAttributeValue("cauditpsnold",
						proceVOs[i].getHeadVO().getCauditpsn());
				...
				for (int i = 0; i < proceVOs.length; i++) {
					...
					for (int j = 0; j < tempBodyVO.length; j++) {
						if (tempBodyVO[j].getCupsourcebilltype() == null)
							continue;// 自制采购发票
						if ((tempBodyVO[j].getCupsourcebilltype().equals("45") || tempBodyVO[j]
								.getCupsourcebilltype().equals("47"))
								&& m_sOrder2InvoiceSettleMode != null
								&& m_sStock2InvoiceSettleMode.equals("审批时自动结算")) {
							if (tempBodyVO[j].getNaccumsettmny() != null
									&& tempBodyVO[j].getNaccumsettmny()
											.doubleValue() != 0.0) {
								MessageDialog
										.showHintDlg(
												this,
			 ...
			 * 二次开发插件支持 by zhaoyha at 2009.2.16
			 */
			getInvokeEventProxy().beforeAction(nc.vo.scm.plugin.Action.UNAUDIT,
					proceVOs);

			PfUtilClient.processBatch("UNAPPROVE"
					+ nc.ui.pub.ClientEnvironment.getInstance().getUser()
							.getPrimaryKey(),
					nc.vo.scm.pu.BillTypeConst.PO_INVOICE, ClientEnvironment
			...
			if (PfUtilClient.isSuccess()) {
				 ...
				 * 二次开发插件支持 by zhaoyha at 2009.2.16
				 */
				getInvokeEventProxy().afterAction(
						nc.vo.scm.plugin.Action.UNAUDIT, proceVOs);
				...
				// operlog.insertBusinessExceptionlog(invVO, "审批", "审批",
				...
				if (mapAuditInfo.size() > 0) {
					for (int i = 0; i < proceVOs.length; i++) {
						...
						listAuditInfo = (ArrayList<Object>) mapAuditInfo
			...
			if ((e instanceof TaskInvalidateException)
				...
				MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes
					...
					&& (strErrMsg.indexOf(nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40040401", "UPP40040401-000163")/*
																		 * @res
																		 * "后续"
																		 */) >= 0 || strErrMsg
							.indexOf(nc.ui.ml.NCLangRes.getInstance()
									.getStrByID("40040401",
											"UPP40040401-000164")/*
																 * @res "不能删除"
																 */) >= 0)) {
				strErrMsg = nc.ui.ml.NCLangRes.getInstance().getStrByID(
						"40040401", "UPP40040401-000023")/*
														 * @res
	...
	public void onAudit() {
		...
		// 填审批人与审批日期
		String strOperPk = nc.ui.pub.ClientEnvironment.getInstance().getUser()
				.getPrimaryKey();
		...
		// 审批日期不能小于制单日期检查
		ClientLink cl = new ClientLink(nc.ui.pub.ClientEnvironment
				.getInstance());// 环境信息
		String errMsg = PuTool.getAuditLessThanMakeMsg(proceVOs,
				"dinvoicedate", "vinvoicecode", cl.getLogonDate(),
				ScmConst.PO_Invoice);
		if (errMsg != null) {
			MessageDialog.showErrorDlg(this, nc.ui.ml.NCLangRes.getInstance()
					.getStrByID("40040401", "UPPSCMCommon-000270")/*
		...
		try {
			 ...
			 * 二次开发插件支持 by zhaoyha at 2009.2.16
			 */
			getInvokeEventProxy().beforeAction(nc.vo.scm.plugin.Action.AUDIT,
					proceVOs);

			PfUtilClient
					.processBatchFlow(this, "APPROVE",
							nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
							ClientEnvironment.getInstance().getDate()
									.toString(), proceVOs, null);
			if (PfUtilClient.isSuccess()) {
				/**
				 * 二次开发插件支持 by zhaoyha at 2009.2.16
				 */
				getInvokeEventProxy().afterAction(
						nc.vo.scm.plugin.Action.AUDIT, proceVOs);
				...
				updateButtons();
				// add by zhang xiao wei 2013-08-18
				// InvoiceHeaderVO headVO = proceVOs.getHeadVO();
			...
			if ((e instanceof TaskInvalidateException)
				...
				MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes
					...
					&& strErrMsg.indexOf(nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40040401", "UPP40040401-000161")/*
																		 * @res
																		 * "应付系统已结帐"
																		 */) >= 0) {
				MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes
		...
		for (int i = 0; i < proceVOs.length; i++) {
			...
			try {
				for (int k = 0; k < invoicevos.length; k++) {
					...
					DJZBItemVO vo = null;
					String sql = "select * from arap_djfb where fb_oid='"
							+ invoicevos[k].getCupsourcebillrowid()
							+ "' and isnull(dr,0)=0 and dwbm='"
							+ ClientEnvironment.getInstance().getCorporation()
									.getPk_corp() + "'";

					vo = (DJZBItemVO) getQueryBS().executeQuery(sql,
							new BeanProcessor(DJZBItemVO.class));
	...
	/*
	 * add by zhang xiao wei 2012-08-24 功能:根据汇总返利计算生成应收单
	 */
	protected void insertYSData(DJZBVO[] proceVOs) {
		// boolean flag = true;

		try {

			DJZBVO[] djzbVos = createBill(proceVOs);
			iARAPVoucher iarap = NCLocator.getInstance().lookup(
					iARAPVoucher.class);
		...
		// if (flag) {
		// this.showWarningMessage("生成应收单成功");
		// } else {
		// this.showWarningMessage("生成应收单失败");
		// }
	}
	 ...
	 * @throws Exception
	 */
	private DJZBVO[] createBill(DJZBVO[] proceVOs) throws Exception {

		// SaleVO headVO = voInvoice.getHeadVO();
		// SaleinvoiceBVO[] bodyvo = voInvoice.getBodyVO();
		DJZBVO[] djzbVos = new DJZBVO[proceVOs.length];
		for (int i = 0; i < proceVOs.length; i++) {
			DJZBHeaderVO header = createBillHead(proceVOs[i]);
			DJZBItemVO[] items = createBillBody(proceVOs[i]);
			...
			if (items != null && items.length > 0) {
				for (DJZBItemVO item : items) {
					ybje = ybje.add(item.getDfybje());// 贷方原币金额
					bbje = bbje.add(item.getDfbbje());// 贷方本币金额
				}
			}
			// 本币金额
			header.setBbje(bbje);
	 ...
	 * 构建应收单表头信息
	 * 
	 * @param calResultHeadVO
	 * @param pk_currency
	 * @return
	 * @throws BusinessException
	 */
	public DJZBHeaderVO createBillHead(DJZBVO proceVOs)
			throws BusinessException {

		DJZBHeaderVO headVO = (DJZBHeaderVO) proceVOs.getParentVO();
		DJZBHeaderVO header = new DJZBHeaderVO();

		// header.setZyx7(headvo.getPk_rebatecal_h()); //存放返利计算PK

		// 必须，单据编号
		header.setDjbh(null);
		// 必须，单据日期(登录日期)
		header.setDjrq(new UFDate(ClientEnvironment.getInstance()
				.getBusinessDate().toString().substring(0, 10)));
		// 必须， 起效日期(登录日期)
		header.setEffectdate(new UFDate(ClientEnvironment.getInstance()
				.getBusinessDate().toString().substring(0, 10)));
		// 必须，往来对象,0 客户 1供应商 2部门 3业务员
		header.setWldx(1);
		// 客商(客商)
		// String sqlpk =
		// "select pk_cubasdoc from bd_cumandoc where pk_cumandoc ='"
		// + headVO.getHbbm() + "' and isnull(dr,0)=0";
		// String pk_cubasdoc = getPk(sqlpk);
		header.setHbbm(headVO.getHbbm());
		// 部门（）
		header.setDeptid(headVO.getDeptid());
		// 业务员（人员管理PK）
		header.setYwybm(ClientEnvironment.getInstance().getUser()
				.getPrimaryKey());
		// 单据大类
		header.setDjdl(headVO.getDjdl());
		// 销售返利类型（自定义项2）（折扣类型）
		// header.setZyx7(headVO.getCinvoiceid());// 存放返利的pk
		// 交易类型
		header.setDjlxbm(headVO.getDjlxbm());
		// 币种
		header.setBzbm(headVO.getBzbm());
		// 本币汇率
		header.setBbhl(new UFDouble(1));
		// 生效状态(默认未生效)
		header.setSxbz(new Integer(10));
		header.setFbje(UFDouble.ZERO_DBL);
		// 必须，单据状态
		header.setDjzt(new Integer(1));
		// 必须，单据会计年度
		// 必须，单据会计年度
		header.setDjkjnd(new UFDate(ClientEnvironment.getInstance()
				.getBusinessDate().toString().substring(0, 10)).getYear()
				+ "");
		String date = String.valueOf(new UFDate(ClientEnvironment.getInstance()
				.getBusinessDate().toString().substring(0, 10)));
		String sqlkq = "SELECT month FROM bd_accperiodmonth  where isnull(dr,0)=0 and '"
				+ date + "' between begindate and enddate ";
		ArrayList lt = null;
		String value = "";
		lt = (ArrayList) getQueryBS().executeQuery(sqlkq,
				new ArrayListProcessor());

		if (lt.size() > 0) {
			Object[] kk = (Object[]) lt.get(0);
			value = kk[0] != null ? kk[0].toString() : "0";
		}
		// 必须，单据会计期间
		header.setDjkjqj(value);

		header.setXslxbm(headVO.getXslxbm());//

		// 非空,单位编码(公司PK)
		header.setDwbm(ClientEnvironment.getInstance().getCorporation()
				.getPk_corp());
		// 结算中心支付
		// header.setJszxzf(new Integer(0));
		header.setLybz(headVO.getLybz());
		// header.setFj(1);
		header.setIspaid(headVO.getIspaid());
		// 预收付款标志
		header.setPrepay(UFBoolean.FALSE);
		// 系统标志
		header.setPzglh(1);
		// 期初标志
		header.setQcbz(UFBoolean.FALSE);
		// header.setIsselectedpay(1);

		// //返利单据类型
		// String sqlbm="select djlxoid from arap_djlx where djlxbm='F0-FL' ";
		// header.setZyx21(getPk(sqlbm));
		// 业务编码
		header.setYwbm(getYWBM("D1", ClientEnvironment.getInstance()
				.getCorporation().getPk_corp()));
		// 设置暂估应付标识。
		header.setZgyf(2);
		// 录入人
		header
				.setLrr(ClientEnvironment.getInstance().getUser()
						.getPrimaryKey());
		// 备注
		header.setScomment("回冲单据'" + headVO.getDjbh() + "'");
		return header;
	}

	public String getYWBM(String djlxbm, String dwbm) throws BusinessException {
		if (djlxbm == null || djlxbm.equals(""))
			return null;
		IUAPQueryBS bs = NCLocator.getInstance().lookup(IUAPQueryBS.class);
		String sqlywbm = "select djlxoid from arap_djlx where djlxbm ='"
				+ djlxbm + "' and dwbm ='" + dwbm + "' and isnull(dr,0)=0";
		Object yw = bs.executeQuery(sqlywbm, new ColumnProcessor());
		if (yw != null && yw.toString().trim().length() > 0) {
			return yw.toString();
		} else {
			throw new BusinessException("单据类型" + djlxbm + "在当前公司" + dwbm
					+ "没有定义");
		}
	}

	public String getPk(String sql) {
		IUAPQueryBS bs = NCLocator.getInstance().lookup(IUAPQueryBS.class);
		Object value = "";
		try {
	 ...
	 * 构建应收单表体信息
	 * 
	 * @param calResultItems
	 * @param header
	 * @return
	 * @throws Exception
	 */
	private DJZBItemVO[] createBillBody(DJZBVO proceVOs) throws Exception {

		// IUAPQueryBS iUAPQueryBS =
		// (IUAPQueryBS)NCLocator.getInstance().lookup(IUAPQueryBS.class.getName());
		DJZBItemVO[] bodyvo = (DJZBItemVO[]) proceVOs.getChildren();
		DJZBHeaderVO header = (DJZBHeaderVO) proceVOs.getParentVO();
		DJZBItemVO[] items = new DJZBItemVO[bodyvo.length];
		String dwbm = ClientEnvironment.getInstance().getCorporation()
				.getPk_corp();
		// SaleVO headVO = voInvoice.getHeadVO();;
		// SaleinvoiceBVO[] bodyvo = voInvoice.getBodyVO();
		// UFDouble money = new UFDouble(0);
		// for (int j = 0; j < bodyvo.length; j++) {
		// money = money.add(bodyvo[j].getNsummny());
		// }
		...
		if (bodyvo != null && bodyvo.length > 0) {
			for (int i = 0; i < bodyvo.length; i++) {
				...
				// 报价单位数量
				item.setBjdwsl(UFDouble.ZERO_DBL);
				// 报价单位无税单价
				item.setBjdwwsdj(UFDouble.ZERO_DBL);
				// 必填，币种编码
				item.setBzbm(bodyvo[i].getBzbm());
				// 必填，方向,1借方,-1贷方
				item.setFx(-1);
				item.setDj(bodyvo[i].getDj());
				// item.setCkdid(bodyvo[i].getCkdid());
				// 必填，借方辅币金额，为0
				item.setJffbje(UFDouble.ZERO_DBL);
				// 必填，借方辅币税金,暂为0
				item.setJffbsj(UFDouble.ZERO_DBL);
				// 必填，借方本币税金,为0
				item.setJfbbsj(UFDouble.ZERO_DBL);
				// 必填，借方辅币无税金额
				item.setWbfbbje(UFDouble.ZERO_DBL);

				item.setShlye(UFDouble.ZERO_DBL);
				item.setDjxtflag(0);
				// 必填，贷方原币金额
				item
						.setDfybje(bodyvo[i].getDfybje().multiply(
								new UFDouble(-1)));
				// 必填，贷方辅币金额
				item.setDffbje(UFDouble.ZERO_DBL);
				// 必填，贷方本币金额 = 贷方原币金额＊汇率
				item
						.setDfbbje(bodyvo[i].getDfbbje().multiply(
								new UFDouble(-1)));
				// 贷方数量
				item.setDfshl(bodyvo[i].getDfshl());
				// 必填，贷方原币税金,0
				item.setDfybsj(UFDouble.ZERO_DBL);
				// 必填，贷方辅币税金,0
				item.setDffbsj(UFDouble.ZERO_DBL);
				// 必填，贷方本币税金,0
				item.setDfbbsj(UFDouble.ZERO_DBL);
				// 必填，贷方原币无税金额
				item.setDfybwsje(bodyvo[i].getDfybwsje().multiply(
						new UFDouble(-1)));
				// 必填，贷方辅币无税金额,0
				item.setWbffbje(UFDouble.ZERO_DBL);
				// 必填，贷方本币无税金额
				item.setDfbbwsje(bodyvo[i].getDfbbwsje().multiply(
						new UFDouble(-1)));
				item.setXgbh(-1);
				item.setWldx(1);
				// 往来对象,0 客户 1供应商 2部门 3业务员
				item.setWldx(1);
				// 必填，辅币余额,0
				item.setFbye(UFDouble.ZERO_DBL);
				item.setDdlx(bodyvo[i].getVouchid());
				item.setDdhh(bodyvo[i].getFb_oid());
				item.setDjlxbm("D1");
				// 设置新增小池
				// item.setZyx19(vos.get(i).getPk_capitalpooltype());

				// 设置发票主键
				// 合作伙伴,其实是客商的基本档案的主键
				item.setHbbm(bodyvo[i].getHbbm());
				item.setCinventoryid(bodyvo[i].getCinventoryid());
				// 必填，借方原币金额,如果单据类型是付款、付款结算单、应收单那对应是中间表金额
				item.setJfybje(UFDouble.ZERO_DBL);

				item.setXyzh(bodyvo[i].getVouchid());
				// 必填，借方原币无税金额＝借方原币金额
				item.setJfybwsje(UFDouble.ZERO_DBL);
				item.setPh(bodyvo[i].getPh());
				item.setShlye(bodyvo[i].getShlye());
				// 必填，原币余额,中间表金额
				item.setYbye(bodyvo[i].getYbye().multiply(new UFDouble(-1)));
				// 必填，本币余额,金额＊汇率
				item.setBbye(bodyvo[i].getBbye().multiply(new UFDouble(-1)));
				// 税率，默认0
				item.setSl(UFDouble.ZERO_DBL);
				// 必填，借方原币税金，默认0
				item.setJfybsj(UFDouble.ZERO_DBL);
				item.setJsfsbm(bodyvo[i].getDjlxbm());
				// 部门
				item.setDeptid(bodyvo[i].getDeptid());
				// 业务员
				item.setYwybm(bodyvo[i].getYwybm());
				// 本币汇率,默认1
				item.setBbhl(UFDouble.ONE_DBL);
				// //返利存货大类名称（自定义项19）
				// InvclVO
				// invclVO=iResult.queryInvByCode(calResultItem.getPk_corp(),
				// calResultItem.getPk_invbasdoc());
				// if(invclVO!=null)
				// item.setZyx19(invclVO.getPrimaryKey());
				// 存货编码
				// item.setCinventoryid(calResultItem.getPk_invbasdoc());
				// 摘要
				item.setZy("回冲发票");
				// item.setFph(bodyvo[i].getFph());
				// 含税单价
				item.setHsdj(bodyvo[i].getHsdj());
				// 单价
				// item.setDj(calResultItem.getCalculateprice());
				// 借方数量
				// item.setJfshl(calResultItem.getSaleordernumber());
				// // 必填，数量余额
				// item.setShlye(new UFDouble(1));
				// 主计量单位（如果没有对应的主计量则不继续生成）
				// String pk_measdoc =queryInvMeasdoc(iUAPQueryBS,
				// calResultItem.getPk_invbasdoc());
				// if(pk_measdoc==null || pk_measdoc.length()<=0)
				// {
				// String
				// invname=execFormular("getColValue(bd_invbasdoc,invname,pk_invbasdoc,pk_invbasdoc)",
				// calResultItem.getPk_invbasdoc());
				// throw new
				// Exception("生成应收单失败:存货【"+invname+"】没有设置主计量单位,无法继续导入!");
				// }
				item.setZjldw(bodyvo[i].getZjldw());
				// 产品线(如果没有产品线，继续生成)
				// String pk_prodline =queryProduceLine(iUAPQueryBS,
				// calResultItem.getPk_invbasdoc());
				// item.setProductline(pk_prodline);

				// 必填，借方本币金额,借方原币金额＊汇率,不能为0必须填值
	...
	private void onBillAudit() {
		...
		for (int i = 0; i < proceVOs.length; i++) {
			if (proceVOs[i] != null
					&& proceVOs[i].getHeadVO().getPrimaryKey() != null) {
				for (int j = 0; j < proceVOs[i].getChildrenVO().length; j++) {
					try {
						if (PuPubVO.getString_TrimZeroLenAsNull(proceVOs[i]
								.getChildrenVO()[j].getPrimaryKey()) == null) {
							proceVOs[i] = InvoiceHelper
		...
		try {
			...
			if (PfUtilClient.isSuccess()) {
				...
				// Operlog operlog = new Operlog();
				// operlog.insertBusinessExceptionlog(resultVO, "审批", "审批",
				// nc.vo.scm.funcs.Businesslog.MSGMESSAGE,
				// nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
				// nc.ui.sm.cmenu.Desktop.getApplet().getParameter("USER_IP"));
				...
				if (iBillStatus.compareTo(nc.vo.scm.pu.BillStatus.FREE) == 0
					...
					showHintMessage(nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40040401", "UPP40040401-000234")/*
																		 * @res
																		 * "审批未成功"
																		 */);
	...
	private void onListAudit() {
		...
		for (int i = 0; i < getInvVOs().length; i++) {
			if (m_listPanel.getHeadBillModel().getRowState(i) == BillModel.SELECTED) {
				...
				validHidKeyVEC.add(getInvVOs()[nCurIndex].getHeadVO()
						.getPrimaryKey());
			}
		}
		...
		if (validVOsVEC.size() == 0) {
			MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes.getInstance()
					.getStrByID("SCMCOMMON", "UPPSCMCommon-000270")/*
																	 * @res "提示"
																	 */,
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40040402",
							"UPP40040402-000003")/*
												 * @res "请先选择要审批的发票！"
												 */);
		...
		try {
			...
			if (strErrMsg != null
					&& strErrMsg.indexOf(nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40040402", "UPP40040402-000019")/*
																		 * @res
																		 * "应付系统已结帐"
																		 */) >= 0) {
				MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes
		...
		if (PfUtilClient.isSuccess()) {
			...
			// operlog.insertBusinessExceptionlog(vo, "审批", "审批",
			// nc.vo.scm.funcs.Businesslog.MSGMESSAGE,
			// nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
			// nc.ui.sm.cmenu.Desktop.getApplet().getParameter("USER_IP"));
			// }
			...
			if (definedVOs == null) {
				...
				for (int i = 0; i < validIndexVEC.size(); i++) {
					try {
						resultH = InvoiceHelper
								.findByPrimaryKeyBantch(validHidKeyVEC);
					} catch (Exception e) {
						// TODO 自动生成 catch 块
						SCMEnv.out(e);
					}
					if (resultH != null
							&& resultH.get(getInvVOs()[((Integer) validIndexVEC
									.get(i)).intValue()].getHeadVO()
									.getPrimaryKey()) != null) {
						getInvVOs()[((Integer) validIndexVEC.get(i)).intValue()] = resultH
								.get(getInvVOs()[((Integer) validIndexVEC
										.get(i)).intValue()].getHeadVO()
										.getPrimaryKey());
					}
				}
	...
	/**
	 * 作者：王印芬 功能：弃审列表的一批选中的单据 不能弃审的条件：自由状态 参数：无 返回：无 例外：无 日期：(2002-3-13
	 * 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-04-15 wyf
	 * 配合后端只进行TS判断是否单据改变，本代码加入一些如是否审批的判断 2002-05-23 wyf 弃审后的发票从界面去掉
	 */
	private void onListUnAudit() {
		...
		try {
			...
			if (strErrMsg != null
					&& (strErrMsg.indexOf(nc.ui.ml.NCLangRes.getInstance()
							.getStrByID("40040402", "UPP40040402-000020")/*
																		 * @res
																		 * "后续"
																		 */) >= 0 || strErrMsg
							.indexOf(nc.ui.ml.NCLangRes.getInstance()
	...
	private void onBillCancel() {
		...
		if (result == MessageDialog.ID_YES) {
			...
			try {
				if (getInvVOs() != null) {
					getInvVOs()[getCurVOPos()] = InvoiceHelper
							.findByPrimaryKey(getInvVOs()[getCurVOPos()]
									.getHeadVO().getPrimaryKey());
			...
			// 从其他单据转入则转入列表
			if (nVOStatus != InvoiceVO.FROM_HAND
					&& nVOStatus != InvoiceVO.FROM_QUERY) {
				// 设置列表界面数据
				setCurOperState(getListOperState());
						...
						if (id != null) {
							...
							if (o != null) {
								...
								if (iBillStatus.intValue() == 4) {
									curVO.getHeadVO().setIbillstatus(
											new Integer(4));
									hBillStatusBeforeEdit.put(id,
											new Integer(0));
								}
							}
						}
	...
	private void onBillCombin() {
		CollectSettingDlg dlg = new CollectSettingDlg(this, NCLangRes
				.getInstance().getStrByID("common", "4004COMMON000000089")/*
																		 * @res
																		 * "合并显示"
																		 */,
				ScmConst.PO_Invoice, "40040401", getCorpId(), ClientEnvironment
						.getInstance().getUser().getPrimaryKey(),
				InvoiceVO.class.getName(), InvoiceHeaderVO.class.getName(),
				InvoiceItemVO.class.getName());
		//
		dlg.initData(getBillCardPanel(),
				new String[] { "invcode", "invname", "invspec", "invtype",
						"cproducearea", "noriginaltaxmny", "noriginalsummny" }, // 固定分组列
				null, // new String[]{"dplanarrvdate"},缺省分组列
				new String[] { "ninvoicenum", "noriginalcurprice",
						"noriginalcurmny", "noriginaltaxpricemny" },// 求和列
				null, new String[] { "noriginalcurprice", "norgnettaxprice" },
				"ninvoicenum");
		dlg.showModal();
		// 更新当前缓存VO
		if (getBillCardPanel() != null) {
			if (getInvVOs()[getCurVOPos()].getPrimaryKey().equals(
					getCurVOonCard().getPrimaryKey()))
	...
	private void onBillQuery() {
		...
		// v51
		m_InvQueDlg.setRefsDataPowerConVOs(getCurOperator(),
	...
	private void onBillUnAudit() {
		...
		try {
			...
			if (PfUtilClient.isSuccess()) {
				...
				// resultVO.getOperatelogVO().setOperatorId(nc.ui.pub.ClientEnvironment.getInstance().getUser().getPrimaryKey());
				// Operlog operlog = new Operlog();
				// operlog.insertBusinessExceptionlog(resultVO, "弃审", "弃审",
				// nc.vo.scm.funcs.Businesslog.MSGMESSAGE,
				// nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
				// nc.ui.sm.cmenu.Desktop.getApplet().getParameter("USER_IP"));
				// getInvVOs()[getCurVOPos()] = resultVO;
	...
	private InvoiceVO[] getDistributedICVMISumVOs(
		...
		SysInitVO para = null;
		try {
			para = nc.ui.pub.para.SysInitBO_Client.queryByParaCode(getCorpId(),
					"PO08");
		} catch (BusinessException e) {
			SCMEnv.error(e);
		}
		...
		for (int i = 0; i < SourceVOsLength; i++) {
			...
			String isplitmode = ((headVO.getAttributeValue("isplitmode") == null || headVO
					.getAttributeValue("isplitmode").toString().trim().equals(
							"")) ? curKey : headVO.getAttributeValue(
		...
		if (ordTable.size() > 0) {
			...
			while (elems.hasMoreElements()) {
				...
				invoiceHeadVO.setAttributeValue("pk_corp", ((headVOs[0]
						...
						.getAttributeValue("pk_corp").toString().trim().equals(
								"")) ? "" : headVOs[0].getAttributeValue(
				...
				for (int j = 0; j < headVOsLength; j++) {
					...
					daRate = m_cardPoPubSetUI2.getBothExchRateValue(
				...
				// 更新行号设置
				BillRowNo.setVORowNoByRule(allVOs[i], ScmConst.PO_Invoice,
						"crowno");
				// 取计划价
		...
		if (isTransPUAssUnit) {
			for (int i = 0; i < allVOs.length; i++) {
				for (int j = 0; j < allVOs[i].getBodyLen(); j++) {
					if (PuTool.isAssUnitManaged(allVOs[i].getBodyVO()[j]
							.getCbaseid())) {
						String cassistunit = PuTool
								.getDefaultPUAssUnit(allVOs[i].getBodyVO()[j]
										.getCbaseid());
						if (PuPubVO.getString_TrimZeroLenAsNull(cassistunit) != null
								&& PuPubVO.getUFDouble_ZeroAsNull(allVOs[i]
										.getBodyVO()[j].getNinvoicenum()) != null) {
							allVOs[i].getBodyVO()[j]
									.setCassistunit(cassistunit);
							UFDouble ufdConv = PuTool.getInvConvRateValue(
									allVOs[i].getBodyVO()[j].getCbaseid(),
									cassistunit);
							allVOs[i].getBodyVO()[j].setNexchangerate(ufdConv);
							allVOs[i].getBodyVO()[j].setNassistnum(allVOs[i]
									.getBodyVO()[j].getNinvoicenum().div(
									ufdConv));
						}
					}
				}
	...
	private void setDefaultBankAccountForAVendor(String strVendorBase,
		...
		// getBillCardPanel().getHeadItem("caccountbankid").getComponent();
		// ((AccountsForVendorRefModel)
		// pane.getRef().getRefModel()).setCvendorbaseid(strVendorBase);
		// added by fangy 2002-10-22 18:34 begin
	...
	private void onButtonClickedBill(nc.ui.pub.ButtonObject btn) {
		...
		// ================业务类型
			...
			// 业务类型导出来源单据类型
			PfUtilClient.retAddBtn(m_btnInvBillNew, nc.ui.pub.ClientEnvironment
					.getInstance().getCorporation().getPrimaryKey(),
					nc.vo.scm.pu.BillTypeConst.PO_INVOICE, btn);
			//
			...
			m_bizButton = btn;
			updateButtons();
		} else if (btn.getParent() == m_btnInvBillNew
				|| btn.getParent() == btnBillAddContinue) {
			showHintMessage(getHeadHintText()
					+ nc.ui.ml.NCLangRes.getInstance().getStrByID("common",
							"UCH028")/*
									 * @res "增加发票"
			...
			if (btn.getTag() != null && btn.getTag().indexOf("makeflag") >= 0) {
				...
				// 当前业务类型
				if (m_strBtnTag != null)
					setCurBizeType(m_strBtnTag);
				onAdd();

				m_isMakedBill = true;

				// 采购发票自制时，采购公司默认为当前登录公司
				getBillCardPanel().getHeadItem("pk_purcorp").setValue(
						getPk_corp());
			} else {
				// 获取来源单据类型
				...
				if ("50".equalsIgnoreCase(strUpBillType)) {
					PfUtilClient.childButtonClicked(btn, getCorpPrimaryKey(),
							"400404", nc.ui.pub.ClientEnvironment.getInstance()
									.getUser().getPrimaryKey(),
							ScmConst.PO_Invoice, this);
				}

				// if(m_bizButton!=null&&m_bizButton.getName()!=null&&m_bizButton.getName().equals("")){
				else {
					PfUtilClient.childButtonClicked(btn,
							nc.ui.pub.ClientEnvironment.getInstance()
									.getCorporation().getPk_corp(),
							getModuleCode(), nc.ui.pub.ClientEnvironment
									.getInstance().getUser().getPrimaryKey(),
					...
					if ("50".equalsIgnoreCase(strUpBillType)) {
						...
						for (int i = 0; i < vos.length; i++) {
							if (PuPubVO.getString_TrimZeroLenAsNull(vos[i]
									.getHeadVO().getPk_purcorp()) == null) {// 采购公司为空则默认为当前登录公司
...
//						}
						...
						if (vos == null) {
							...
							for (int i = 0; i < vos.length; i++) {
								...
								if (PuPubVO.getString_TrimZeroLenAsNull(vos[i]
										.getHeadVO().getPk_purcorp()) == null) {// 采购公司为空则默认为当前登录公司
									vos[i].getHeadVO().setPk_purcorp(
											getPk_corp());
								}
								// 更新币种和行号设置
								BillRowNo.setVORowNoByRule(vos[i],
										ScmConst.PO_Invoice, "crowno");
							...
							if (BillTypeConst.PO_ORDER
									.equalsIgnoreCase(strUpBillType)
									|| BillTypeConst.STORE_PO
											.equalsIgnoreCase(strUpBillType)) {
					...
					if (btn.getParent() == m_btnInvBillNew) {
						...
						if (arySourceVOs != null && arySourceVOs.length > 0) {
							...
							InvoiceHeaderVO voCur = getCurVOonCard()
									.getHeadVO();// 当前卡片上的表头数据
							InvoiceHeaderVO newvo = null;
							try {
								newvo = (InvoiceHeaderVO) ObjectUtils
										.serializableClone(voCur);
							} catch (Exception e) {
								SCMEnv.out(e);
							}

							BillItem[] headItems = getBillCardPanel()
									.getHeadShowItems();// 必输项目的itemkey

							String[] headNotNullKeys = null;// 非空单据头ItemKey
							Vector<String> v1 = new Vector<String>();
							for (int i = 0; headItems != null
									&& i < headItems.length; i++) {
							...
							for (int i = 0; i < arySourceVOs.length; i++) {
								...
								for (int j = 0; j < headNotNullKeys.length; j++) {
									if (arySourceVOs[i].getParentVO()
											.getAttributeValue(
													headNotNullKeys[j]) != null
											&& !(arySourceVOs[i].getParentVO()
													.getAttributeValue(
															headNotNullKeys[j]) instanceof UFDate)
											&& !arySourceVOs[i]
													.getParentVO()
													.getAttributeValue(
															headNotNullKeys[j])
...
//										}
						...
						getInvVOs()[getCurVOPos()]
								.setChildrenVO((InvoiceItemVO[]) continueItemVO
										.toArray(new InvoiceItemVO[continueItemVO
												.size()]));
						// 更新币种和行号设置
						BillRowNo.setVORowNoByRule(getInvVOs()[getCurVOPos()],
								ScmConst.PO_Invoice, "crowno");
					}
					// 显示发票
		...
		// 预览
			...
			// nc.ui.rc.pub.PurchasePrintDS(getModuleCode(),
			// getInvBillPanel()));
			showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"common", "UCH041"));
		}
		...
		// 送审
			...
			if (getInvVOs() != null && getInvVOs()[getCurVOPos()] != null) {
				...
				boolean isNeedSendToAuditQ = BusiBillManageTool
						.isNeedSendToAudit(ScmConst.PO_Invoice, getPk_corp(),
	...
	private UFDouble[] queryPlanPrices(String cMangID[], String pk_calbody) {
		...
		if (pk_calbody != null && pk_calbody.trim().length() > 0) {
			...
			Object[][] retObj = null;
			try {
				// retObj = PubHelper.queryResultsFromTableByWhere("bd_produce",
				// new String[] {
				// "jhj","pk_invmandoc"
				// }, alWhere.toArray(new String[alWhere.size()]));
				retObj = PubHelper.queryResultsFromAnyTable("bd_produce",
						new String[] { "pk_invmandoc", "jhj" }, "PK_CALBODY='"
								+ pk_calbody + "' and PK_INVMANDOC in "
								+ whereInPart);
			} catch (Exception be) {
				SCMEnv.out(be);
			...
			if (retObj != null && retObj.length > 0) {
				for (int i = 0; i < retObj.length; ++i) {
					...
					if (oneRow == null || oneRow[0] == null
							|| oneRow[0].toString().length() == 0)
						continue;
					planPrices.put(oneRow[0].toString(), PuPubVO
							.getUFDouble_NullAsZero(oneRow[1]));
				}
			}
	...
	public boolean onClosing() {
		if (getListOperState() == STATE_LIST_FROM_BILLS) {
			int ret = MessageDialog.showYesNoCancelDlg(this, nc.ui.ml.NCLangRes
					.getInstance()
					.getStrByID("40040401", "UPPSCMCommon-000270"),
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40040401",
							"UPP40040401-000040")/*
												 * @res "是否放弃转入未转入完毕的单据，退出?"
												 */);
	...
	private void onCopy() {
		...
		dealWhenCopy();
		// 检查是否有封存的存货
		BillCardPanelHelper.clearSealedInventories(getBillCardPanel(),
				"cmangid", new String[] { "cmangid", "cbaseid",
						"cinventorycode", "cinventoryname", "cspecification",
						"ctype" });
	...
	private void onDiscard() {
		...
		((InvoiceHeaderVO) invVO.getParentVO())
				.setCoperatoridnow(nc.ui.pub.ClientEnvironment.getInstance()
						.getUser().getPrimaryKey());
		...
		while (!invVO.getUserConfirmFlag().booleanValue()) {
			try {
				...
				// 佛山碧桂园:记录业务日志
				invVO.getOperatelogVO().setOpratorname(
						nc.ui.pub.ClientEnvironment.getInstance().getUser()
								.getUserName());
				invVO.getOperatelogVO().setCompanyname(
						nc.ui.pub.ClientEnvironment.getInstance()
								.getCorporation().getUnitname());
				invVO.getOperatelogVO().setOperatorId(
						nc.ui.pub.ClientEnvironment.getInstance().getUser()
								.getPrimaryKey());
				Operlog operlog = new Operlog();
				operlog.insertBusinessExceptionlog(invVO, "删除", "删除",
						nc.vo.scm.funcs.Businesslog.MSGMESSAGE,
						nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
				 ...
				 * 二次开发插件支持 by zhaoyha at 2009.2.16
				 */
				getInvokeEventProxy().beforeAction(
						nc.vo.scm.plugin.Action.DELETE,
						new AggregatedValueObject[] { invVO });

				PfUtilClient.processBatch("DISCARD",
						nc.vo.scm.pu.BillTypeConst.PO_INVOICE, getToday()
		...
		if (PfUtilClient.isSuccess()) {
			...
			// "'"+willSavedVO.getBodyVO()[0].getCupsourcebillid()+"'");
			updatearap.updateArap(invVO.getBodyVO()[0].getCupsourcebillid(),
					action);
			// end
		}
		showHintMessage("");
		...
		for (int i = 0; i < iLen; i++) {
			if (billVO.getBodyVO()[i].getCupsourcebilltype().equals("45")
					&& billVO.getBodyVO()[i].getCcurrencytypeid().equals(
							lStr_NativeCurrencyID)) {// 根据入库单生成的，且币种为本位币的采购发票可以进行“优质优价取价”

				PricParaVO tempVO = new PricParaVO();
				tempVO.setCgeneralbid(billVO.getBodyVO()[i]
		...
		if (VOs != null && VOs.length > 0) {
			for (int i = 0; i < VOs.length; i++) {
				...
				if (VOs[i].getNtaxprice() != null) {
					...
					if (getUFDouble(VOs[i].getNtaxprice()) != null
							&& getUFDouble(VOs[i].getNtaxprice()).doubleValue() < 0.0) {
						error.append(billVO.getBodyVO()[lint_row].getCrowno()
								+ ",");
					} else {
						getBillCardPanel().setBodyValueAt(
								VOs[i].getNtaxprice(), lint_row,
								"norgnettaxprice");
						BillEditEvent event = new BillEditEvent(
								getBillCardPanel().getBillTable(), VOs[i]
										.getNtaxprice(), "norgnettaxprice",
								lint_row);// 原币净含税单价
	...
	private void onInsertLine() {
		...
		BillRowNo.insertLineRowNo(getBillCardPanel(), ScmConst.PO_Invoice,
				"crowno");
	}
	...
	private void onListDiscard() {
			...
			for (int j = 0; j < iLen; j++) {
				if (voaItem[j].getNaccumsettmny() != null
						&& voaItem[j].getNaccumsettmny().compareTo(
								nc.vo.scm.pu.VariableConst.ZERO) != 0) {
					iSettle = i + 1;
					break;
				}
			}
		...
		int ret = MessageDialog.showYesNoDlg(this, nc.ui.ml.NCLangRes
				.getInstance().getStrByID("common", "UCH002")/*
															 * @res "询问"
															 */,
				nc.ui.ml.NCLangRes.getInstance().getStrByID("40040401",
		...
		for (int i = 0; i < proceVOs.length; i++) {
			...
			((InvoiceHeaderVO) proceVOs[i].getParentVO())
					.setCoperatoridnow(nc.ui.pub.ClientEnvironment
							.getInstance().getUser().getPrimaryKey());

			// 佛山碧桂园:记录业务日志
			proceVOs[i].getOperatelogVO().setOpratorname(
					nc.ui.pub.ClientEnvironment.getInstance().getUser()
							.getUserName());
			proceVOs[i].getOperatelogVO().setCompanyname(
					nc.ui.pub.ClientEnvironment.getInstance().getCorporation()
							.getUnitname());
			proceVOs[i].getOperatelogVO().setOperatorId(
					nc.ui.pub.ClientEnvironment.getInstance().getUser()
							.getPrimaryKey());
			Operlog operlog = new Operlog();
			operlog.insertBusinessExceptionlog(proceVOs[i], "删除", "删除",
					nc.vo.scm.funcs.Businesslog.MSGMESSAGE,
					nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
					nc.ui.sm.cmenu.Desktop.getApplet().getParameter("USER_IP"));
		...
		for (int i = 0; i < len; i++) {
			...
			while (!vos[i].getUserConfirmFlag().booleanValue()) {
				try {
					...
					PfUtilClient.processBatch("DISCARD",
							BillTypeConst.PO_INVOICE, ClientEnvironment
									.getInstance().getDate().toString(),
							new InvoiceVO[] { proceVOs[i] }, paraLists);
					break;
				} catch (RwtPiToPoException ex) {
					SCMEnv.out(ex);
					// if(MessageDialog.showYesNoDlg(this,
	...
	private void onListModify() {
		...
		if (getCurOperState() != STATE_LIST_FROM_BILLS) {
			...
			Integer iStatus = getInvVOs()[nCurIndex].getHeadVO()
					.getIbillstatus();
			if (iStatus != null
					&& iStatus.compareTo(nc.vo.scm.pu.BillStatus.FREE) != 0
			...
			for (int i = 0; i < getInvVOs()[nCurIndex].getBodyVO().length; i++) {
				if ((getInvVOs()[nCurIndex].getBodyVO()[i].getNaccumsettmny() != null && getInvVOs()[nCurIndex]
						.getBodyVO()[i].getNaccumsettmny().doubleValue() != 0.0)) {
					MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes
							.getInstance().getStrByID("40040401",
									"UPPSCMCommon-000270")/* @res "提示" */,
							nc.ui.ml.NCLangRes.getInstance().getStrByID(
									"40040401", "UPP40040401-000056")/*
																	 * @res
																	 * "该发票已结算，不能修改！"
																	 */);
					return;
		...
		if (getBillListPanel().getHeadBillModel().getValueAt(VOPos,
				"cinvoiceid") == null
				&& splitFlag) {
			// if(nCurIndex != 0){
			...
			if (getBillCardPanel().getBodyValueAt(i, "idiscounttaxtype") == null) {
				getBillCardPanel().setBodyValueAt(
						((UIComboBox) getBillCardPanel().getHeadItem(
								...
								.getSelectedItem(), i, "idiscounttaxtype");
			}
			// 带出存货的默认税率
	...
	private void onLnkQuery() {
		...
		if (m_nCurOperState == STATE_EDIT)
			vo = (InvoiceVO) getBillCardPanel().getBillValueVO(
					InvoiceVO.class.getName(), InvoiceHeaderVO.class.getName(),
					InvoiceItemVO.class.getName());
		else
		...
		nc.ui.scm.sourcebill.SourceBillFlowDlg soureDlg = new nc.ui.scm.sourcebill.SourceBillFlowDlg(
				this, ScmConst.PO_Invoice, ((InvoiceHeaderVO) vo.getParentVO())
						.getPrimaryKey(), null, nc.ui.pub.ClientEnvironment
						.getInstance().getUser().getPrimaryKey(),
				((InvoiceHeaderVO) vo.getParentVO()).getVinvoicecode());
		soureDlg.showModal();
	}
	...
	private void onCardEdit() {
		...
		for (int i = 0; i < rows; ++i) {
			...
			if (null != vf && !StringUtil.isEmptyWithTrim(vf.toString())) {
				beforeEdit(new BillEditEvent(this, null, null, "vfree0", i,
						BillItem.BODY));
			}
		}
	...
	private void onPasteLine() {
		...
		if (nPastRowCount > 0) {
			...
			for (int i = iSelectRow - nPastRowCount; i < iSelectRow; i++) {
				...
				// 设置新插入的表体行币种默认为表头币种
				getBillCardPanel()
						.setBodyValueAt(strCurr, i, "ccurrencytypeid");
				// 设置汇率
				setExchangeRateBody(i, true, null);
			}
			BillRowNo.pasteLineRowNo(getBillCardPanel(), ScmConst.PO_Invoice,
					"crowno", nPastRowCount);
		}
	}
	...
	private void onPasteLineToTail() {
		...
		if (iOldRowCnt > 0 && iNewRowCnt > 0 && iOldRowCnt == iNewRowCnt)
			...
			if (nPastRowCount > 0) {
				// int iSelectRow =
				// getInvBillPanel().getBillTable().getSelectedRow();
				// 取表头币种
				String strCurr = getBillCardPanel().getHeadItem(
				...
				for (int i = iOldRowCnt; i < iNewRowCnt; i++) {
					...
					getBillCardPanel().setBodyValueAt(null, i, "ts");
					// 设置新插入的表体行币种默认为表头币种
					getBillCardPanel().setBodyValueAt(strCurr, i,
							"ccurrencytypeid");
					// 设置汇率
					setExchangeRateBody(i, true, null);
				}
				...
				BillRowNo.addLineRowNos(getBillCardPanel(),
						ScmConst.PO_Invoice, "crowno", nPastRowCount);
			}
			showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"40040401", "UPP40040401-000058")/*
													 * @res (iNewRowCnt -
													 * iOldRowCnt) + "行发票被粘贴"
													 */);
		}
	...
	public void onQueryForAudit() {
		...
		if (getInvVOs() != null && getInvVOs().length > 0) {
			...
			nc.ui.pub.workflownote.FlowStateDlg approvestatedlg = new nc.ui.pub.workflownote.FlowStateDlg(
					this, "25", getInvVOs()[getCurVOPos()].getHeadVO()
							.getCbiztype(), getInvVOs()[getCurVOPos()]
							.getHeadVO().getPrimaryKey());
	...
	private boolean onSave() {
		...
		try {
			...
			if (paraList.size() != 3) {
				...
				forSettleList.add(nc.ui.pub.ClientEnvironment.getInstance()
						.getDate());
				forSettleList.add(new UFBoolean(nc.ui.pub.ClientEnvironment
						.getInstance().isGroup()));
			...
			if (willSavedVO != null
					...
					&& ((InvoiceHeaderVO) willSavedVO.getParentVO())
							.getIbillstatus().equals(BillStatus.FREE)) {
				((InvoiceHeaderVO) willSavedVO.getParentVO())
						.setCoperator(nc.ui.pub.ClientEnvironment.getInstance()
								.getUser().getPrimaryKey());
			}
			// 为满足审批流，传入当前操作员
			((InvoiceHeaderVO) willSavedVO.getParentVO())
					.setCuserid(nc.ui.pub.ClientEnvironment.getInstance()
							.getUser().getPrimaryKey());
			// 为判断是否可修改、作废其他人单据
			((InvoiceHeaderVO) willSavedVO.getParentVO())
					.setCoperatoridnow(nc.ui.pub.ClientEnvironment
			...
			timer.addExecutePhase(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"40040401", "UPP40040401-000103")/*
													 * @res "保存发票前的准备"
													 */);

			// 设置表头税率
			String ntaxrate = getBillCardPanel().getHeadItem("ntaxrate")
					.getValue();
			/**
			 * 二次开发插件支持 by zhaoyha at 2009.2.16
			 */
			getInvokeEventProxy().beforeAction(nc.vo.scm.plugin.Action.SAVE,
					new AggregatedValueObject[] { willSavedVO });
			...
			if (tempVO[0].getCupsourcebilltype() != null) {
				...
				while (!willSavedVO.getUserConfirmFlag().booleanValue()) {
					try {
						...
						SCMEnv.out(ex);
						if (MessageDialog.showYesNoDlg(this, nc.ui.ml.NCLangRes
								.getInstance().getStrByID("40040401",
										"UPPSCMCommon-000270")/* @res "提示" */, ex
								.getMessage()) == MessageDialog.ID_YES) {
							// 继续保存
							bConfirm = true;
						} else {
							return false;
				...
				// add by zhang xiao wei 2013-09-09 更新参照标识
				if (InvoiceVO.FROM_VMI == willSavedVO.getSource()) {
					String action = "save";
					IUpdateArap updatearap = (IUpdateArap) NCLocator
							.getInstance().lookup(IUpdateArap.class.getName());
					// MessageDialog.showWarningDlg(this, "提示",
					// "'"+willSavedVO.getBodyVO()[0].getCupsourcebillid()+"'");
					updatearap.updateArap(willSavedVO.getBodyVO()[0]
							.getCupsourcebillid(), action);
			...
			if (vos != null) {
				...
				// Operlog operlog = new Operlog();
				// operlog.insertBusinessExceptionlog(vos, "保存", "保存",
				// nc.vo.scm.funcs.Businesslog.MSGMESSAGE,
				// nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
				// nc.ui.sm.cmenu.Desktop.getApplet().getParameter("USER_IP"));
			}
		 ...
		 */
		try {
			getInvokeEventProxy().afterAction(nc.vo.scm.plugin.Action.SAVE,
					new AggregatedValueObject[] { willSavedVO });
		} catch (BusinessException e) {
			// 日志异常
	...
	private boolean onSendAudit(InvoiceVO vo) {
		...
		boolean isNeedSendToAuditQ = BusiBillManageTool.isNeedSendToAudit(
				ScmConst.PO_Invoice, getPk_corp(), getCurBizeType(), billid,
				getCurOperator());
		try {
			// 编辑状态送审＝“保存”＋“送审”
			...
			if (isNeedSendToAuditQ
				...
				Object[] retValues = (Object[]) PfUtilClient.processAction(
						"SAVE", nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
						ClientEnvironment.getInstance().getDate().toString(),
						vo);
				//
				InvoiceVO resultVO = null;
	...
	private void setButtonsAndPanelState() {
		...
		if (getCurOperState() == STATE_INIT) {
			...
			// if((iBillStatus.compareTo(nc.vo.scm.pu.BillStatus.FREE) == 0 ||
			// (iBillStatus.compareTo(nc.vo.scm.pu.BillStatus.AUDITFAIL) ==
			// 0))){
			// if(iBillStatus.compareTo(nc.vo.scm.pu.BillStatus.FREE) == 0 ){
			// m_btnSendAudit.setEnabled(isNeedSendToAuditQ);}
			// else if((iBillStatus.compareTo(nc.vo.scm.pu.BillStatus.AUDITFAIL)
			// == 0)
			// || (iBillStatus.compareTo(nc.vo.scm.pu.BillStatus.AUDITFAIL) ==
			// 0)){
			...
			if (!isNeedSendToAuditQ
					&& (iBillStatus.compareTo(nc.vo.scm.pu.BillStatus.AUDITING) == 0)
					&& PuPubVO
							.getString_TrimZeroLenAsNull(getInvVOs()[getCurVOPos()]
									.getHeadVO().getCauditpsn()) == null) {
			...
			if (m_bAdd) {
				isNeedSendToAuditQ = BusiBillManageTool.isNeedSendToAudit(
						ScmConst.PO_Invoice, getPk_corp(),
						m_bizButton.getTag(), null, getClientEnvironment()
								.getUser().getPrimaryKey());
			} else {
				isNeedSendToAuditQ = setSendAuditBtnState();
			}
	...
	private void setButtonsStateBrowseNormal() {
		if (getCurPanelMode() == INV_PANEL_CARD) {
			...
			m_btnInvShift.setName(NCLangRes.getInstance().getStrByID(
					"SCMCOMMON", "UPPSCMCommon-000464")/** @res* "列表显示" */
			);
		} else if (getCurPanelMode() == INV_PANEL_LIST) {
			...
			m_btnInvShift.setName(NCLangRes.getInstance().getStrByID(
					"SCMCOMMON", "UPPSCMCommon-000463")/** @res* "卡片显示" */
			);
		}
		...
		if (getCurPanelMode() == INV_PANEL_LIST
				|| m_btnInvBillBusiType.isVisible() == false
				|| getCurBizeType() == null) {
			// 没有业务类型则增加按钮可用
		...
		if (getInvVOs() == null) {
			...
			if (getInvVOs() != null
					...
					&& getCurPanelMode() != INV_PANEL_LIST) {
			...
			if (getCurVOPos() != -1) {
				...
				if (intBillStatus == 0) { // 自由
					...
					if (item != null) {
						String strAuditPsn = PuPubVO
								.getString_TrimZeroLenAsNull(item
										.getValueObject());
			...
			if (VO[i].getHeadVO().getIbillstatus().intValue() != 0) {
				...
				for (int j = 0; j < bodyVO.length; j++) {
					if (bodyVO[j].getNaccumsettmny() != null
							&& Math.abs(bodyVO[j].getNaccumsettmny()
									.doubleValue()) > 0) {
						bModify = false;
						break;
					}
	...
	private void setCurrMoneyDigitAndReCalToBill(String strCurr) {
		...
		for (int i = 0; i < getBillCardPanel().getBillModel().getRowCount(); i++) {
			noriginaltaxmny = getBillCardPanel().getBodyValueAt(i,
					"noriginaltaxmny") == null ? new UFDouble(0)
					: (UFDouble) getBillCardPanel().getBodyValueAt(i,
							"noriginaltaxmny");// 原币税额
			noriginalcurmny = getBillCardPanel().getBodyValueAt(i,
					"noriginalcurmny") == null ? new UFDouble(0)
					: (UFDouble) getBillCardPanel().getBodyValueAt(i,
							"noriginalcurmny");// 原币无税金额
			noriginalsummny = getBillCardPanel().getBodyValueAt(i,
					"noriginalsummny") == null ? new UFDouble(0)
					: (UFDouble) getBillCardPanel().getBodyValueAt(i,
							"noriginalsummny");// 原币价税合计
			if (noriginaltaxmny.add(noriginalcurmny).compareTo(noriginalsummny) != 0) {
				afterEditInvBillRelations(new BillEditEvent(getBillCardPanel()
						.getBodyItem("ninvoicenum"), getBillCardPanel()
						.getBodyValueAt(i, "ninvoicenum"), "ninvoicenum", i,
						BillItem.BODY));
			}
		}
	 ...
	 * 如果传入空，则自动计算折本汇率，否则取该值 UFDouble dARate 折辅汇率 如果传入空，则自动计算折辅汇率，否则取该值 返回：无
	 * 例外：无 日期：(2002-3-13 11:39:21) 修改日期，修改人，修改原因，注释标志： 2002-07-11 wyf 改为使用公共方法
	 * 2002-07-17 wyf 修改折辅汇率写错KEY的问题
	 */
	private void setCurrRateToBillHead(String sCurr, UFDouble dBRate) {

		String sRateDate = getBillCardPanel().getHeadItem("darrivedate")
				.getValue();
	...
	private void setDefaultBankAccountForAVendor(String strVendorBase) {
		...
		// if (nc.vo.scm.pu.PuPubVO.getString_TrimZeroLenAsNull(strVendorBase)
		// != null) {
	...
	private void setDefaultInfoForAFreeCust(String cfreeId) {
		...
		Object[][] strInfo = null;
		try {
			strInfo = nc.ui.pu.pub.PubHelper.queryResultsFromAnyTable(
					"so_freecust", new String[] { "vphone", "vaccname",
							"vaccount" }, "cfreecustid='" + cfreeId + "'");
		} catch (Exception e1) {
			SCMEnv.out(e1);
			return;
		}
	...
	private void setEditableWhenEdit_ExchRate() {
		// 汇率是否可编辑
		String sCurr = getBillCardPanel().getHeadItem("ccurrencytypeid")
				.getValue();
		boolean[] baEditable = POPubSetUI.getBothExchRateEditable(getPk_corp(),
				sCurr);
		getBillCardPanel().getHeadItem("nexchangeotobrate").setEnabled(
	...
	private void setEditableWhenEdit_FreeCustAndBank() {
		...
		if (getBillCardPanel().getHeadItem("cfreecustid").getValue() == null) {
			...
			if (getBillCardPanel().getHeadItem("cvendorbaseid").getValue() == null
					|| getBillCardPanel().getHeadItem("cvendorbaseid")
				...
				String sFreeFlag = "N";
				Object oTemp = getBillCardPanel().getHeadItem("cvendorbaseid")
						.getValue();
				if (oTemp != null)
				...
				if (sFreeFlag != null && sFreeFlag.trim() != null
						&& sFreeFlag.equals("Y")) {
					getBillCardPanel().getHeadItem("cfreecustid").setEnabled(
							true);
					getBillCardPanel().getHeadItem("caccountbankid")
							.setEnabled(false);
				} else {
		...
		getBillCardPanel().getHeadItem("cvendorphone").getComponent()
				.setEnabled(false);
	...
	private void setEditableWhenEdit_Inventory() {
		...
		for (int i = 0; i < iCount; i++) {
			String sUpSourceBillType = (String) getBillCardPanel()
					.getBodyValueAt(i, "cupsourcebilltype");
			if (sUpSourceBillType != null
					&& (sUpSourceBillType.trim().equals(
							nc.vo.scm.pu.BillTypeConst.STORE_PO)
							|| sUpSourceBillType.trim().equals(
									nc.vo.scm.pu.BillTypeConst.STORE_SC)
							|| sUpSourceBillType.trim().equals(
									nc.vo.scm.pu.BillTypeConst.STORE_ES) || sUpSourceBillType
							.trim().equals("50"))) {
	...
	private void setEditableWhenEdit_VProduceNum() {
		...
		for (int i = 0; i < iRow; i++) {
			String sMangId = (String) getBillCardPanel().getBodyValueAt(i,
					"cmangid");
			if (sMangId == null || sMangId.trim().length() < 1) {
				...
				// =====是否可编辑
				getBillCardPanel().setCellEditable(i, "vproducenum",
	...
	public void setHeadCurrency(InvoiceVO inv) {
		...
		if (head != null && items != null && items.length > 0) {
			...
			for (int i = 0; i < items.length; i++) {
				...
				// 得到折本、折辅汇率精度
				iaExchRateDigit = m_listPoPubSetUI2.getBothExchRateDigit(
						getPk_corp(), sCurrId);
				if (iaExchRateDigit == null || iaExchRateDigit.length == 0) {
					getBillCardPanel().getBillModel().getItemByKey(
							"nexchangeotobrate").setDecimalDigits(5);
				} else {
					getBillCardPanel().getBillModel().getItemByKey(
			...
			if (iaExchRateDigit == null || iaExchRateDigit.length == 0) {
				getBillCardPanel().getHeadItem("nexchangeotobrate")
						.setDecimalDigits(5);
			} else {
				getBillCardPanel().getHeadItem("nexchangeotobrate")
	...
	private void setInvEditFormulaInfo(BillCardPanel pnlBillCard,
		...
		try {
			...
			// ================解析基本档案：计量档案
			String[] saFormula = new String[] { "getColValue(bd_measdoc,measname,pk_measdoc,cmessureunit)" };
			PuTool.getFormulaParse().setExpressArray(saFormula);
			int iFormulaLen = saFormula.length;
	...
	private void setInvoiceTypeComItem() {
		...
		// getBillCardPanel().getHeadItem("iinvoicetype").getComponent();
		// if (getCurOperState() == STATE_EDIT) {
	...
	private void setNewLineDefaultValue(int iNewRow) {
		// 设置新增的表体行的扣税类别与表头相同
		getBillCardPanel().setBodyValueAt(
				((UIComboBox) getBillCardPanel()
						.getHeadItem("idiscounttaxtype").getComponent())
						.getSelectedItem(), iNewRow, "idiscounttaxtype");
		// 税率=表头税率
		if (getBillCardPanel().getHeadItem("ntaxrate").getValue() != null) {
			getBillCardPanel().setBodyValueAt(
					getBillCardPanel().getHeadItem("ntaxrate").getValue(),
					iNewRow, "ntaxrate");
		}
		// 折本汇率=表头折本汇率,折辅汇率=表辅折本汇率
		getBillCardPanel().setBodyValueAt(
				getBillCardPanel().getHeadItem("nexchangeotobrate").getValue(),
				iNewRow, "nexchangeotobrate");
		// 币种=表头币种 （1）为币种ID设值（2）显示币种名称
		String strCurrTypeId = ((UIRefPane) getBillCardPanel().getHeadItem(
				"ccurrencytypeid").getComponent()).getRefPK();
		getBillCardPanel().setBodyValueAt(strCurrTypeId, iNewRow,
				"ccurrencytypeid");
		getBillCardPanel().setBodyValueAt(
				((UIRefPane) getBillCardPanel().getHeadItem("ccurrencytypeid")
	...
	public void setRelated_Taxrate(int iBeginRow, int iEndRow) {
		...
		for (int i = 0; i < iSize; i++) {
			...
			if (dCurTaxRate != null) {
				...
				BillEditEvent tempE = new BillEditEvent(getBillCardPanel()
	...
	private boolean setSendAuditBtnState() {
		...
		if (curVO.getHeadVO().getIbillstatus().intValue() == 4
				&& getCurOperState() == STATE_EDIT) {
			// 审批未通过,修改时,应设置为自由态
			curVO.getHeadVO().setIbillstatus(new Integer(0));
			hBillStatusBeforeEdit.put(curVO.getHeadVO().getCinvoiceid(),
					new Integer(4));
		}
		// 效率优化 by zhaoyha at 2009.8
		UFBoolean needSend = (UFBoolean) curVO.getHeadVO().getAttributeValue(
				ConstantVO.EXT_ATTR_ISNEEDSENDAUDIT);
		if (null != needSend)
			b = needSend.booleanValue();
		// 送审
		else
			b = PuTool.isNeedSendToAudit(nc.vo.scm.pu.BillTypeConst.PO_INVOICE,
					curVO);
		// if (b){
		// m_btnSendAudit.setEnabled(true);
		// m_btnInvBillAudit.setEnabled(false);
	...
	private void setVoBodyToListPanle(int nCurIndex) {
		...
		if (nCurIndex < 0 || nCurIndex >= getInvVOs().length) {
			...
			if (vTemp.size() > 0) {
				...
				new nc.ui.scm.pub.FreeVOParse().setFreeVO(tempbodyVO, "vfree0",
						"vfree", null, "cmangid", false);
			}
			// 设置金额等位数为最大的位数
			setMaxMnyDigitList(iMaxMnyDigit);
			// 设置VO
			getBillListPanel().setBodyValueVO(
					getInvVOs()[nCurIndex].getBodyVO());
			// 表体币种精度
			try {
				resetBodyValueRelated_Curr(getPk_corp(), getInvVOs()[nCurIndex]
						.getHeadVO().getCcurrencytypeid(), getBillListPanel()
						.getBodyBillModel(), new BusinessCurrencyRateUtil(
						getPk_corp()), getBillListPanel().getBodyBillModel()
						.getRowCount(), m_listPoPubSetUI2);
			} catch (Exception e) {
				SCMEnv.out(e);
			}
			...
			// ccur =
			// getInvVOs()[nCurIndex].getBodyVO()[i].getCcurrencytypeid();
			// //设置表体金额等精度
			// PiPqPublicUIClass.setCurrMoneyDigitToListBody(getInvListPanel(),
			// ccur);
			// //表体币种精度
		...
		for (int i = 0; i < getInvHVOs().length; i++) {
			if (getInvVOs()[i].getHeadVO().getCinvoiceid() == null) {
				...
				if (getInvHVOs()[i].getNexchangeotobrate() == null) {
					...
					UFDouble[] d = POPubSetUI.getBothExchRateValue(
							getPk_corp(), getInvHVOs()[i].getCcurrencytypeid(),
							getInvHVOs()[i].getDarrivedate());

					getInvHVOs()[i].setNexchangeotobrate(d[0]);
				}
				// 设置折本及折辅汇率的精度
				PiPqPublicUIClass.setCurrRateDigitToListHead(
						getBillListPanel(), getInvHVOs()[i]
								.getCcurrencytypeid());
				// 设置VO
				if (j == 0 && count == 0) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (invoiceHeaderVO.getCinvoiceid() == null) {
					if (invoiceHeaderVO.getFinitflag() == null
							|| invoiceHeaderVO.getFinitflag().intValue() == 0) // 普通
	...
	private void setVOsToListPanel() {
		...
		for (int i = 0; i < getInvHVOs().length; i++) {
			// 加行
			getBillListPanel().getHeadBillModel().addLine();
			getBillListPanel().getHeadBillModel().setBodyRowVO(getInvHVOs()[i],
					i);
	...
	private void setVOToAuditedBillPanel() {
		...
		// 表头币种与表体币种保持一致
		if (head.getCcurrencytypeid() == null
				|| head.getCcurrencytypeid().trim().length() < 1) {
			// 查出的发票
			if (curVO.getBodyVO() != null) {
				head.setCcurrencytypeid(curVO.getBodyVO()[0]
						.getCcurrencytypeid());
		...
		Vector vTemp = new Vector();
		for (int i = 0; i < bodyVO.length; i++) {
			if (bodyVO[i].getVfree1() != null || bodyVO[i].getVfree2() != null
					|| bodyVO[i].getVfree3() != null
					|| bodyVO[i].getVfree4() != null
					|| bodyVO[i].getVfree5() != null)
				vTemp.addElement(bodyVO[i]);
		...
		if (head.getCvendormangid() != null
				&& (strVendorBase == null || strVendorBase.trim().equals(""))) {
			// strVendorBase =
			// getResultFromFormula("getColValue(bd_cumandoc,pk_cubasdoc,pk_cumandoc,cvendormangid)","cvendormangid",head.getCvendormangid())
			// ;
		...
		if (curVO.getHeadVO().getCfreecustid() == null) {
			...
			// 开户银行:
			// UIRefPane pane1 = (UIRefPane)
			// getBillCardPanel().getHeadItem("caccountbankid").getComponent();
			// ((AccountsForVendorRefModel)
			// pane1.getRef().getRefModel()).setCvendorbaseid(strVendorBase);
			if (head.getCaccountbankid() != null) {
				// pane1.setPK(head.getCaccountbankid());
				String strAccount = null;
				try {
					Object[][] retOb = nc.ui.pu.pub.PubHelper
							.queryResultsFromAnyTable("bd_bankaccbas",
									new String[] { "account" },
									"pk_bankaccbas='"
			...
			if (head.getCaccountbankid() != null)
				getBillCardPanel().setHeadItem(
						"cvendoraccount",
						PiPqPublicUIClass.getAResultFromTable("bd_bankaccbas",
								"account", "pk_bankaccbas", head
										.getCaccountbankid()));
		} else {
			// 开户银行不可编辑,按钮不可见
			((UIRefPane) getBillCardPanel().getHeadItem("caccountbankid")
		...
		// 设置业务员
		UIRefPane pane2 = (UIRefPane) getBillCardPanel().getHeadItem(
				"cemployeeid").getComponent();
		...
		// bd_deptdoc.pk_deptdoc from bd_deptdoc where (bd_deptdoc.deptattr =
		// '2' or
		// bd_deptdoc.deptattr = '4') and bd_deptdoc.pk_corp ='"+pk_corp+"'))";
		...
		// 扣税类别
		int selectedIndex = ((UIComboBox) getBillCardPanel().getHeadItem(
				"idiscounttaxtype").getComponent()).getSelectedIndex();
		((UIComboBox) getBillCardPanel().getHeadItem("idiscounttaxtype")
		...
		if (curVO.getBodyVO() != null) {
			for (int i = 0; i < curVO.getBodyVO().length; i++) {
				...
				getBillCardPanel().setBodyValueAt(
						getBillCardPanel().getBodyValueAt(i, "nmoney"), i,
						"nmoney");
				getBillCardPanel().setBodyValueAt(
						getBillCardPanel().getBodyValueAt(i, "ntaxmny"), i,
						"ntaxmny");
				getBillCardPanel().setBodyValueAt(
						getBillCardPanel().getBodyValueAt(i, "nsummny"), i,
						"nsummny");
				getBillCardPanel().setBodyValueAt(
						getBillCardPanel().getBodyValueAt(i, "npaymentmny"), i,
						"npaymentmny");
			}
		}
		// wyf 2002-09-18 add begin
		...
		getBillCardPanel().getHeadItem("iinvoicetype").setValue(
				head.getIinvoicetype().intValue());
	 ...
	 * 加入对来源单据类型、来源单据号的处理 2002-09-27 wyf 加入对发票类型下拉框的限制 2002-10-23 wyf
	 * 业务员参照修改为树表状 2003-02-25 fangy 修改
	 */
	private void setVOToBillPanel() {
		...
		if (items != null && items.length > 0) {
			...
			if (vTemp.size() > 0) {
				...
				new nc.ui.scm.pub.FreeVOParse().setFreeVO(bodyVO, "vfree0",
						"vfree", null, "cmangid", false);
			}
		}
		...
		// 设置部门
		String pk_corp = head.getPk_purcorp();
		UIRefPane pane2 = (UIRefPane) getBillCardPanel().getHeadItem("cdeptid")
				.getComponent();
		...
		// 设置业务员
		pane2 = (UIRefPane) getBillCardPanel().getHeadItem("cemployeeid")
				.getComponent();
		// wyf 2002-10-23 modify begin
		// ((EmployeesForDeptRefModel)pane2.getRef().getRefModel()).setCdeptid(head.getCdeptid())
		// ;
		// String s = " bd_psndoc.indocflag='Y' and (bd_psndoc.pk_deptdoc in
		// (select
		// bd_deptdoc.pk_deptdoc from bd_deptdoc where (bd_deptdoc.deptattr =
		// '2' or
		// bd_deptdoc.deptattr = '4') and bd_deptdoc.pk_corp ='"+pk_corp+"')) ";
		...
		// if(!StringUtil.isEmptyWithTrim(curVO.getHeadVO().getCvendormangid())
		// && curVO.getHeadVO().getCvendormangid().equals(
		// getBillCardPanel().getHeadItem("cvendormangid").getValueObject())
		// ){
		// String vendorManId=(String)
		// getBillCardPanel().getHeadItem("cvendormangid").getValueObject();
		// AbstractRefModel vendorRefModel=((UIRefPane)
		// getBillCardPanel().getHeadItem("cvendormangid").getComponent())
		// .getRefModel();
		// CustmandocDefaultRefModel newVendorRefModel= new
		// CustmandocDefaultRefModel("供应商档案");
		// Vector allData=vendorRefModel.getData();
		// Vector selectedData=vendorRefModel.getSelectedData();
		// curVO.getHeadVO().setCvendormangid(null);
		// //vendorRefModel.setSelectedData(null);
		// ((UIRefPane)
		// getBillCardPanel().getHeadItem("cvendormangid").getComponent()).setRefModel(newVendorRefModel);
		// Object a=BatchMatchContext.getShareInstance().htUIRePaneTable;
		// //getBillCardPanel().getHeadItem("cvendormangid").setDataType(BillItem.STRING);
		// // 设置VO
		// getBillCardPanel().setBillValueVO(curVO);
		// getBillCardPanel().getBillData().addBillItem(cunpos, tablecode, item)
		// //getBillCardPanel().getHeadItem("cvendormangid").setDataType(BillItem.UFREF);
		// newVendorRefModel.setData(allData);
		// newVendorRefModel.setSelectedData(selectedData);
		//      
		// curVO.getHeadVO().setCvendormangid(vendorManId);
		// }
		// else
		// 设置VO
		getBillCardPanel().setBillValueVO(curVO);
		...
		// 设置表头币种精度
		resetHeadCurrDigits();
		getBillCardPanel().getHeadItem("nexchangeotobrate").setValue(
		...
		if (voFirstItem.getIdiscounttaxtype() != null
				&& voFirstItem.getIdiscounttaxtype().toString().trim().length() > 0) {
			getBillCardPanel().getHeadItem("idiscounttaxtype").setValue(
		...
		getBillCardPanel().getHeadItem("ntaxrate").setValue(
		...
		if (curVO.getHeadVO().getCfreecustid() == null) {
			...
			setDefaultPhoneForAVendor(strVendorBase);
			// 开户银行:
			UIRefPane pane1 = (UIRefPane) getBillCardPanel().getHeadItem(
					"caccountbankid").getComponent();
			// ((AccountsForVendorRefModel)
			// pane1.getRef().getRefModel()).setCvendorbaseid(strVendorBase);
			if (head.getCaccountbankid() != null) {
				...
				String strAccount = null;
				try {
					Object oTemp = CacheTool.getColumnValue("bd_bankaccbas",
							"pk_bankaccbas", "accountname", new String[] { head
									.getCaccountbankid() });
					if (oTemp != null)
						strAccount = ((Object[]) oTemp)[0] == null ? null
								: ((Object[]) oTemp)[0].toString();
		...
		if (curVO.getBodyVO() != null) {
			for (int i = 0; i < curVO.getBodyVO().length; i++) {
				...
				// 因无法得到旧币种ID，因此不管币种精度是否改变，均重新计算，虽然有可能导致数据改变。
				// UFDouble ninvoicenum = curVO.getBodyVO()[i].getNinvoicenum();
				// UFDouble ncurprice =
				// curVO.getBodyVO()[i].getNoriginalcurprice();
				// UFDouble summy = curVO.getBodyVO()[i].getNoriginalsummny();
				// String sChangedKey = null;
				// if(ninvoicenum != null && ninvoicenum.doubleValue() > 0.00){
				// sChangedKey = "ninvoicenum";
				// }else if ((ninvoicenum == null || (ninvoicenum != null &&
				// ninvoicenum.doubleValue() == 0.00)) && (summy != null &&
				// summy.doubleValue() > 0.00)){
				// sChangedKey = "noriginalsummny";
				// }
				// afterEditWhenBodyRelationsCal(new BillEditEvent(
				// getInvBillPanel().getBodyItem(sChangedKey),
				// getInvBillPanel().getBodyValueAt(i, sChangedKey),
				// sChangedKey, i, BillItem.BODY));
		...
		if (getBillCardPanel().getHeadItem("arrvcorpname") != null) {
			refpnl = (UIRefPane) getBillCardPanel().getHeadItem("arrvcorpname")
					.getComponent();
			refpnl.setValue((String) ob);
		}
	...
	protected static void resetBodyValueRelated_Curr(String pk_corp,
		...
		String[] saMnyItem = new String[] { "noriginalcurmny",
				"noriginaltaxmny", "noriginalsummny", "noriginalpaymentmny",
				"nmoney", "ntaxmny", "nsummny", "npaymentmny" };
		...
		if (bCard && getBillCardPanel().getRowCount() > 0) {
			...
			if (item != null && item.isShow()) {
				...
				for (int i = 0; i < nRowCount; i++)
					cMangID[i] = (String) getBillCardPanel().getBodyValueAt(i,
							"cmangid");
				UFDouble nPrice[] = queryPlanPrices(cMangID, getBillCardPanel()
						.getHeadItem("cstoreorganization").getValue());
				if (nPrice != null) {
					for (int i = 0; i < nRowCount; i++)
						getBillCardPanel().setBodyValueAt(nPrice[i], i,
								"nplanprice");
				}
			...
			if (item != null && item.isShow()) {
				int nRowCount = getBillListPanel().getBodyBillModel()
						.getRowCount();
				String cMangID[] = new String[nRowCount];
				for (int i = 0; i < nRowCount; i++)
					cMangID[i] = (String) getBillListPanel().getBodyBillModel()
							.getValueAt(i, "cmangid");
				UFDouble nPrice[] = queryPlanPrices(
						cMangID,
						(String) getBillListPanel()
								.getHeadBillModel()
	...
	private void shiftShowModeTo(int stateIndex) {
		if (stateIndex == INV_PANEL_LIST && getCurPanelMode() != INV_PANEL_LIST) {
			...
			// setButtonsAndPanelState();
			updateButtons();
		} else if (stateIndex == INV_PANEL_CARD
				&& getCurPanelMode() != INV_PANEL_CARD) {
			// wyf 2002-08-20 delete begin
			// setTitleText("维护发票") ;
			// wyf 2002-08-20 delete end
			remove(getCurPanel());
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		if (iaSelectedRow == null || iaSelectedRow.length == 0) {
			...
			if (iCount == 1) {
				// 单选时,才显示表体
				int nCurIndex = PuTool.getIndexBeforeSort(getBillListPanel(),
						iaSelectedRow[0]);
				setCurVOPos(nCurIndex);
			} else {
				// 多选时,不显示任何表体
	...
	private void onCardPrint() {
		...
		try {
			...
			// List<BillItem>
			// maskedItems=PuTool.maskUIRefType(getBillCardPanel());
	...
	private void onCardPrintPreview() {
		...
		try {
			...
			printCard.onCardPrintPreview(getBillCardPanel(),
					getBillListPanel(), ScmConst.PO_Invoice);
			if (PuPubVO
					.getString_TrimZeroLenAsNull(printCard.getPrintMessage()) != null) {
				MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes
						.getInstance().getStrByID("SCMCOMMON",
	...
	private void onBatchPrintPreview() {
		...
		try {
			printList.onBatchPrintPreview(getBillListPanel(),
					getBillCardPanel(), ScmConst.PO_Invoice);
			if (PuPubVO
					.getString_TrimZeroLenAsNull(printList.getPrintMessage()) != null) {
				MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes
						.getInstance().getStrByID("SCMCOMMON",
	 ...
	 * 
	 * @see
	 * nc.ui.scm.pub.print.ISetBillVO#setBillVO(nc.vo.pub.AggregatedValueObject)
	 */
	public void setBillVO(AggregatedValueObject vo) {
		...
		if (getCurPanelMode() == INV_PANEL_LIST) {
			for (int i = 0; i < getInvVOs().length; i++) {
				if (getInvVOs()[i].getPrimaryKey().equalsIgnoreCase(
						((InvoiceVO) vo).getPrimaryKey())) {
					setCurVOPos(i);
				}
	...
	public void doMaintainAction(ILinkMaintainData maintaindata) {
		...
		comItem.addItem(nc.ui.ml.NCLangRes.getInstance().getStrByID(
				nc.vo.scm.pu.InvoiceType.m_sModuleNamePath,
		...
		for (int i = 0; i < bodyVO.length; i++) {
			if (bodyVO[i].getNaccumsettmny() != null
					&& Math.abs(bodyVO[i].getNaccumsettmny().doubleValue()) > 0)
				return;
		}
	...
	public void doQueryAction(ILinkQueryData querydata) {
		...
		try {
			...
			if (vo == null) {
				MessageDialog.showHintDlg(this,
						NCLangRes.getInstance().getStrByID("SCMCOMMON",
								"UPPSCMCommon-000270")/* "提示" */, NCLangRes
								.getInstance().getStrByID("SCMCOMMON",
			...
			if (queryDlg.getAllTempletDatas() == null
				...
				queryDlg.setTempletID(strPkCorp, getModuleCode(),
						nc.ui.pub.ClientEnvironment.getInstance().getUser()
								.getPrimaryKey(), null);
			...
			alcorp.add(ClientEnvironment.getInstance().getCorporation()
					.getPrimaryKey());
			queryDlg.initCorpRef(IDataPowerForInv.CORPKEY, ClientEnvironment
					.getInstance().getCorporation().getPrimaryKey(), alcorp);
			queryDlg.setCorpRefs(IDataPowerForInv.CORPKEY,
			...
			InvoiceVO[] VOs = null;
			VOs = InvoiceHelper.queryInvoiceVOsByCondsMy(getClientEnvironment()
					.getUser().getPrimaryKey(), new NormalCondVO[] {
					new NormalCondVO("公司", strPkCorp),
					new NormalCondVO("单据ID", billID) }, voaCond);// 设置NormalCondVO条件：按照单据所属公司加载
	...
	public void doApproveAction(ILinkApproveData approvedata) {
			...
			for (int i = 0; i < aryForAudit.length; i++) {
				...
				if (aryForAudit[i].getChildCount() > 0) {
					for (int j = 0; j < aryForAudit[i].getChildCount(); j++)
						aryForAudit[i].getChildButtonGroup()[j]
								.setEnabled(false);
				}
			}
			for (int i = 0; i < aryForAudit.length; i++) {
		...
		getBillListPanel().getHeadBillModel()
				.setRowState(0, BillModel.SELECTED);
	}

	/**
	 * 作者：汪维敏 功能：(本次开票数量+累计开票数量)*订单或入库单的数量>0 参数： 返回： 例外： 创建日期：(2004-4-15
	 * 11:23:07)
	 */
	private boolean negativeAndPlusCtrl(InvoiceVO billVO) throws Exception {
		...
		if (!sUpSourceType.equals(ScmConst.PO_Order)
				&& !sUpSourceType.equals(ScmConst.SC_Order)
		...
		try {
			...
			IInvoiceD bo = (IInvoiceD) nc.bs.framework.common.NCLocator
		...
		for (int i = 0; i < newItems.length; i++) {
			sUpSourceRowID = newItems[i].getCupsourcebillrowid();
			if (sUpSourceRowID == null
					|| newItems[i].getStatus() == VOStatus.DELETED)
				continue;
			...
			for (int j = 0; j < oldItems.length; j++) {
				if (sUpSourceRowID.equals(oldItems[j].getCupsourcebillrowid())
	...
	private void setDefaultValueByUser() {
		...
		if (cemployeeid != null && cemployeeid.getRefPK() == null) {
			PsndocVO voPsnDoc = PuTool.getPsnByUser(cemployeeid.getRefPK(),
					getPk_corp());
			// IUserManageQuery qrySrv = (IUserManageQuery)
			// NCLocator.getInstance().lookup(IUserManageQuery.class.getName());
			// PsndocVO voPsnDoc = null;
			// try {
			// voPsnDoc =
			// qrySrv.getPsndocByUserid(nc.ui.pub.ClientEnvironment.getInstance().getCorporation().getPrimaryKey(),
			// getCurOperator());
			// }
			// catch (BusinessException be) {
			// SCMEnv.out(be);
	...
	private boolean isNeedSendAudit(int iBillStatus) {
		...
		if (iBillStatus == -1) {
			if (getSelectedRowCount() > 1)
				return m_btnSendAudit.isEnabled();
			if (m_bizButton != null && m_bizButton.getTag() != null) {
				isNeedSendToAuditQ = BusiBillManageTool
						.isNeedSendToAudit("25", getClientEnvironment()
								.getCorporation().getPrimaryKey(), m_bizButton
								.getTag(), null, getClientEnvironment()
								.getUser().getPrimaryKey());
			}
		}
		// 自由(修改情况)
			...
			String cbizType = getInvVOs()[getCurVOPos()].getHeadVO()
					.getCbiztype();
			// 效率优化 by zhaoyha at 2009.8
			UFBoolean needSend = (UFBoolean) getInvVOs()[getCurVOPos()]
					.getHeadVO().getAttributeValue(
							ConstantVO.EXT_ATTR_ISNEEDSENDAUDIT);
			if (null == needSend
					&& PuPubVO.getString_TrimZeroLenAsNull(cbizType) != null) {
				isNeedSendToAuditQ = BusiBillManageTool
						.isNeedSendToAudit("25", getClientEnvironment()
		...
		if (getCurVOPos() >= 0
				&& null != getInvVOs()
				&& !getCurOperator().equals(
						getInvVOs()[getCurVOPos()].getHeadVO().getCoperator()))
	...
	private void resetStoreorgAndStordoc(InvoiceVO vos[], String strUpBillType) {
		...
		for (int j = 0; j < intLen; j++) {
			...
			boolean isFSJJPur = OrderPubVO.isFSJJPur(strPurCorp, strArrCorp,
					strInvoiceCorp);
			if (isFSJJPur) {
		...
		if (BillTypeConst.STORE_PO.equalsIgnoreCase(strUpBillType)) {// 来源于库存入库单的清空表头库存组织和表体仓库
			...
			for (int j = 0; j < IsFSJJPurVOLen; j++) {
				...
				ClientCacheHelper
						.getColValue(new InvoiceHeaderVO[] { aryIsFSJJPurVO[j]
								.getHeadVO() },
								new String[] { "cstoreorganization" },
								"bd_purorg", "pk_purorg",
								new String[] { "settlestockorg" },
								"cpurorganization");
				sCstoreorganization = aryIsFSJJPurVO[j].getHeadVO()
						.getCstoreorganization();
				if (PuPubVO.getString_TrimZeroLenAsNull(sCstoreorganization) != null) {
					vctStoreorganization.add(sCstoreorganization);// 库存组织ID
				}
			}
			String[] saCstoreorganization = new String[vctStoreorganization
					.size()];
			...
			if (t != null && t.size() > 0) {
				for (int jj = 0; jj < IsFSJJPurVOLen; jj++) {
					...
					if (items != null) {
						int itemSize = items.length;
						String Cstoreorganization = aryIsFSJJPurVO[jj]
								.getHeadVO().getCstoreorganization();// 库存组织
						for (int jjj = 0; jjj < itemSize; jjj++) {
							Object oTemp = t.get(Cstoreorganization);// 采购发票表体仓库应为直运仓
							items[jjj].setCwarehouseid(oTemp == null ? null
									: oTemp.toString());
						}
					}
				}
	 ...
	 *            森工NC
	 */
	public void afterEditWhenBodyAssist(ToftPanel uiPanel, BillCardPanel bcp,
			BillEditEvent e) {
		int iRow = e.getRow();
	 ...
	 * @param e
	 */
	public void afterEditWhenBodyRate(ToftPanel ui, BillCardPanel bcp,
			BillEditEvent e) {
		UFDouble ninvoicenum = PuPubVO.getUFDouble_NullAsZero(bcp
				.getBodyValueAt(e.getRow(), "ninvoicenum"));
		...
		// sCassId);
		// 单价
		UFDouble nOriginalCurPrice = PuPubVO.getUFDouble_NullAsZero(bcp
				.getBodyValueAt(e.getRow(), "noriginalcurprice"));
		// 修改计量换算率,请购数量变化
		...
		if (nExchangeRate != null) {
			if (nExchangeRate.doubleValue() < 0) {
				MessageDialog.showErrorDlg(ui, nc.ui.ml.NCLangRes.getInstance()
						.getStrByID("40040101", "UPP40040101-000040")
				/*
				 * @res "计算数据"
				 */, nc.ui.ml.NCLangRes.getInstance().getStrByID("40040101",
						"UPP40040101-000045")
				/*
				 * @res "计量换算率不能为负！"
				 */);
			...
			if (ninvoicenum.doubleValue() != 0) {
				bcp.setBodyValueAt(ninvoicenum.div(nExchangeRate), e.getRow(),
						"nassistnum");
			} else {
				bcp.setBodyValueAt(nAssistNum.multiply(nExchangeRate), e
						.getRow(), "ninvoicenum");
				afterEditWhenBodyRelationsCal(new BillEditEvent(bcp
						.getBodyItem("ninvoicenum"), bcp.getBodyValueAt(e
						.getRow(), "ninvoicenum"), "ninvoicenum", e.getRow(),
						BillItem.BODY));
			}
			// 发票数量变化,金额自动变化
			...
			// bcp.setBodyValueAt(new UFDouble(d), e.getRow(),
			// "noriginalcurmny");
			// }
		} else {
	 ...
	 *            森工NC
	 */
	public void afterEditWhenBodyAssNum(ToftPanel ui, BillCardPanel bcp,
			BillEditEvent e) {
		UFDouble ninvoicenum = PuPubVO.getUFDouble_NullAsZero(bcp
				.getBodyValueAt(e.getRow(), "ninvoicenum"));
		UFDouble nOriginalCurPrice = PuPubVO.getUFDouble_NullAsZero(bcp
		...
		if (nAssistNum != null) {
			...
			// 发票数量变化,金额自动变化
			// ninvoicenum = (UFDouble) bcp.getBodyValueAt(e.getRow(),
			// "ninvoicenum");
	...
	private void aftereditWhenBodyInvoicenum(ToftPanel ui, BillCardPanel bcp,
			BillEditEvent e) {
		// 发票数量
		UFDouble ninvoicenum = PuPubVO.getUFDouble_NullAsZero(bcp
				.getBodyValueAt(e.getRow(), "ninvoicenum"));
		...
		if (bFixedFlag) {
			...
			if (nAssistNum != null) {
				if (nAssistNum.doubleValue() != 0.0) {
					bcp.setBodyValueAt(ninvoicenum.div(nAssistNum), e.getRow(),
							"nexchangerate");
				} else {
					...
					if (nExchangeRate != null
						...
						bcp.setBodyValueAt(ninvoicenum.div(nExchangeRate), e
								.getRow(), "nassistnum");
					}
				}
			} else {
	...
	private void createAddContinueBtn() {
		// 为参照增行增加子按钮
		String strBizType = getBillCardPanel().getHeadItem("cbiztype")
				.getValue();
		btnBillAddContinue.setEnabled(false);
		if (PuPubVO.getString_TrimZeroLenAsNull(strBizType) == null) {
			return;
		}
		if (getInvVOsByContinue() == null || getInvVOsByContinue().length < 1) {
			return;
		}
		String sUpsourcebilltype = PuPubVO
				.getString_TrimZeroLenAsNull(getBillCardPanel().getBodyValueAt(
						0, "cupsourcebilltype"));
		...
		PfUtilClient.retAddBtn(btnBillAddContinue, nc.ui.pub.ClientEnvironment
				.getInstance().getCorporation().getPrimaryKey(),
				nc.vo.scm.pu.BillTypeConst.PO_INVOICE, bo);
		Vector vecBtn = btnBillAddContinue.getChildren();
		if (vecBtn.size() > 0) {
			int intLen = vecBtn.size();
			for (int i = 0; i < intLen; i++) {
				ButtonObject btn = (ButtonObject) vecBtn.get(i);
				if ("自制单据".equalsIgnoreCase(btn.getName())
						|| "Self-prepared document".equalsIgnoreCase(btn
								.getName())
						|| !sContinueBillTypeName.equalsIgnoreCase(btn
								.getName())) {
				...
				if (BillTypeConst.PO_ORDER.equalsIgnoreCase(sUpsourcebilltype)
					...
					if (BillTypeConst.getBillTypeName(BillTypeConst.PO_ORDER)
							.equalsIgnoreCase(btn.getName())) {
						btn.setVisible(true);
					}
				}
				// For V56 by zhaoyha
				if (BillTypeConst.STORE_PO.equalsIgnoreCase(sUpsourcebilltype)
						|| isRefFeeOrder(strBizType).booleanValue()) {
					if (BillTypeConst.getBillTypeName(BillTypeConst.STORE_PO)
							.equalsIgnoreCase(btn.getName())) {
						btn.setVisible(true);
					}
				}
	...
	private void setButtonsBAPFlag() {
		if (getCurOperState() == STATE_EDIT
				|| getCurOperState() == STATE_LIST_FROM_BILLS) {
		...
		if (getInvVOs() == null
				|| getInvVOs()[getCurVOPos()] == null
				|| BillStatus.AUDITED.compareTo(getInvVOs()[getCurVOPos()]
						.getHeadVO().getIbillstatus()) != 0) {
			m_btnCrtAPBill.setEnabled(false);
			m_btnDelAPBill.setEnabled(false);
			return;
		}
		// 虚拟发票，传应付与取消传应付均不可用
		Integer iInvoiceType = getInvVOs()[getCurVOPos()].getHeadVO()
				.getIinvoicetype();
		boolean bVirtFlag = (iInvoiceType != null && iInvoiceType.intValue() == 3);
		//
		UFBoolean bapflag = getInvVOs()[getCurVOPos()].getHeadVO().getBapflag();
		if (PuPubVO.getUFBoolean_NullAs(bapflag, UFBoolean.FALSE)
				.booleanValue()) {
			m_btnCrtAPBill.setEnabled(false);
		...
		if (PuPubVO.getUFBoolean_NullAs(bapflag, UFBoolean.FALSE)
				.booleanValue()) {
			m_btnInvBillUnAudit.setEnabled(BillStatus.AUDITED
					.compareTo(getInvVOs()[getCurVOPos()].getHeadVO()
							.getIbillstatus()) == 0
					&& (isBusitypeAppriveDrive(getInvVOs()[getCurVOPos()]
							.getHeadVO().getCbiztype()) || bVirtFlag));
		}
	...
	private void onCrtAPBill() {
		...
		if (getCurPanelMode() == INV_PANEL_CARD) {
			...
			for (int i = 0; i < inVO.length; i++) {
				if (!PuPubVO.getUFBoolean_NullAs(
						inVO[i].getHeadVO().getBapflag(), UFBoolean.FALSE)
						.booleanValue()) {
		...
		try {
			voaInvoice[0].getHeadVO().setCoperatoridnow(
					ClientEnvironment.getInstance().getUser().getPrimaryKey());
			PfUtilClient.processBatch(this, "CRTAPBILL",
					nc.vo.scm.pu.BillTypeConst.PO_INVOICE, ClientEnvironment
							.getInstance().getDate().toString(), voaInvoice,
					oaUser);
		...
		if (cinvoiceid != null && cinvoiceid.length > 0) {
			...
			if (resultH != null) {
				for (int i = 0; i < getInvVOs().length; i++) {
					if (resultH.containsKey(getInvVOs()[i].getHeadVO()
							.getPrimaryKey())) {
						getInvVOs()[i] = resultH.get(getInvVOs()[i].getHeadVO()
								.getPrimaryKey());
					}
				}
			}
	...
	private void onDelAPBill() {
		...
		try {
			Object o = PfUtilClient.processBatch(this, "DELAPBILL",
					nc.vo.scm.pu.BillTypeConst.PO_INVOICE, ClientEnvironment
							.getInstance().getDate().toString(), voaInvoice,
					oaUser);
			boolean bSucced = PfUtilClient.isSuccess();
	...
	private void setRelated_AssistUnit(int iBeginRow, int iEndRow) {
		...
		String[] saBaseId = (String[]) PuGetUIValueTool.getArrayNotNull(
				getBillCardPanel().getBillModel(), "cbaseid", String.class,
				iBeginRow, iEndRow);
		...
		getBillCardPanel().getBillModel().execFormulas(aryAssistunit,
				iBeginRow, iEndRow);
		//
		String sBaseId = null;
		String sAssistUnit = null;
		for (int iRow = iBeginRow; iRow <= iEndRow; iRow++) {
			sBaseId = (String) getBillCardPanel().getBodyValueAt(iRow,
					"cbaseid");
			if (PuTool.isAssUnitManaged(sBaseId)) {
				sAssistUnit = (String) getBillCardPanel().getBodyValueAt(iRow,
						"cassistunit");
				// 为批量加载作准备
		...
		if (iAssistUnitLen > 0) {
			...
			for (int i = 0; i < iAssistUnitLen; i++) {
				...
				if (oConvRate == null) {
					...
					getBillCardPanel().getBillModel().setValueAt(
							(UFDouble) oConvRate[0], iRow, "nexchangerate");
				}

				// 换算率改变，重新计算
				BillEditEvent tempE = new BillEditEvent(getBillCardPanel()
						.getBodyItem("nexchangerate"), getBillCardPanel()
						.getBodyValueAt(iRow, "nexchangerate"),
						"nexchangerate", iRow);
				afterEditInvBillRelations(tempE);
			}
		}
	...
	protected boolean isBusitypeAppriveDrive(String strBiztypeId) {
		...
		IPFMetaModel srvPFMetaModel = (IPFMetaModel) NCLocator.getInstance()
				.lookup(IPFMetaModel.class.getName());
		try {
			MessagedriveVO[] driveVOs = srvPFMetaModel.queryAllMsgdrvVOs(
					getCorpPrimaryKey(), BillTypeConst.PO_INVOICE,
					strBiztypeId, "APPROVE");
			if (driveVOs == null || driveVOs.length == 0) {
				m_mapBusitypeAppriveDrive.put(strBiztypeId, UFBoolean.FALSE);
			} else {
				driveVOs = srvPFMetaModel.queryAllMsgdrvVOs(
						getCorpPrimaryKey(), BillTypeConst.PO_INVOICE,
						strBiztypeId, "UNAPPROVE");
				if (driveVOs == null || driveVOs.length == 0) {
					m_mapBusitypeAppriveDrive
							.put(strBiztypeId, UFBoolean.FALSE);
				} else {
	...
	private InvoiceVO getCurVOonCard() {
		return (InvoiceVO) getBillCardPanel().getBillValueVO(
				InvoiceVO.class.getName(), InvoiceHeaderVO.class.getName(),
				InvoiceItemVO.class.getName());
	}
	...
	public InvokeEventProxy getInvokeEventProxy() {
		if (invokeEventProxy == null) {
			invokeEventProxy = new InvokeEventProxy("pu",
					BillTypeConst.PO_INVOICE, getPluginUI());
		}
	...
	private UFBoolean isRefFeeOrder() {
		InvoiceVO vo = getCurVOonCard();
		if (vo != null
				&& PuPubVO.getString_TrimZeroLenAsNull(vo.getHeadVO()
						.getCbiztype()) != null) {
			return isRefFeeOrder(PuPubVO.getString_TrimZeroLenAsNull(vo
					.getHeadVO().getCbiztype()));
		}
	...
	private UFBoolean isRefFeeOrder(String bizType) {
		if (!onlyRefFeeOrder.containsKey(bizType))
			onlyRefFeeOrder.put(bizType, new UFBoolean(InvoiceHelper
					.isPIOnlyRefFeeOrder(bizType)));
	...
	private void setNumEditable() {
		if (!isRefFeeOrder().booleanValue())
			return;
		int iCount = getBillCardPanel().getRowCount();
		for (int i = 0; i < iCount; i++) {
			if (ScmConst.PO_Order.equals(getBillCardPanel().getBodyValueAt(i,
					"cupsourcebilltype"))
					&& PuTool.isFee(PuPubVO
							.getString_TrimZeroLenAsNull(getBillCardPanel()
									.getBodyValueAt(i, "cbaseid")))) {
	...
	private void onSendAuditBatch() {
		...
		try {
			...
			Object[] retValues = PfUtilClient.runBatch(this, "SAVE",
					ScmConst.PO_Invoice, ClientEnvironment.getInstance()
							.getDate().toString(), vos, null, null, null);
			if (PfUtilClient.isSuccess()) {
				// 更新缓存及UI
	...
	private InvoiceVO[] getSendAuditVos() {
		...
		for (int row : getBillListPanel().getHeadTable().getSelectedRows()) {
			int status = (Integer) getBillListPanel().getHeadBillModel()
					.getValueAt(row, "ibillstatus");
			String billMaker = (String) getBillListPanel().getHeadBillModel()
					.getValueAt(row, "coperator");
			if ((BillStatus.FREE.intValue() == status || BillStatus.AUDITFAIL
					.intValue() == status)
					&& getCurOperator().equals(billMaker)) {
				InvoiceVO vo = new InvoiceVO();
	...
	private void setHeadInfoFrmList(InvoiceHeaderVO head, int row) {
		head.setCinvoiceid((String) getBillListPanel().getHeadBillModel()
				.getValueAt(row, "cinvoiceid"));
		head.setVinvoicecode((String) getBillListPanel().getHeadBillModel()
				.getValueAt(row, "vinvoicecode"));
		head.setIbillstatus((Integer) getBillListPanel().getHeadBillModel()
				.getValueAt(row, "ibillstatus"));
		head.setCoperator((String) getBillListPanel().getHeadBillModel()
				.getValueAt(row, "coperator"));
		head.setPk_corp((String) getBillListPanel().getHeadBillModel()
				.getValueAt(row, "pk_corp"));
		head.setCbiztype((String) getBillListPanel().getHeadBillModel()
				.getValueAt(row, "cbiztype"));
	}
	...
	private Map<String, Integer> getBillListIndexMap() {
		...
		for (int i = 0; i < getBillListPanel().getHeadBillModel().getRowCount(); i++)
			billsOnList.put((String) getBillListPanel().getHeadBillModel()
					.getValueAt(i, "cinvoiceid"), i);
		return billsOnList;
	}
	...
	private boolean isNeedLoadSrcInfo(BillModel model) {
		String[] checkItems = new String[] { "csourcebillcode",
				"csourcebillrowno", "cancestorbillcode", "cancestorbillrowno" };
		for (String key : checkItems)
			if (null != model.getItemByKey(key)
					&& model.getItemByKey(key).isShow())
				return true;
		return false;
	}
