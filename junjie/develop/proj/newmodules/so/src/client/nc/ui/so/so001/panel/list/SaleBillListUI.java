...
public abstract class SaleBillListUI extends SaleBillCardUI implements
	...
	protected String csaleid = null; // 当前选中单据ID
		...
		if (boCoRefPo == null) {
			boCoRefPo = new ButtonObject(nc.ui.ml.NCLangRes.getInstance()
					.getStrByID("sopub", "UPPsopub-000325")/*
															 * @res "参照协同采购订单"
															 */, nc.ui.ml.NCLangRes.getInstance().getStrByID("sopub",
					"UPPsopub-000325")/*
										 * @res "参照协同采购订单"
	...
	private void loadListTemplet() {
		...
		BillListData bd = new BillListData(billtempletVO);
		billtempletVO = null;
		...
		// 表体加背景色
		String[] tableCodes = getBillListPanel().getBillListData()
				.getBodyTableCodes();
		for (String tableCode : tableCodes) {
			getBillListPanel().getBodyTable(tableCode).setRowSelectionAllowed(
					true);
			getBillListPanel().getBodyTable(tableCode)
	...
	private void setListPanelByPara(BillListData bdData) {
		...
		try {
			...
			/*
			 * if (SA_16 != null && SA_16.booleanValue()) { if
			 * (bdData.getBodyItem("sharecode2", "cadvisecalbody") != null)
			 * bdData.getBodyItem("sharecode2", "cadvisecalbody").setShow(
			 * true); else bdData.getBodyItem("cadvisecalbody").setShow(true); }
			 * else { if (bdData.getBodyItem("sharecode2", "cadvisecalbody") !=
			 * null) bdData.getBodyItem("sharecode2", "cadvisecalbody").setShow(
			 * false); else bdData.getBodyItem("cadvisecalbody").setShow(false); }
			 */
		...
		try {
			...
			/*
			 * if (defs != null) bdData.updateItemByDef(defs, "vdef", true); //
			 * 表体 // 查得对应于公司的该单据的自定义项设置 defs = body_defs;
			 * 
			 * if (defs != null) bdData.updateItemByDef(defs, "vdef", false);
			 */
			// 列表
			DefSetTool.updateBillListPanelUserDef(getBillListPanel(),
					getClientEnvironment().getCorporation().getPk_corp(),
					SaleBillType.SaleOrder, "vdef", "vdef");

			getBillListPanel().setListData(bdData);
	...
	protected void switchInterface() {
		if (strShowState.equals("列表")) { /*-=notranslate=-*/
			...
			getBillCardTools().setBodyItemEnable(
					SOBillCardTools.getSaleOrderItems_Price_Mny_NoOriginal(),
					false);
			// 隐藏捆绑
	...
	protected SOBillQueryDlg getQueryDlg(String pk_corp) {
		if (dlgQuery == null) {
			...
			dlgQuery.addCurToCurDate("so_sale.dbilldate");
		}
	...
	protected void updateCacheVO() {
		if (strShowState.equals("列表")) { /*-=notranslate=-*/
			...
			try {
				 ...
				 * saleorder.getBodyVOs()[0].getCcurrencytypeid(), new String[] {
			...
			} // 联查->双击销售单进入为卡片模板(并非有列表->卡片)，所以num为初始值-1，没有被赋值0 liuzwc
			// 2009-06-09
			else if (getBillListPanel().getHeadBillModel().getRowCount() == 1) {
				hvo = vocache.getSaleOrderVO(hvo.getCsaleid()).getHeadVO();
				getBillListPanel().getHeadBillModel().setBodyRowVO(hvo, 0);
			}
		}
	...
	private ArrayList getMustUpdateBodyNames() {
		...
		String[] heads = new String[] { "csaleid", "corder_bid", "frowstatus",
				...
				"ntranslossnum", "ntotalreceivenumber", "ntotalinvoicenumber",
	...
	protected ButtonObject getBoSplitPrint() {
		if (boSplitPrint == null) {
			boSplitPrint = new ButtonObject(
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
							"UPT40060301-000595")/* @res "分单打印" */,
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
							"UPT40060301-000595")/* @res "分单打印" */, 0, "分单打印"); /*-=notranslate=-*/
		}
		return boSplitPrint;
	}
	...
	protected ButtonObject getBoAuditFlowStatus() {
		if (boAuditFlowStatus == null) {
			boAuditFlowStatus = new ButtonObject(
					...
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
							"UPT40060301-000025")/* @res "审批流状态" */, 0,
					"审批流状态");
	...
	protected ButtonObject getBoPasteLineToTail() {
		if (boPasteLineToTail == null) {
			boPasteLineToTail = new ButtonObject(
					nc.ui.ml.NCLangRes.getInstance().getStrByID("common",
							"SCMCOMMON000000266")/* @res "粘贴行到表尾" */,
					nc.ui.ml.NCLangRes.getInstance().getStrByID("common",
							"SCMCOMMON000000266")/* @res "粘贴行到表尾" */, 0,
					"粘贴行到表尾"); /*-=notranslate=-*/
		}
	...
	protected ButtonObject getBoSendInv() {
		if (boSendInv == null) {
			boSendInv = new ButtonObject(
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
							"UPT40060301-000571")/* @res "发货安排" */,
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
	...
	protected ButtonObject getBoSupplyInv() {
		if (boSupplyInv == null) {
			boSupplyInv = new ButtonObject(
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
							"UPT40060301-000572")/* @res "补货安排" */,
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
	...
	protected ButtonObject getBoDirectInv() {
		if (boDirectInv == null) {
			boDirectInv = new ButtonObject(
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
							"UPT40060301-000573")/* @res "直运安排" */,
					nc.ui.ml.NCLangRes.getInstance().getStrByID("40060301",
	...
	// 何意求 增加 10-09-15 start
	protected ButtonObject getBoMdmx() {
		if (boMdmx == null) {
			boMdmx = new ButtonObject("码单锁定", "码单锁定", "码单锁定");
		}
		return boMdmx;
	}
	// 何意求 增加 10-09-15 end
