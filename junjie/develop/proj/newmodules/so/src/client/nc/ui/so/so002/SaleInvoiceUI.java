...
public class SaleInvoiceUI extends ToftPanel implements
  ...
  private boolean UnitePrintFlag = false;  //联合打印标志 add by QuSida 2010-8-7 （顺德骏杰）
...
// getBillListPanel().bodyRowChange(new BillEditEvent(getBillListPanel().getHeadTable(),-1,getVOCache().getPos()));
  ...
  private void initialize() {
    ...
    getBtns().m_boPrintManage.addChildButton(m_boUnitePrint); //合并打印 add by QuSida 2010-8-6 10:24:33 (顺德骏杰)
    setButtonsStateBrowse();
 ...
 */
    ...
    try {
      ...
      if (getBillCardPanel().isNewBill()) {
        newinvoicevo = (SaleinvoiceVO) PfUtilClient
            .processActionNoSendMessage(this, ISaleInvoiceAction.PreSave,
  ...
  private boolean onApprove(SaleinvoiceVO saleinvoice) throws Exception {
    ...
    try {
      if (SaleInvoiceTools.getLoginDate().after(saleinvoice.getHeadVO().getDbilldate()))
  ...
  public void onButtonClicked(nc.ui.pub.ButtonObject bo) {
    ...
    // 打印
      ...
      // 合并显示
        ...
        onBillCombin();
      //合并打印  add by QuSida 2010-8-6 10:27:43 （顺德骏杰）
