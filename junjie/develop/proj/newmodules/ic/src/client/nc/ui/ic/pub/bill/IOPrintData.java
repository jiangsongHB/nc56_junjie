...
public class IOPrintData implements IDataSource {
	...
	/*
	 * 根据卡片界面得到所有的数据项名称数组 也就是返回所有定义的数据项的名称
	 */
	...
	/*
	 * 返回所有的数据项对应的内容 参数： 数据项的名字 返回： 数据项对应的内容，只能为 String[]； 如果 sItemExpress
	 * 拥有依赖项，则： 1 个依赖项：打印系统将根据依赖项的内容的顺序来判断 String[] 中的存放的数据 2
			...
			for (int i = 0; i < iHLen; i++) {
				...
				if (itemH.getKey().trim().equals(sItemExpress.trim())) {
					if (sItemExpress.trim().equals("freplenishflag")) {
						String flag = m_pnlCard.getHeadItem("freplenishflag")
								.getValue().toString();
		...
		if (!bFound && m_pnlCard.getBodyItems() != null) {
			...
			for (int i = 0; i < iBLen; i++) {
				...
				if (m_pnlCard.getBodyItems()[i].getKey().trim().equals(
					...
					// Object total =
					// m_pnlCard.getTotalTableModel().getValueAt(0,
					// m_pnlCard.getBillModel().getBodyColByKey(sItemExpress));
					// // vecValue.addElement("");
					// if (total == null) {
					// vecValue.addElement("--------");
					// }
			...
			/**
			 * add by ouyangzhb 2010-11-19 begin 功能：在订单维护的打印模板中增加费用信息的打印，
			...
			if (sItemExpress.equalsIgnoreCase("i_sumcostprice")
					|| sItemExpress.equalsIgnoreCase("i_sumcostmny")) {
...
//				}
			...
			if (sItemExpress.equalsIgnoreCase("i_ccostunitid"))
				for (int y = 0; y < inforcost.length; y++) {
					if (y % 2 == 0)
						vecValue.addElement(m_pnlCard.getBillData()
								.getBillModel("jj_scm_informationcost")
								.getValueAt(y, "costunit"));
				}
			...
			if (sItemExpress.equalsIgnoreCase("i_nnumber"))
				for (int y = 0; y < inforcost.length; y++) {
					if (y % 2 == 0)
						vecValue.addElement(inforcost[y].getNnumber()
								.toString());
				}
			...
			/*
			 * add by ouyangzhb 2012-04-09 新增费用行折行打印 add by ouyangzhb end
			 */
			...
			if (getMdcrkVO() != null && getMdcrkVO().length > 0) {
				if (sItemExpress.indexOf("m_") == 0) {
					for (int i = 0; i < mdcrkvos.length; i++) {
						vecValue.addElement(mdcrkvos[i]
								.getAttributeValue(sItemExpress.substring(2,
										sItemExpress.length())));
					}
				}
	...
	/*
	 * add by ouyangzhb 2012-05-28 获取要打印的码单信息
	 */
	...
	public MdcrkVO[] getMdcrkVO() {
		if (mdcrkvos == null) {
			...
			String mdsql = "select * from nc_mdcrk k where k.cgeneralbid in (select b.cgeneralbid from ic_general_b b where b.cgeneralhid='"
					+ cgeneralhid + "')";
			try {
				mdcrkvolist = (ArrayList<MdcrkVO>) iuapquerybs.executeQuery(
						mdsql, new BeanListProcessor(MdcrkVO.class));
