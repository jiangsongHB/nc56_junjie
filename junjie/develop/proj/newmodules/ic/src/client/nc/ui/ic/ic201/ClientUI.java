   ...
   getBoInfoCost().setEnabled(false);
	...
	protected void afterBillEdit(nc.ui.pub.bill.BillEditEvent e) {
		...
		if("ninnum".equalsIgnoreCase(sItemKey)||"ninassistnum".equalsIgnoreCase(sItemKey)){//2010-10-14 MeiChao 增加
		    ...
		    //function 当入库数量修改后及时更新费用信息中的数量 //2010-11-02 MeiChao 注释掉. 问题0000041
...
//		    		pmny = (UFDouble) ((GeneralButtonManager)getButtonManager()).getLmny().get(i);
//		    	}
		...
		if (c.isCloseOK()) {
			...
			if (infoCostVOs != null && infoCostVOs.length != 0){
				...
				for (int i = 0; i < infoCostVOs.length; i++) {
					//infoCostVOs[i].setNnumber(number); //2010-11-02 MeiChao 取消对费用信息中存货数量的赋值
